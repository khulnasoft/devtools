...
* Watch the repository on [GitHub](https://github.com/khulnasoft/devtools/subscription)
...
```sh
docker run --name devdocs -d -p 9292:9292 ghcr.io/khulnasoft/devtools:latest
```

This will start DevDocs at [localhost:9292](http://localhost:9292). We provide both regular and Alpine-based images:
- `ghcr.io/khulnasoft/devtools:latest` - Standard image
- `ghcr.io/khulnasoft/devtools:latest-alpine` - Alpine-based (smaller size)
...
```sh
git clone https://github.com/khulnasoft/devtools.git && cd devdocs
...
**Note:** there is currently no update mechanism other than `git pull origin main` to update the code and `thor docs:download --installed` to download the latest version of the docs. To stay informed about new releases, be sure to [watch](https://github.com/khulnasoft/devtools/subscription) this repository.
...
Copyright 2013–2024 Thibaut Courouble and [other contributors](https://github.com/khulnasoft/devtools/graphs/contributors)
