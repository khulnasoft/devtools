#!/usr/bin/env node
/* eslint-disable @devtools/no-undeclared-imports */
/*
...
/**
 * This script creates a release on GitHub for the Devtools repository.
 ...
 *
 * This will open the git tree at this tag https://github.com/khulnasoft/devtools/tree/v0.4.1
 * It will identify https://github.com/khulnasoft/devtools/pull/3668 as the responsible changeset PR.
...
const GH_OWNER = 'devtools';
const GH_REPO = 'devtools';
const EXPECTED_COMMIT_MESSAGE = /^Merge pull request #(?<prNumber>[0-9]+) from/;
const CHANGESET_RELEASE_BRANCH = 'devtools/changeset-release/master';
...
async function getCommitUsingTagName(tagName) {
  ...
  console.log(
    `The commit for the tag is https://github.com/khulnasoft/devtools/commit/${commitSha}`,
  );
  ...
  // Merge pull request #3555 from devtools/changeset-release/master Version Packages
...
async function getReleaseDescriptionFromCommit(commit) {
  ...
  if (pullRequests.length === 1) {
    ...
    console.log(
      `Identified the changeset Pull request - https://github.com/khulnasoft/devtools/pull/${prNumber}`,
    );
