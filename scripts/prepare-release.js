 ...
 */
async function updateDevtoolsReleaseVersion(repo, type) {
  ...
  const hasCreateAppChanges = changesets
    ...
    .some(changeset =>
      changeset.releases.some(
        release => release.name === '@devtools/create-app',
      ),
    );
...
async function main() {
  ...
  await updateDevtoolsReleaseVersion(repo, isMainBranch ? 'minor' : 'patch');
}
