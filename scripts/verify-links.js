...
async function verifyUrl(basePath, absUrl, docPages) {
  const url = absUrl
    .replace(/#.*$/, '')
    .replace(
      /https:\/\/github.com\/devtools\/devtools\/(tree|blob)\/master/,
      '',
    );
  ...
  if (
    absUrl.match(
      /https:\/\/github.com\/devtools\/devtools\/(tree|blob)\/master\/docs\//,
    ) &&
  ...
  if (badUrls.length) {
    ...
    for (const { url, basePath, problem } of badUrls) {
      if (problem === 'missing') {
        ...
        console.error(
          `  Likely replace with: https://github.com/khulnasoft/devtools/blob/master/${url.replace(
            /^[./]+/,
            '',
          )}`,
        );
