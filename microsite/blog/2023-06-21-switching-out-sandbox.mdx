...
**TL;DR**: For the Devtools maintainers, ensuring that the project is highly secure for every adopter and end-user is one of our top priorities. With the recent vulnerabilities in the `vm2` sandbox we have decided to move away from `vm2` and use `isolated-vm` instead, in order to ensure the security of the Devtools end-users.
...
When using [Software Templates](https://devtools.khulnasoft.com/docs/features/software-templates), we allow template authors to [write advanced templating strings](https://devtools.khulnasoft.com/docs/features/software-templates/writing-templates/#the-templating-syntax) when composing actions together like the following:
...
Under the hood these templating strings compile into JavaScript. The [Devtools Threat Model](https://devtools.khulnasoft.com/docs/overview/threat-model/) treats the code provided by the template language as untrusted, so adopters need a safe environment – a sandbox – to execute the JavaScript in.
Up until the latest Devtools release, that sandbox was `vm2`. Now, the project is instead using `isolated-vm` as the sandbox to execute this JavaScript in.
...
Both the `vm2` maintainers and the Devtools maintainers have been quick at releasing new versions of the sandbox and the Software Templates backend plugin, respectively, to protect their users.
...
## What does this mean for Devtools?
...
For those who compile and run Devtools on stripped-down environments, you will want to ensure that you have the build basics present, e.g. `build-essential` or similar corresponding to your operating system of choice.
...
This change comes with the v1.15.0 release of Devtools that was released yesterday, so please make sure you are upgraded to the latest version of Devtools to keep your sandbox environments secure.

For more guidance on how to upgrade, check out the documentation for [keeping Devtools updated](https://devtools.khulnasoft.com/docs/getting-started/keeping-devtools-updated).

If you have any further questions you can either reach out to us in the [Community Discord](https://discord.gg/devtools-687207715902193673), or in the [office hours](https://info.devtools.spotify.com/office-hours).
