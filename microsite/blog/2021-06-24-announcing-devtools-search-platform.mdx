---
# prettier-ignore
title: "Announcing the Devtools Search platform: a customizable search tool built just for you"
author: Emma Indal, Spotify
authorURL: https://www.linkedin.com/in/emma-indal
---

![Devtools Search platform](assets/21-06-24/devtools-search-platform.png)

**TLDR;** The new Devtools Search is now available in alpha, ready for you to start building on. A total rethinking of the core search feature in Devtools, it’s more than just a box you type into — it’s a mini platform all by itself. With its composable frontend and extensible backend, you can design and build the search tool that suits your organization’s needs.

So, you don’t just get an improved out-of-the-box experience for searching whatever is in your software catalog. You can also add support for searching other sources, too. Customize it the way you want and you can search your catalog, your plugins and docs — and even external sources, like Stack Overflow and Confluence — all at once, all right inside Devtools.
...
Being able to easily explore your ecosystem — to discover software, tools, documentation, and other valuable knowledge — is one of [the three main jobs of Devtools](https://devtools.khulnasoft.com/blog/2021/05/20/adopting-devtools#three-jobs-create-manage-explore). Teams should be able to find what other teams have already built, so they can reuse and contribute to components instead of unknowingly duplicating them. Data endpoints should be shared, not siloed away. Services and their APIs should be easily discoverable. Best practices and technical documentation should be easily found.

Along with the [Devtools Service Catalog](https://devtools.khulnasoft.com/blog/2020/06/22/devtools-service-catalog-alpha), Devtools Search is essential to enabling this discoverability — allowing new hires and old hands alike to explore your infrastructure instead of getting lost inside it.
...
Since finding what you are looking for in Devtools is critical for success, we started by identifying the needs and goals of search.

## Rethinking search, inside and out

Spotify’s internal version of Devtools has had some of the features of Devtools Search for a while, and open sourcing them has been top of mind since day one. But we didn’t want to just port our internal version to the open source version. We wanted to take the opportunity to apply what we’ve learned inside Spotify over the last year, address the needs we’ve observed in the community, and ultimately open source not just a search feature but a search platform. We started the process by looking at the [jobs to be done](https://hbr.org/2016/09/know-your-customers-jobs-to-be-done).

![Devtools Search platform](assets/21-06-24/jobs-to-be-done.png)
...
## Say hello to the Devtools Search platform

![Devtools Search platform](assets/21-06-24/search-results.png)

We are now happy and proud to announce our alpha version of the [Devtools Search Platform](https://devtools.khulnasoft.com/docs/features/search/architecture), featuring:
...
With the Search Integration Layer, your organization can bring your search engine of choice to Devtools — instead of relying on Devtools to support a specific search engine that might not fit the needs of your organization, either today or in the future.

But that doesn’t mean “batteries not included”. The current version of Devtools Search ships with Lunr support built-in — and support for ElasticSearch is not very far off. And we hope the number of supported search engines will continue to grow with the community’s help.

### Collators for easily indexing content from plugins and other sources

Since Devtools’s functionality comes from its plugins, we wanted the process of making plugin content searchable to be as frictionless as possible. Therefore we decided on a concept we call collators. Collators are responsible for collecting documents to index from a plugin. Your collators live inside your own plugin, but are registered in the Devtools app’s search backend.
...
That's why we have put effort into making your search page experience composable to your organization's needs. What do we mean by that? When you adopt Devtools and set up your app, you can set up — or, compose — your search page by using existing components or by creating your own custom ones.
...
## Getting started with Devtools Search

We put together [a getting started guide](https://devtools.khulnasoft.com/docs/features/search/getting-started) that provides two different ways to set up Devtools Search:

- Create a new app and get the most out of the search setup right out of the box, or
- Add the new Devtools Search setup to your existing Devtools app.

Whichever situation you’re in, we have you covered.

## What’s next?

We’ve built the foundation for the Devtools Search platform, and we can't wait to see the exciting engines, collators, and components the community builds on the platform.

You can check out our [project roadmap](https://devtools.khulnasoft.com/docs/features/search/#project-roadmap) in our search documentation or track the progress of our [Beta milestone](https://github.com/khulnasoft/devtools/milestone/27) and [GA milestone](https://github.com/khulnasoft/devtools/milestone/28).

For any questions, feedback or ideas about the Devtools Search platform, join us in the #search channel on [Discord](https://discord.gg/devtools-687207715902193673)!
