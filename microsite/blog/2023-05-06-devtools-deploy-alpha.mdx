---
title: 'Welcome: Devtools Deploy CLI (Alpha)'
...
![Devtools Deploy banner](assets/2023-05-06/Welcome-Devtools-Deploy.png)

TL;DR: Devtools released a new CLI earlier this year as part of a [new Devtools project](https://github.com/khulnasoft/devtools-deploy). This new capability allows early adopters to deploy their Devtools proof of concept faster than before. In this blog, you’ll learn a little bit more about the Deploy CLI and how you can get started with it.

{/* truncate */}

## Why Devtools Deploy

We’ve heard from Devtools adopters that before reaching widespread adoption at their organizations, teams test out a Devtools proof of concept (POC). To create the POC, teams scaffold an instance, set up the catalog, and change the UI to suit their company’s branding. Once these milestones are completed, there's just one last step before showing off the awesomeness of a Devtools POC internally... deploying the Devtools POC from a local machine to the cloud. This final step often introduces a lot of questions that attempt to determine the easiest and fastest way to do that.
...
To address these questions and reduce friction from deploying a POC, we built a [new CLI](https://github.com/khulnasoft/devtools-deploy). The CLI is simply called `deploy` and is invokable with `npx`. With this new CLI, you can generate a Dockerfile and deploy a Devtools instance onto a preferred cloud provider. While the package’s infrastructure is built to support all cloud providers, currently the CLI only offers an AWS implementation. In the future, we plan to add additional cloud providers to the package and [welcome any contributions](https://github.com/khulnasoft/devtools/blob/6996b3338d678efc03307112524060e9dc2ad769/CONTRIBUTING.md) extending the suite of cloud provider implementations!
...
It's now time to put the Deploy CLI into action and deploy a Devtools POC. The following steps guide you through the deployment process.
...
You can find the step-by-step installation documentation for Pulumi under the [Deployment section](https://devtools.khulnasoft.com/docs/deployment/devtools-deploy/aws-lightsail).

But we summarize the steps to deploy your POC here. You can invoke the CLI you can use the following command:

```bash
$ npx devtools-deploy aws --create-dockerfile
Starting Pulumi
...
https://devtools-container-service.xxx.us-east-1.cs.amazonlightsail.com/
```
...
- [Devtools Deploy](https://github.com/khulnasoft/devtools-deploy)
- [Documentation](https://devtools.khulnasoft.com/docs/deployment/devtools-deploy/aws-lightsail)
