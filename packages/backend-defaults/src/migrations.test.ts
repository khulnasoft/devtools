...
describe('migrations', () => {
  ...
  it.each(databases.eachSupportedId())(
    ...
    async databaseId => {
      ...
      await knex('devtools_backend_tasks__tasks').insert({
        id: 'test',
        settings_json: '{}',
        next_run_start_at: knex.fn.now(),
      });

      await expect(knex('devtools_backend_tasks__tasks')).resolves.toEqual([
        {
          id: 'test',
          settings_json: '{}',
      ...
      await expect(knex('devtools_backend_tasks__tasks')).rejects.toEqual(
        expect.anything(),
      );
  ...
  it.each(databases.eachSupportedId())(
    ...
    async databaseId => {
      ...
      await expect(
        knex('devtools_backend_tasks__tasks').insert({
          id: 'test',
          settings_json: '{}',
          next_run_start_at: knex.raw('null'),
        }),
