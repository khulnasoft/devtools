...
import { ServiceFactoryTester } from '@devtools/backend-test-utils';
...
describe('schedulerFactory', () => {
  it('creates sidecar database features', async () => {
    ...
    await expect(
      client.from('devtools_backend_tasks__tasks').count(),
    ).resolves.toEqual([{ 'count(*)': 1 }]);
    await expect(
      client.from('devtools_backend_tasks__knex_migrations').count(),
    ).resolves.toEqual([{ 'count(*)': expect.any(Number) }]);
    await expect(
      client.from('devtools_backend_tasks__knex_migrations_lock').count(),
    ).resolves.toEqual([{ 'count(*)': expect.any(Number) }]);
  });
});
