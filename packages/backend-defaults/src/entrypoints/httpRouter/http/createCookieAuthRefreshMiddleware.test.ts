...
import { mockCredentials, mockServices } from '@devtools/backend-test-utils';
...
describe('createCookieAuthRefreshMiddleware', () => {
  ...
  it('should issue the user cookie', async () => {
    const response = await request(app).get('/.devtools/auth/v1/cookie');
    expect(response.status).toBe(200);
    expect(response.header['set-cookie'][0]).toMatch(
      `devtools-auth=${mockCredentials.limitedUser.token()}`,
    );
  });

  it('should remove the user cookie', async () => {
    const response = await request(app).delete('/.devtools/auth/v1/cookie');
    expect(response.status).toBe(204);
    expect(response.header['set-cookie'][0]).toMatch('devtools-auth=');
  });
});
