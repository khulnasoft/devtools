...
/**
 * Handles all types of external caller token types (i.e. not Devtools user
 * tokens, nor Devtools backend plugin tokens).
 *
 * @internal
 */
export class ExternalTokenHandler {
  static create(options: {
    ...
    if (legacyConfigs.length && !loggedDeprecationWarning) {
      ...
      logger.warn(
        `DEPRECATION WARNING: The ${OLD_CONFIG_KEY} config has been replaced by ${NEW_CONFIG_KEY}, see https://devtools.khulnasoft.com/docs/auth/service-to-service-auth`,
      );
    }
