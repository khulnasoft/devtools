...
import { toInternalDevtoolsCredentials } from './helpers';
...
describe('authServiceFactory', () => {
  ...
  it('should not support tokens issued with legacy auth', async () => {
    server.use(
      rest.get(
        'http://localhost:7007/api/catalog/.devtools/auth/v1/jwks.json',
        (_req, res, ctx) => res(ctx.status(404)),
      ),
      rest.get(
        'http://localhost:7007/api/search/.devtools/auth/v1/jwks.json',
        (_req, res, ctx) => res(ctx.status(404)),
      ),
    );
  ...
  it('should issue a service token for the new system even if the target plugin does not support it', async () => {
    server.use(
      rest.get(
        'http://localhost:7007/api/permission/.devtools/auth/v1/jwks.json',
        (_req, res, ctx) => res(ctx.status(404)),
      ),
    );
  ...
  it('should issue service on-behalf-of user tokens', async () => {
    ...
    */
    server.use(
      ...
      rest.get(
        'http://localhost:7007/api/kubernetes/.devtools/auth/v1/jwks.json',
        (_req, res, ctx) => res(ctx.status(404)),
      ),
    );
    ...
    expect(toInternalDevtoolsCredentials(oboCredentials).token).toBe(
      limitedToken,
    );
