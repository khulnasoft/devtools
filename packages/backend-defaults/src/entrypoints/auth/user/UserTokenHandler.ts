...
/**
 * An identity client to interact with auth-backend and authenticate Devtools
 * tokens
 *
 * @internal
 */
export class UserTokenHandler {
  ...
  createLimitedUserToken(devtoolsToken: string) {
    const [headerRaw, payloadRaw] = devtoolsToken.split('.');
    ...
    if (!tokenType || tokenType === tokenTypes.limitedUser.typParam) {
      return { token: devtoolsToken, expiresAt: new Date(payload.exp * 1000) };
    }
