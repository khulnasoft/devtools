...
describe('UserTokenHandler', () => {
  ...
  describe('verifyToken', () => {
    ...
    it('should verify a valid legacy devtools token', async () => {
      const expectedIssuedAt = 1712071714;
      const expectedExpiresAt = 1712075314;
  ...
  describe('createLimitedUserToken', () => {
    it('should return the original token if it a legacy devtools token', async () => {
      const devtoolsToken = await createToken({
        // Without header.typ param
        header: { alg: 'ES256' },
        payload: {},
      });
      const result = userTokenHandler.createLimitedUserToken(devtoolsToken);
    ...
    it('should return the original token if it is already a limited user token', async () => {
      const devtoolsToken = await createToken({
        header: { typ: 'vnd.devtools.user', alg: 'ES256' },
      ...
      expect(result).toEqual({
        token: await createToken({
          header: { typ: 'vnd.devtools.limited-user', alg: 'ES256' },
          payload: { sub: 'mock' },
          signature: 'proof',
        }),
        expiresAt: expect.any(Date),
    ...
    it('should throw an AuthenticationError if the token type is invalid', async () => {
      ...
      expect(() => {
        userTokenHandler.createLimitedUserToken(devtoolsToken);
      }).toThrow(
