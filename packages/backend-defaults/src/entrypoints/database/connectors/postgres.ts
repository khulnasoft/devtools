...
import { LifecycleService, LoggerService } from '@devtools/backend-plugin-api';
...
export class PgConnector implements Connector {
   ...
   * which is the pluginId prefixed with 'devtools_plugin_'. If `pluginDivisionMode` is
  ...
  private getConnectionConfig(pluginId: string): Knex.StaticConnectionConfig {
    ...
    ).application_name ||= `devtools_plugin_${pluginId}`;
