...
describe('rootHttpRouterServiceFactory', () => {
  it('should make the health endpoints available', async () => {
    ...
    await request(app)
      .get('/.devtools/health/v1/liveness')
      .expect(200, { status: 'ok' });

    await request(app).get('/.devtools/health/v1/readiness').expect(503, {
    ...
    await request(app).get('/.devtools/health/v1/readiness').expect(200, {
      status: 'ok',
    });
  ...
  it('should include custom headers for health endpoint', async () => {
    ...
    const lRes = await request(app).get('/.devtools/health/v1/liveness');

    expect(lRes.status).toBe(200);
    expect(lRes.get('x-test-header')).toBe('test');

    const r1Res = await request(app).get('/.devtools/health/v1/readiness');
    ...
    const r2Res = await request(app).get('/.devtools/health/v1/readiness');
