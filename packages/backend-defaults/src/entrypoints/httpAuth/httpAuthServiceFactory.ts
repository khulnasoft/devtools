...
import { AuthenticationError, NotAllowedError } from '@devtools/errors';
import { parse as parseCookie } from 'cookie';
import { Request, Response } from 'express';

const FIVE_MINUTES_MS = 5 * 60 * 1000;

const DEVTOOLS_AUTH_COOKIE = 'devtools-auth';
...
function getCookieFromRequest(req: Request) {
  ...
  if (cookieHeader) {
    ...
    const token = cookies[DEVTOOLS_AUTH_COOKIE];
    if (token) {
      return token;
    }
  }
...
const credentialsSymbol = Symbol('devtools-credentials');
const limitedCredentialsSymbol = Symbol('devtools-limited-credentials');

type RequestWithCredentials = Request & {
  [credentialsSymbol]?: Promise<DevtoolsCredentials>;
  [limitedCredentialsSymbol]?: Promise<DevtoolsCredentials>;
};
...
export class DefaultHttpAuthService implements HttpAuthService {
  ...
  async credentials<TAllowed extends keyof DevtoolsPrincipalTypes = 'unknown'>(
    req: Request,
    options?: {
  ...
  async issueUserCookie(
    ...
    let credentials: DevtoolsCredentials<DevtoolsUserPrincipal>;
    if (options?.credentials) {
      if (this.#auth.isPrincipal(options.credentials, 'none')) {
        res.clearCookie(
          DEVTOOLS_AUTH_COOKIE,
    ...
    res.cookie(DEVTOOLS_AUTH_COOKIE, token, {
      ...(await this.#getCookieOptions(res.req)),
      expires: expiresAt,
    });
  ...
  async #getCookieOptions(_req: Request): Promise<{
    ...
    // once https://github.com/khulnasoft/devtools/issues/24169 has landed
 ...
 *
 * See {@link @devtools/code-plugin-api#HttpAuthService}
