...
import { NotModifiedError, NotFoundError } from '@devtools/errors';
import {
  GitLabIntegration,
...
describe('GitlabUrlReader', () => {
  ...
  describe('readTree', () => {
    ...
    beforeEach(() => {
      worker.use(
        rest.get(
          'https://gitlab.com/api/v4/projects/devtools%2Fmock/repository/archive',
          (_, res, ctx) =>
            res(
              ctx.status(200),
        ...
        rest.get(
          'https://gitlab.com/api/v4/projects/devtools%2Fmock',
          (_, res, ctx) =>
            res(
              ctx.status(200),
        ...
        rest.get(
          'https://gitlab.com/api/v4/projects/devtools%2Fmock/repository/commits',
          (req, res, ctx) => {
        ...
        rest.get(
          'https://gitlab.mycompany.com/api/v4/projects/devtools%2Fmock',
          (_, res, ctx) =>
            res(
              ctx.status(200),
              ctx.set('Content-Type', 'application/json'),
        ...
        rest.get(
          'https://gitlab.mycompany.com/api/v4/projects/devtools%2Fmock/repository/commits',
          (req, res, ctx) => {
            const refName = req.url.searchParams.get('ref_name');
            if (refName === 'main') {
              const filepath = req.url.searchParams.get('path');
        ...
        rest.get(
          'https://gitlab.mycompany.com/api/v4/projects/devtools%2Fmock/repository/archive',
          (_, res, ctx) =>
            res(
              ctx.status(200),
              ctx.set('Content-Type', 'application/zip'),
    ...
    it('returns the wanted files from an archive', async () => {
      const response = await gitlabProcessor.readTree(
        'https://gitlab.com/devtools/mock/tree/main',
      );
    ...
    it('creates a directory with the wanted files', async () => {
      const response = await gitlabProcessor.readTree(
        'https://gitlab.com/devtools/mock',
      );
    ...
    it('returns the wanted files from hosted gitlab', async () => {
      worker.use(
        rest.get(
          'https://gitlab.mycompany.com/devtools/mock/-/archive/main.tar.gz',
          (_, res, ctx) =>
            res(
              ctx.status(200),
              ctx.set('Content-Type', 'application/zip'),
      ...
      const response = await hostedGitlabProcessor.readTree(
        'https://gitlab.mycompany.com/devtools/mock/tree/main/docs',
      );
    ...
    it('returns the wanted files from an archive with a subpath', async () => {
      const response = await gitlabProcessor.readTree(
        'https://gitlab.com/devtools/mock/tree/main/docs',
      );
    ...
    it('throws a NotModifiedError when given a etag in options matching last commit', async () => {
      const fnGitlab = async () => {
        await gitlabProcessor.readTree('https://gitlab.com/devtools/mock', {
          etag: 'sha123abc',
        });
      };
      ...
      const fnHostedGitlab = async () => {
        await hostedGitlabProcessor.readTree(
          'https://gitlab.mycompany.com/devtools/mock',
          {
            etag: 'sha123abc',
          },
        );
    ...
    it('throws a NotModifiedError when given a etag in options matching last commit affecting specified filepath', async () => {
      const fnGitlab = async () => {
        await gitlabProcessor.readTree(
          'https://gitlab.com/devtools/mock/blob/main/testFilepath',
          {
            etag: 'sha456def',
          },
        );
      ...
      const fnHostedGitlab = async () => {
        await hostedGitlabProcessor.readTree(
          'https://gitlab.mycompany.com/devtools/mock/blob/main/testFilepath',
          {
            etag: 'sha456def',
          },
        );
    ...
    it('should throw error on missing branch', async () => {
      const fnGitlab = async () => {
        await gitlabProcessor.readTree(
          'https://gitlab.com/devtools/mock/tree/branchDoesNotExist',
        );
      };
  ...
  describe('search', () => {
    ...
    it('works for the naive case', async () => {
      const result = await gitlabProcessor.search(
        'https://gitlab.com/devtools/mock/tree/main/**/index.*',
      );
      expect(result.etag).toBe('sha123abc');
      expect(result.files.length).toBe(1);
      expect(result.files[0].url).toBe(
        'https://gitlab.com/devtools/mock/tree/main/docs/index.md',
      );
    ...
    it('load only relevant path', async () => {
      const result = await gitlabProcessor.search(
        'https://gitlab.com/devtools/mock/tree/main/docs/**/index.*',
      );
