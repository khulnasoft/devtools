...
import { NotModifiedError } from '@devtools/errors';
...
describe('BitbucketUrlReader', () => {
  ...
  describe('readUrl', () => {
    it('should be able to readUrl via buffer without ETag', async () => {
      worker.use(
        rest.get(
          'https://api.bitbucket.org/2.0/repositories/devtools-verification/test-template/src/master/template.yaml',
          (req, res, ctx) => {
      ...
      const result = await bitbucketProcessor.readUrl(
        'https://bitbucket.org/devtools-verification/test-template/src/master/template.yaml',
      );
  ...
  describe('readTree', () => {
    ...
    beforeEach(() => {
      worker.use(
        ...
        rest.get(
          ...
          (_, res, ctx) =>
            res(
              ...
              ctx.set(
                'content-disposition',
                'attachment; filename=devtools-mock-12ab34cd56ef.tar.gz',
              ),
              ctx.body(repoBuffer),
            ),
        ),
        rest.get(
          'https://api.bitbucket.org/2.0/repositories/devtools/mock/commits/master',
          (_, res, ctx) =>
            res(
              ctx.status(200),
              ctx.json({
        ...
        rest.get(
          'https://api.bitbucket.mycompany.net/rest/api/1.0/projects/devtools/repos/mock/archive',
          (_, res, ctx) =>
            res(
              ctx.status(200),
              ctx.set('Content-Type', 'application/zip'),
              ctx.set(
                'content-disposition',
                'attachment; filename=devtools-mock.tgz',
              ),
              ctx.body(privateBitbucketRepoBuffer),
            ),
        ),
        rest.get(
          'https://api.bitbucket.mycompany.net/rest/api/1.0/projects/devtools/repos/mock/commits',
          (_, res, ctx) =>
            res(
              ctx.status(200),
              ctx.json({
    ...
    it('creates a directory with the wanted files', async () => {
      const response = await bitbucketProcessor.readTree(
        'https://bitbucket.org/devtools/mock',
      );
    ...
    it('returns the wanted files from an archive with a subpath', async () => {
      const response = await bitbucketProcessor.readTree(
        'https://bitbucket.org/devtools/mock/src/master/docs',
      );
  ...
  describe('search hosted', () => {
    ...
    it('works for the naive case', async () => {
      const result = await bitbucketProcessor.search(
        'https://bitbucket.org/devtools/mock/src/master/**/index.*',
      );
      expect(result.etag).toBe('12ab34cd56ef');
      expect(result.files.length).toBe(1);
      expect(result.files[0].url).toBe(
        'https://bitbucket.org/devtools/mock/src/master/docs/index.md',
      );
    ...
    it('works in nested folders', async () => {
      const result = await bitbucketProcessor.search(
        'https://bitbucket.org/devtools/mock/src/master/docs/index.*',
      );
      expect(result.etag).toBe('12ab34cd56ef');
      expect(result.files.length).toBe(1);
      expect(result.files[0].url).toBe(
  ...
  describe('search private', () => {
    ...
    it('works for the naive case', async () => {
      const result = await hostedBitbucketProcessor.search(
        'https://bitbucket.mycompany.net/projects/devtools/repos/mock/browse/**/index.*?at=master',
      );
      expect(result.etag).toBe('12ab34cd56ef');
      expect(result.files.length).toBe(1);
      expect(result.files[0].url).toBe(
        'https://bitbucket.mycompany.net/projects/devtools/repos/mock/browse/docs/index.md?at=master',
      );
      await expect(result.files[0].content()).resolves.toEqual(
        Buffer.from('# Test\n'),
      );
    ...
    it('works in nested folders', async () => {
      const result = await hostedBitbucketProcessor.search(
        'https://bitbucket.mycompany.net/projects/devtools/repos/mock/browse/docs/index.*?at=master',
      );
      expect(result.etag).toBe('12ab34cd56ef');
      expect(result.files.length).toBe(1);
      expect(result.files[0].url).toBe(
