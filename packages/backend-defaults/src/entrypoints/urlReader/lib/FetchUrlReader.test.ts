...
describe('FetchUrlReader', () => {
  ...
  beforeEach(() => {
    worker.use(
      rest.get('https://devtools.khulnasoft.com/some-resource', (req, res, ctx) => {
    ...
    worker.use(
      rest.get('https://devtools.khulnasoft.com/not-exists', (_req, res, ctx) => {
        return res(ctx.status(404));
      }),
    );

    worker.use(
      rest.get('https://devtools.khulnasoft.com/error', (_req, res, ctx) => {
  ...
  describe('read', () => {
    it('should return etag from the response', async () => {
      const { buffer } = await fetchUrlReader.readUrl(
        'https://devtools.khulnasoft.com/some-resource',
      );
    ...
    it('should throw NotFound if server responds with 404', async () => {
      await expect(
        fetchUrlReader.readUrl('https://devtools.khulnasoft.com/not-exists'),
      ).rejects.toThrow(NotFoundError);
    });

    it('should throw Error if server responds with 500', async () => {
      await expect(
        fetchUrlReader.readUrl('https://devtools.khulnasoft.com/error'),
  ...
  describe('readUrl', () => {
    it('should throw NotModified if server responds with 304 from etag', async () => {
      await expect(
        fetchUrlReader.readUrl('https://devtools.khulnasoft.com/some-resource', {
          etag: 'foo',
        }),
    ...
    it('should send Authorization header if token is provided', async () => {
      ...
      worker.use(
        rest.get(
          'https://devtools.khulnasoft.com/requires-authentication',
          (req, res, ctx) => {
