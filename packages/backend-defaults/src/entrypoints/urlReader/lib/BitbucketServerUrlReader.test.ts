  ...
  describe('readTree', () => {
    ...
    beforeEach(() => {
      worker.use(
        ...
        rest.get(
          'https://api.bitbucket.mycompany.net/rest/api/1.0/projects/devtools/repos/mock/branches',
          (_, res, ctx) =>
            res(
              ctx.status(200),
              ctx.json({
    ...
    it('uses default branch when no branch is provided', async () => {
      const response = await reader.readTree(
        'https://bitbucket.mycompany.net/projects/devtools/repos/mock/browse/src',
      );
