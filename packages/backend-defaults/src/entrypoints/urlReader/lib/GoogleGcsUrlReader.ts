...
/**
 * Implements a {@link @devtools/backend-plugin-api#UrlReaderService} for files on Google GCS.
 *
 * @public
 */
  ...
  static factory: ReaderFactory = ({ config, logger }) => {
    ...
    if (!gcsConfig.clientEmail || !gcsConfig.privateKey) {
      ...
      storage = new GoogleCloud.Storage({
        userAgent: `devtools/backend-defaults.GoogleGcsUrlReader/${packageinfo.version}`,
      });
    } else {
      storage = new GoogleCloud.Storage({
        credentials: {
