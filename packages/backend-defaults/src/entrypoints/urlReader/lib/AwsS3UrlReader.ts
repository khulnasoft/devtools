...
/**
 * Implements a {@link @devtools/backend-plugin-api#UrlReaderService} for AWS S3 buckets.
 *
 * @public
 */
export class AwsS3UrlReader implements UrlReaderService {
  ...
  private static async buildCredentials(
    ...
    if (roleArn) {
      return fromTemporaryCredentials({
        ...
        params: {
          RoleSessionName: 'devtools-aws-s3-url-reader',
          RoleArn: roleArn,
  ...
  private async buildS3Client(
    ...
    const s3 = new S3Client({
      customUserAgent: 'devtools-aws-s3-url-reader',
      region: region,
