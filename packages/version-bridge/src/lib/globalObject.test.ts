...
describe('getOrCreateGlobalSingleton', () => {
  beforeEach(() => {
    delete anyGlobal['__@devtools/my-thing__'];
  });

  it('should return an existing value', () => {
    const myThing = {};
    anyGlobal['__@devtools/my-thing__'] = myThing;
  ...
  it('should should create a new value', () => {
    ...
    expect(anyGlobal['__@devtools/my-thing__']).toBe(undefined);
    expect(getOrCreateGlobalSingleton('my-thing', () => myNewThing)).toBe(
      myNewThing,
    );
    expect(anyGlobal['__@devtools/my-thing__']).toBe(myNewThing);
