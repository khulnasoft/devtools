...
describe('Integration Test', () => {
  ...
  it('should throw an error when required external plugin routes are not bound', async () => {
    ...
    const expectedMessage =
      "External route 'extRouteRef1' of the 'blob' plugin must be bound to a target route. See https://devtools.khulnasoft.com/link?bind-routes for details.";
  ...
  describe('relative url resolvers', () => {
    it.each([
      ...
      [
        ['http://localhost/devtools', 'http://localhost/devtools'],
        {
          backend: {
            baseUrl: 'http://test.com/devtools',
          },
          app: {
            baseUrl: 'http://test.com/devtools',
          },
      ...
      [
        [
          'http://localhost/devtools/instance',
          'http://localhost/devtools/instance',
        ],
        {
          backend: {
            baseUrl: 'http://test.com/devtools/instance',
          },
          app: {
            baseUrl: 'http://test.com/devtools/instance',
          },
      ...
      [
        [
          'http://localhost/devtools/instance',
          'http://test.com/devtools/instance',
        ],
        {
          backend: {
            baseUrl: 'http://test.com/devtools/instance',
          ...
          app: {
            baseUrl: 'http://test-front.com/devtools/instance',
          },
        },
      ],
    ])(
  ...
  it('should clear app cookie when the user logs out', async () => {
    ...
    server.use(
      rest.delete(
        'http://localhost:7007/app/.devtools/auth/v1/cookie',
        (_req, res, ctx) => {
          logoutSignal();
    ...
    const Root = app.createRoot(
      <AppRouter>
        <meta name="devtools-app-mode" content="protected" />
        <SignOutButton />
      </AppRouter>,
    );
