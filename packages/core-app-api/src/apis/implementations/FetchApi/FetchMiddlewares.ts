...
import { DiscoveryApi, IdentityApi } from '@devtools/core-plugin-api';
...
export class FetchMiddlewares {
   ...
   * `https://devtools.example.net/api/catalog`, the resulting query would be
   * `https://devtools.example.net/api/catalog/entities?filter=x=y`.
   *
  ...
  /**
   * Injects a Devtools token header when the user is signed in.
   *
   * @remarks
   *
