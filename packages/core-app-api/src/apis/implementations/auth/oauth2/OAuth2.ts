    ...
    SessionApi
{
  static create(options: OAuth2CreateOptions) {
    ...
    const connector = new DefaultAuthConnector({
      ...
      sessionTransform({
        ...
        if (devtoolsIdentity) {
          session.devtoolsIdentity = {
            token: devtoolsIdentity.token,
            identity: devtoolsIdentity.identity,
            expiresAt: devtoolsIdentity.expiresInSeconds
              ? new Date(Date.now() + devtoolsIdentity.expiresInSeconds * 1000)
              : undefined,
          };
        }
        return session;
    ...
    const sessionManager = new RefreshingAuthSessionManager({
      ...
      sessionShouldRefresh: (session: OAuth2Session) => {
        // TODO(Rugvip): Optimize to use separate checks for provider vs devtools session expiration
        let min = Infinity;
        ...
        if (session.devtoolsIdentity?.expiresAt) {
          min = Math.min(
            min,
            (session.devtoolsIdentity.expiresAt.getTime() - Date.now()) / 1000,
          );
        }
        return min < 60 * 3;
      },
  ...
  async getDevtoolsIdentity(
    ...
    return session?.devtoolsIdentity;
  }
