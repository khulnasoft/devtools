...
import { DevtoolsRouteObject } from './types';
...
export const MATCH_ALL_ROUTE: DevtoolsRouteObject = {
  caseSensitive: false,
  path: '*',
...
const pluginSet = (
  ...
  const set = new Set<DevtoolsPlugin>();
  if (plugin) {
    set.add(plugin);
  }
  return set;
...
function collectSubTree(
  node: ReactNode,
  entries = new Array<{ routeRef: RouteRef; plugin?: DevtoolsPlugin }>(),
) {
  Children.forEach(node, element => {
    ...
    if (routeRef) {
      const plugin = getComponentData<DevtoolsPlugin>(element, 'core.plugin');
...
export const routingV2Collector = createCollector(
  () => ({
    ...
    objects: new Array<DevtoolsRouteObject>(),
  }),
  (acc, node, parent, ctx?: RoutingV2CollectorContext) => {
    ...
    if (pathProp !== undefined) {
      ...
      if (getComponentData<boolean>(node, 'core.gatherMountPoints')) {
        ...
        const newObj = {
          ...
          plugins: new Set<DevtoolsPlugin>(),
        };
    ...
    if (mountPoint) {
      ...
      const mountPointPlugin = getComponentData<DevtoolsPlugin>(
        node,
        'core.plugin',
      );
      if (mountPointPlugin) {
    ...
    if (routeRef) {
      ...
      if (path) {
        currentObj = {
          ...
          children: [MATCH_ALL_ROUTE],
          plugins: pluginSet(
            getComponentData<DevtoolsPlugin>(
              node.props.element,
              'core.plugin',
            ),
          ),
