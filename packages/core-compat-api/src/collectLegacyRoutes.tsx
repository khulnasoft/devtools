...
import {
  AnyRouteRefParams,
  DevtoolsPlugin as LegacyDevtoolsPlugin,
  RouteRef,
  getComponentData,
...
function visitRouteChildren(options: {
  ...
  context: {
    ...
    discoverPlugin: (plugin: LegacyDevtoolsPlugin) => void;
  };
}): void {
  ...
  Children.forEach(children, node => {
    ...
    const plugin = getComponentData<LegacyDevtoolsPlugin>(node, 'core.plugin');
...
export function collectLegacyRoutes(
  ...
  const getPluginExtensions = (plugin: LegacyDevtoolsPlugin) => {
  ...
  React.Children.forEach(
    ...
    (route: ReactNode) => {
      ...
      const plugin = getComponentData<LegacyDevtoolsPlugin>(
        routeElement,
        'core.plugin',
      );
      ...
      if (!plugin) {
        throw new Error(
          ...
          `Route with path ${path} has en element that can not be converted as it does not belong to a plugin. Make sure that the top-level React element of the element prop is an extension from a Devtools plugin, or remove the Route completely.`,
        );
      }
      if (path === undefined) {
        throw new Error(
