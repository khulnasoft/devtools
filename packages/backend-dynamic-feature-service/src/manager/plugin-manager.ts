...
import { PackageRole, PackageRoles } from '@devtools/cli-node';
import { findPaths } from '@devtools/cli-common';
import * as fs from 'fs';
import {
  FeatureDiscoveryService,
  featureDiscoveryServiceRef,
} from '@devtools/backend-plugin-api/alpha';
...
export class DynamicPluginManager implements DynamicPluginProvider {
  static async create(
    ...
    const devtoolsRoot = findPaths(__dirname).targetRoot;
  ...
  private async loadPlugins(): Promise<DynamicPlugin[]> {
    ...
    for (const scannedPlugin of this.packages) {
      const role = scannedPlugin.manifest.devtools.role;
      ...
      switch (platform) {
        ...
        case 'web':
          loadedPlugins.push({
            ...
            role: scannedPlugin.manifest.devtools.role,
            platform: 'web',
  ...
  private async loadBackendPlugin(
    ...
    const dynamicPlugin: BackendDynamicPlugin = {
      ...
      platform: 'node',
      role: plugin.manifest.devtools.role,
    };
