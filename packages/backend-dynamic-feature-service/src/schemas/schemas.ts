...
import { ConfigSchema, loadConfigSchema } from '@devtools/config-loader';
        ...
        async addDynamicPluginsSchemas(configSchema: ConfigSchema): Promise<{
          ...
          if (!additionalSchemas) {
            ...
            const scanner = PluginScanner.create({
              ...
              devtoolsRoot: findPaths(__dirname).targetRoot,
              preferAlpha: true,
            });
          ...
          if (serialized?.devtoolsConfigSchemaVersion !== 1) {
            throw new Error(
