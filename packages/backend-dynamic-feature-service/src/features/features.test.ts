...
import { overridePackagePathResolution } from '@devtools/backend-plugin-api/testUtils';
...
describe('dynamicPluginsFeatureLoader', () => {
  ...
  // A dummy `@devtools/backend-plugin-api` package which throws an error is available inside the test fixtures,
  ...
  // this dummy package would be loaded by the backend dynamic plugins instead of the one of the devtools root.
    ...
    expect(dynamicPLuginsLister.loadedPlugins).toMatchObject([
      {
        ...
        version: '0.0.0',
        failure:
          'Error: False @devtools/backend-plugin-api package which should be skipped by the CommonJSModuleLoader',
      },
      expect.anything(),
    ]);
  });
...
`),
  ...
  it('should allow overriding logger options based on config', async () => {
    ...
    expect(mockedTransport.logs).toContainEqual(
      'info: [a very nice label] Found 0 new secrets in config that will be redacted {"service":"devtools"}',
    );
  });
  ...
  it('should redact the secret config values of dynamic plugin config schemas in logs', async () => {
    ...
    expect(mockedTransport.logs).toContainEqual(
      'info: Found 1 new secrets in config that will be redacted {"service":"devtools"}',
    );

    expect(mockedTransport.logs).toContainEqual(
      'info: This secret value should be hidden by the dynamic-plugin-aware logger: *** {"service":"devtools"}',
    );
  });
...
`,
    ...
    const { server } = await startTestBackend({
      features: [
        ...
        import('@devtools/plugin-app-backend'),
      ],
    });
...
<script type="devtools.khulnasoft.com/config">
[
  {
    "context": "app",
    "deprecatedKeys": [],
  ...
  it('should access the module federation assets of the frontend plugin through the backend plugin', async () => {
    ...
    expect(await manifest.json()).toMatchObject({
      exposes: [],
      id: 'devtools__plugin_test',
      name: 'devtools__plugin_test',
      metaData: {
        buildInfo: {
          buildName: '@devtools/plugin-test',
          buildVersion: '0.0.0',
        },
        globalName: 'devtools__plugin_test',
        name: 'devtools__plugin_test',
        pluginVersion: '0.0.0',
        publicPath: 'auto',
      },
  ...
  it('should load a backend plugin from the alpha package first', async () => {
    ...
    expect(mockedTransport.logs).toContainEqual(
      'info: This plugin has been loaded from the alpha package. {"service":"devtools"}',
    );
