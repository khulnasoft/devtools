...
import { ForwardedError } from '@devtools/errors';
...
export class PluginScanner {
  ...
  private constructor(
    ...
    private readonly devtoolsRoot: string,
  ...
  private applyConfig(): void | never {
    ...
    const dynamicPluginsRootPath = path.isAbsolute(dynamicPlugins.rootDirectory)
      ...
      : path.resolve(this.devtoolsRoot, dynamicPlugins.rootDirectory);

    if (
      !path
        .dirname(dynamicPluginsRootPath)
        .startsWith(path.resolve(this.devtoolsRoot))
    ) {
      const nodePath = process.env.NODE_PATH;
      const devtoolsNodeModules = path.resolve(
        this.devtoolsRoot,
        'node_modules',
      );
      if (
        !nodePath ||
        !nodePath.split(path.delimiter).includes(devtoolsNodeModules)
      ) {
        throw new Error(
          `Dynamic plugins under '${dynamicPluginsRootPath}' cannot access devtools modules in '${devtoolsNodeModules}'.\n` +
            `Please add '${devtoolsNodeModules}' to the 'NODE_PATH' when running the devtools backend.`,
        );
      }
    }
  ...
  async scanRoot(): Promise<ScanRootResponse> {
    ...
    for (const dirEnt of await fs.readdir(dynamicPluginsLocation, {
      ...
      try {
        ...
        if (!scannedPlugin.manifest.devtools?.role) {
          throw new Error("field 'devtools.role' not found in 'package.json'");
        }
      } catch (e) {
