...
export class CommonJSModuleLoader implements ModuleLoader {
  ...
  async bootstrap(
    ...
    const devtoolsRootNodeModulesPath = `${devtoolsRoot}/node_modules`;
    ...
    this.module._nodeModulePaths = (from: string): string[] => {
      ...
      const filtered = result.filter(nodeModulePath => {
        return (
          nodeModulePath === devtoolsRootNodeModulesPath ||
    ...
    //
    // Since the packages for dynamic plugins are not located in the main devtools
    ...
    this.module._resolveFilename = (
      ...
      try {
        ...
        errorToThrow = e;
        this.options.logger.debug(
          `Could not resolve '${request}' inside the Core devtools backend application`,
          e instanceof Error ? e : undefined,
        );
      }

      // Are we trying to resolve a `package.json` from an originating module of the core devtools application
      // (this is mostly done by calling `@devtools/backend-plugin-api/resolvePackagePath`).
      const resolvingPackageJsonFromDevtoolsApplication =
      ...
      if (!resolvingPackageJsonFromDevtoolsApplication) {
        throw errorToThrow;
      }
