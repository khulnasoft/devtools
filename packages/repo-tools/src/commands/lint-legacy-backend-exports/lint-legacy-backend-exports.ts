...
import { DevtoolsPackageJson, PackageGraph } from '@devtools/cli-node';
...
export async function lint(paths: string[]) {
  const pkgs = (await PackageGraph.listTargetPackages()).filter(pkg => {
    return (
      pkg.packageJson.devtools?.role === 'backend-plugin' ||
      pkg.packageJson.devtools?.role === 'backend-plugin-module'
    );
  });
...
function verifyIndex(pkg: string, packageJson?: DevtoolsPackageJson) {
  ...
  if (
    '@devtools/backend-common' in pkgJson.dependencies ||
    '@devtools/backend-common' in pkgJson.devDependencies
  ) {
    console.log('   ❌ Stop depending on @devtools/backend-common');
  }

  if (
    '@devtools/backend-tasks' in pkgJson.dependencies ||
    '@devtools/backend-tasks' in pkgJson.devDependencies
  ) {
    console.log('   ❌ Stop depending on @devtools/backend-tasks');
  }
}
