...
import { PackageGraph, PackageRole } from '@devtools/cli-node';
...
export default async (opts: CreateFixPackageInfoYamlsOptions) => {
  ...
  const results = await Promise.allSettled<string>(
    packages.map(({ packageJson, dir }) =>
      limit(async () => {
        if (!isDevtoolsPackage(packageJson)) {
          return '';
        }
...
type CreateOptions = {
  yamlPath: string;
  packageJson: DevtoolsPackageJson;
...
type DevtoolsPackageEntity = Entity & {
  spec: {
    type: 'devtools-package';
    devtoolsRole: PackageRole;
    lifecycle: string;
    owner: string;
    [key: string]: any;
  };
...
async function fixCatalogInfoYaml(options: FixOptions) {
  ...
  let yamlJson: DevtoolsPackageEntity;

  try {
    yamlJson = YAML.load(yamlString) as DevtoolsPackageEntity;
  } catch (e) {
  ...
  const badType =
    yamlJson.spec?.type !== `devtools-${packageJson.devtools.role}`;
  ...
  if (badOwner || badTitle || badName || badType) {
    ...
    if (ci) {
      ...
      if (badType) {
        instructions.push(
          `  spec.type cannot be "${yamlJson.spec.type}" because it must be exactly "devtools-${packageJson.devtools.role}", as derived from package.json devtools.role`,
        );
      }
    }
...
/**
 * Canonical representation on how to create (or update) a devtools-package
 * component.
 */
function createOrMergeEntity(
  packageJson: DevtoolsPackageJson,
  owner: string,
  existingEntity: DevtoolsPackageEntity | Record<string, any> = {},
): DevtoolsPackageEntity {
  ...
  return {
    ...
    spec: {
      ...
      ...existingEntity.spec,
      type: `devtools-${packageJson.devtools.role}`,
      owner,
    },
  };
}
