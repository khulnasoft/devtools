...
async function verify(directoryPath: string) {
  ...
  if (
    ...
    console.warn(
      `\`${OLD_SCHEMA_PATH}\` is deprecated. Please re-run \`yarn devtools-repo-tools package schema openapi generate\` to update it.`,
    );
  ...
  if (!isEqual(schema.spec, yaml)) {
    ...
    throw new Error(
      `\`${YAML_SCHEMA_PATH}\` and \`${TS_SCHEMA_PATH}\` do not match. Please run \`yarn devtools-repo-tools package schema openapi generate\` from '${path}' to regenerate \`${TS_SCHEMA_PATH}\`.`,
    );
  }
}
