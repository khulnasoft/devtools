...
async function generateKnipConfig({ packageDir }: KnipConfigOptions) {
  const knipConfig = {
    ...
    ignoreDependencies: [
      '@devtools/cli', // everything depends on this for its package.json commands
      '@devtools/theme', // this uses `declare module` in .d.ts so is implicitly used whenever extensions are needed
    ],
  };
  await fs.writeFile(
