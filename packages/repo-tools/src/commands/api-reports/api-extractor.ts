...
export async function createTemporaryTsConfig(includedPackageDirs: string[]) {
  ...
  try {
    assetTypeFile = [
      require.resolve('@devtools/cli/asset-types/asset-types.d.ts'),
    ];
  } catch {
    /** ignore */
  }
    ...
    for (const packageEntryPoint of packageEntryPoints) {
      ...
      const extractorResult = Extractor.invoke(extractorConfig, {
        ...
        messageCallback(message) {
          ...
          // Warning: You have changed the API signature for this project. Please copy the file "/home/runner/work/khulnasoft/devtools/node_modules/.cache/api-extractor/backend-test-utils/report.api.md" to "report.api.md", or perform a local build (which does this automatically). See the Git repo documentation for more info.
          if (
...
export async function buildDocs({
  ...
  class CustomMarkdownDocumenter extends (MarkdownDocumenter as any) {
    constructor(options: IMarkdownDocumenterOptions) {
      ...
      ).docNodeManager.registerDocNodes('@devtools/docs', [
    ...
    _writeBreadcrumb(output: any, apiItem: ApiItem & { name: string }) {
      ...
      if (name) {
        ...
        title = 'Package Index';
        description = 'Index of all Devtools Packages';
    ...
    _writeModelTable(
      ...
      for (const apiMember of apiModel.members) {
        ...
        if (apiMember.kind === 'Package') {
          ...
          if (apiMember.name.startsWith('@devtools/plugin-')) {
    ...
    Array(10)
      ...
      .map(async () => {
        for (;;) {
          ...
          const role = pkgJson?.devtools?.role;
          if (!role) {
