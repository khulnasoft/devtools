...
import { resolvePackagePath } from '@devtools/backend-plugin-api';
import {
...
async function generate(
  ...
  await exec(
    ...
    [
      ...
      resolvePackagePath(
        '@devtools/repo-tools',
        'templates/typescript-devtools-client.yaml',
      ),
      '--generator-key',
      'v3.0',
    ...
    ],
    {
      ...
      cwd: resolvePackagePath('@devtools/repo-tools'),
      env: {
        ...process.env,
      },
    },
  ...
  await exec(`yarn devtools-cli package lint --fix ${parentDirectory}`, [], {
    signal: abortSignal?.signal,
  });
