...
async function generateSpecFile() {
  ...
  // Devtools repo. Otherwise the auto-generated comment will be removed by the
  // lint call below.
  await fs.writeFile(
    tsPath,
    `//
...
// ******************************************************************
import {createValidatedOpenApiRouterFromGeneratedEndpointMap} from '@devtools/backend-openapi-utils';
...
`,
  ...
  await exec(`yarn devtools-cli package lint`, ['--fix', tsPath, indexFile]);
...
async function generate(abortSignal?: AbortController) {
  ...
  await exec(
    ...
    [
      ...
      resolvePackagePath(
        '@devtools/repo-tools',
        'templates/typescript-devtools-server.yaml',
      ),
      '--generator-key',
      'v3.0',
    ],
  ...
  await exec(
    `yarn devtools-cli package lint --fix ${resolvedOutputDirectory}`,
    [],
    {
      signal: abortSignal?.signal,
    },
