...
capture:
    ${YAML_SCHEMA_PATH}:
      ...
      cliPaths.targetDir
        ...
        server:
            # This will not be used by 'devtools-repo-tools schema openapi test', but may be useful for interactive updates.
        ...
        requests:
            ...
            run:
                ...
                command: yarn devtools-cli package test --no-watch ${ROUTER_TEST_PATHS.map(
                  e => `"${e}"`,
                ).join(' ')} 
  `,
  );
