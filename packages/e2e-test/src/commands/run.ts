...
import { findPaths } from '@devtools/cli-common';
...
export async function run(opts: OptionValues) {
  const rootDir = await fs.mkdtemp(resolvePath(os.tmpdir(), 'devtools-e2e-'));
...
async function buildDistWorkspace(workspaceName: string, rootDir: string) {
  ...
  function appendDeps(pkg: any) {
    Array<string>()
      ...
      .filter(name => name.startsWith('@devtools/'))
  ...
  // eslint-disable-next-line @devtools/no-forbidden-package-imports
  appendDeps(require('@devtools/create-app/package.json'));
...
/**
 * Pin the yarn version in a directory to the one we're using in the Devtools repo
 */
...
async function createApp(
  ...
  const child = spawnPiped(
    [
      'node',
      resolvePath(workspaceDir, 'packages/create-app/bin/devtools-create-app'),
      '--skip-install',
    ],
    {
      cwd: rootDir,
...
async function dropClientDatabases(client: string) {
  await Promise.all(
    [
      ...
      'techdocs',
      'search',
    ].map(name => dropDB(`devtools_plugin_${name}`, client)),
  );
}
