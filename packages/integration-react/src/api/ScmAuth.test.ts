...
describe('ScmAuth', () => {
  it('should provide credentials for GitHub and GHE', async () => {
    ...
    await expect(
      api.getCredentials({ url: 'https://github.com/khulnasoft/devtools' }),
    ).resolves.toEqual({
  ...
  it('should throw an error for unknown URLs', async () => {
    ...
    await expect(
      ...
      "No auth provider available for 'http://example.com', see https://devtools.khulnasoft.com/link?scm-auth",
    );
    ...
    await expect(
      ...
      "No auth provider available for 'http://not.example.com', see https://devtools.khulnasoft.com/link?scm-auth",
    );
    await expect(
      scmAuth.getCredentials({ url: 'http://example.com:8080' }),
    ).rejects.toThrow(
      "No auth provider available for 'http://example.com:8080', see https://devtools.khulnasoft.com/link?scm-auth",
    );
  });
});
