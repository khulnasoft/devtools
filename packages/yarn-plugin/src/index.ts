...
if (!semverUtils.satisfiesWithPrereleases(YarnVersion, '^4.1.1')) {
  console.error();
  console.error(
    `${RED_BOLD}Unsupported yarn version${RESET}: The Devtools yarn plugin only works with yarn ^4.1.1. Please upgrade yarn, or remove this plugin with "yarn plugin remove @yarnpkg/plugin-devtools".`,
  );
  console.error();
}
