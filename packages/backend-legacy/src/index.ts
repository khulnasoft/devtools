...
import { DefaultEventBroker } from '@devtools/plugin-events-backend';
import { DefaultEventsService } from '@devtools/plugin-events-node';
import { DefaultSignalsService } from '@devtools/plugin-signals-node';
import { UrlReaders } from '@devtools/backend-defaults/urlReader';
import { DefaultSchedulerService } from '@devtools/backend-defaults/scheduler';
...
async function main() {
  ...
  logger.info(
    `You are running an example backend, which is supposed to be mainly used for contributing back to Devtools. ` +
      `Do NOT deploy this to production. Read more here https://devtools.khulnasoft.com/docs/getting-started/`,
  );
