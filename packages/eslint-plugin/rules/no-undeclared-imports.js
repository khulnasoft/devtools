...
function getExpectedDepType(
  ...
  const role = localPkg?.devtools?.role;
...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      url: 'https://github.com/khulnasoft/devtools/blob/master/packages/eslint-plugin/docs/rules/no-undeclared-imports.md',
    },
  },
  create(context) {
    ...
    return {
      ...
      ...visitImports(context, (node, imp) => {
        ...
        if (
          imp.type === 'internal' &&
          imp.package.packageJson.devtools?.inline
        ) {
          for (const depType of Object.values(depFields)) {
