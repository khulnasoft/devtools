...
import { AnyRouteRef, DevtoolsRouteObject } from './types';
...
function resolveBasePath(
  ...
  for (
    ...
    matchIndex = match.findIndex(m =>
      (m.route as DevtoolsRouteObject).routeRefs.has(targetSearchRef!),
    );
    if (matchIndex !== -1) {
      break;
    }
...
export class RouteResolver implements RouteResolutionApi {
  constructor(
    ...
    private readonly routeObjects: DevtoolsRouteObject[],
