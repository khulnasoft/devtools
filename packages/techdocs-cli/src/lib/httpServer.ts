...
export default class HTTPServer {
  ...
  private readonly devtoolsBundleDir: string;
  ...
  constructor(
    ...
    this.devtoolsBundleDir = devtoolsBundleDir;
    ...
    return new Promise<http.Server>((resolve, reject) => {
      ...
      const server = http.createServer(
        (request: http.IncomingMessage, response: http.ServerResponse) => {
          ...
          if (request.url === '/api/techdocs/.devtools/auth/v1/cookie') {
      ...
      server.listen(this.devtoolsPort, () => {
        if (this.verbose) {
          logger.info(
            `[techdocs-preview-bundle] Running local version of Devtools at http://localhost:${this.devtoolsPort}`,
          );
        }
        resolve(server);
      });
