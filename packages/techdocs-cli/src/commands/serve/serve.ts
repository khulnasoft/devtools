...
import { getMkdocsYml } from '@devtools/plugin-techdocs-node';
...
export default async function serve(opts: OptionValues) {
  ...
  // This will run the devtools http server on a different port and only used
  // for proxying mkdocs to the devtools app running locally (e.g. with webpack-dev-server)
  const isDevMode = Object.keys(process.env).includes('TECHDOCS_CLI_DEV_MODE')
    ? true
    : false;

  const devtoolsBackendPort = 7007;
  ...
  // Wait until mkdocs server has started so that Devtools starts with docs loaded
  // Takes 1-5 seconds
  ...
  const port = isDevMode ? devtoolsBackendPort : opts.previewAppPort;
  ...
  httpServer
    ...
    .then(() => {
      ...
      logger.info(
        `Serving docs in Devtools at http://localhost:${port}/docs/default/component/local/\nOpening browser.`,
      );
    });
