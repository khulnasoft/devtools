...
class Cache {
  ...
  async getOrCreateToken(
    ...
    if (!this.appliesToRepo(existingInstallationData, repo)) {
      throw new Error(
        `The Devtools GitHub application used in the ${owner} organization does not have access to a repository with the name ${repo}`,
      );
    }
  ...
  implements GithubCredentialsProvider
{
   ...
   *   url: 'github.com/khulnasoft/foobar'
   * })
   * ```
   *
