...
describe('LocationV1alpha1Validator', () => {
  ...
  it('silently accepts v1beta1 as well', async () => {
    (entity as any).apiVersion = 'devtools.khulnasoft.com/v1beta1';
    await expect(validator.check(entity)).resolves.toBe(true);
  });
  ...
  it('accepts good target', async () => {
    (entity as any).spec.target =
      'https://github.com/khulnasoft/devtools/blob/master/plugins/catalog-backend/examples/artist-lookup-component.yaml';
    await expect(validator.check(entity)).resolves.toBe(true);
  });
  ...
  it('accepts good targets', async () => {
    (entity as any).spec.targets = [
      'https://github.com/khulnasoft/devtools/blob/master/plugins/catalog-backend/examples/artist-lookup-component.yaml',
      'https://github.com/khulnasoft/devtools/blob/master/plugins/catalog-backend/examples/playback-order-component.yaml',
    ];
    await expect(validator.check(entity)).resolves.toBe(true);
  });
