...
- 51a9f8f122: **BREAKING**:
  ...
  - Removed the previously deprecated type `LocationSpec` type, which has been moved to `@devtools/plugin-catalog-backend`.
...
- ed09ad8093: Deprecated the `LocationSpec` export, which should now be imported from `@devtools/plugin-catalog-backend` instead.
...
- e72d371296: **BREAKING**: Removed `TemplateEntityV1beta2` from the model and moved it to
  `@devtools/plugin-scaffolder-common` where `TemplateEntityV1beta3` already
  lived. It has also been marked as deprecated in the process - please consider
  [migrating to `v1beta3` templates](https://devtools.khulnasoft.com/docs/features/software-templates/migrating-from-v1beta2-to-v1beta3).
...
- 244d24ebc4: Deprecates the `Location` type export from this package. Imports of the `Location` type should now be done from the `@devtools/catalog-client` package instead.
...
- 8b5a7763d5: Added `alpha` release stage type declarations, accessible via `@devtools/catalog-model/alpha`.
  ...
  **BREAKING**: The experimental entity `status` field was removed from the base `Entity` type and is now only available on the `AlphaEntity` type from the alpha release stage, along with all accompanying types that were previously marked as `UNSTABLE_`.
  ...
  ```ts
  import { UNSTABLE_EntityStatusItem } from '@devtools/catalog-model';
  ```

  Becomes this:

  ```ts
  import { EntityStatusItem } from '@devtools/catalog-model/alpha';
  ```
...
- 77db0c454: Changed the regex to validate names following the Kubernetes validation rule, this allow to be more permissive validating the name of the object in Devtools.
...
- 6e612ce25: Adds a new optional `links` metadata field to the Entity class within the `catalog-model` package (as discussed in [[RFC] Entity Links](https://github.com/khulnasoft/devtools/issues/3787)). This PR adds support for the entity links only. Follow up PR's will introduce the UI component to display them.
...
- 069cda35f: Marked the field `spec.implementsApis` on `Component` entities for deprecation on Dec 14th, 2020.
  ...
  After Dec 14th, the fields will be removed from types and classes of the Devtools repository. At
