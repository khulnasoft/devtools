...
- 0ade9d02b: Include `devDependencies` and `optionalDependencies` in the detection of Devtools packages when collecting configuration schema.
...
- 2cf98d279: Resolve the path to app-config.yaml from the current working directory. This will allow use of `yarn link` or running the CLI in other directories and improve the experience for local devtools development.
...
- 4e7091759: Fix typo of "visibility" in config schema reference
  ...
  see https://devtools.khulnasoft.com/docs/conf/defining#visibility.
