...
import { assertError } from '@devtools/errors';
...
export async function collectConfigSchemas(
  ...
  async function processItem(item: Item) {
    ...
    if (pkgPath) {
      ...
      try {
        ...
        // as they are likely not part of the Devtools ecosystem anyway.
      }
    }
    if (!pkgPath) {
      return;
    ...
    //               we've left the Devtools ecosystem, but this will do for now.
    const hasSchema = 'configSchema' in pkg;
    const hasDevtoolsDep = depNames.some(_ => _.startsWith('@devtools/'));
    if (!hasSchema && !hasDevtoolsDep) {
      return;
    }
    if (hasSchema) {
  ...
  const hasBackendDefaults = allSchemas.some(
    ({ packageName }) => packageName === '@devtools/backend-defaults',
  );
  ...
  if (hasBackendDefaults) {
    ...
    // led to issues such as https://github.com/khulnasoft/devtools/issues/28170
    return allSchemas.filter(
      ({ packageName }) => packageName !== '@devtools/backend-common',
    );
  }
