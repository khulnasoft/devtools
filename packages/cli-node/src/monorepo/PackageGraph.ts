...
/**
 * Known fields in Devtools package.json files.
 *
 * @public
 */
export interface DevtoolsPackageJson {
  name: string;
  version: string;
  private?: boolean;
  ...
  // The `bundled` field is a field known within Devtools, it means
...
/**
 * A local Devtools monorepo package
 *
 * @public
 */
export type DevtoolsPackage = {
  dir: string;
...
/**
 * Represents a local Devtools monorepo package graph.
 *
 * @public
 */
export class PackageGraph extends Map<string, PackageGraphNode> {
   ...
   */
  static async listTargetPackages(): Promise<DevtoolsPackage[]> {
    const { packages } = await getPackages(paths.targetDir);

    return packages as DevtoolsPackage[];
  }
  ...
  static fromPackages(packages: Package[]): PackageGraph {
    ...
    for (const pkg of packages) {
      ...
      graph.set(name, {
        name,
        dir: pkg.dir,
        packageJson: pkg.packageJson as DevtoolsPackageJson,
