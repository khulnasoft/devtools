...
type Options = {
   ...
   * annotates the package exports field with the `devtools` export type.
   */
...
export async function createDistWorkspace(
  ...
  if (options.buildDependencies) {
    ...
    for (const pkg of packages) {
      ...
      if (!buildScript.startsWith('devtools-cli package build')) {
        console.warn(
...
async function moveToDistWorkspace(
  ...
  // Repacking in parallel is much faster and safe for all packages outside of the Devtools repo
