...
export async function productionPack(options: ProductionPackOptions) {
  ...
  const pkg = JSON.parse(pkgContent) as DevtoolsPackageJson;
  ...
  for (const entryPoint of entryPoints) {
    ...
    if (exp.types && featureDetectionProject) {
      ...
      if (defaultFeatureType) {
        // This ensures that the `devtools` field is at the top of the
        ...
        exp = { devtools: defaultFeatureType, ...exp };
      }
    }
