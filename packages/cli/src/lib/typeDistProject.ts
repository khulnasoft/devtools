...
const targetFeatureTypes = [
  '@devtools/BackendFeature',
  '@devtools/DevtoolsPlugin',
  '@devtools/FrontendPlugin',
  '@devtools/FrontendModule',
] as const;

export type DevtoolsPackageFeatureType = (typeof targetFeatureTypes)[number];
...
export const getEntryPointDefaultFeatureType = (
  ...
  project: Project,
  entryPoint: string,
): DevtoolsPackageFeatureType | null => {
...
// that are valid Devtools package features
function getSourceFileDefaultFeatureType(
  ...
  for (const exportSymbol of sourceFile.getExportSymbols()) {
    ...
    if (exportName && exportType) {
      const $$type = getDevtoolsPackageFeature$$TypeFromType(exportType);
...
// Given a TS type, returns the Devtools package feature $$type value
function getDevtoolsPackageFeature$$TypeFromType(
  type: Type,
): DevtoolsPackageFeatureType | null {
...
// Returns whether an export is a valid Devtools package feature type
function isTargetFeatureType(
  type: string | DevtoolsPackageFeatureType,
): type is DevtoolsPackageFeatureType {
  return (
    !!type && targetFeatureTypes.includes(type as DevtoolsPackageFeatureType)
  );
}
