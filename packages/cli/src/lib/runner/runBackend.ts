...
const loaderArgs = [
  '--enable-source-maps',
  '--require',
  require.resolve('@devtools/cli/config/nodeTransform.cjs'),
...
export async function runBackend(options: RunBackendOptions) {
  ...
  const restart = debounce(async () => {
    ...
    const userArgs = process.argv
      .slice(['node', 'devtools-cli', 'package', 'start'].length)
    ...
    child = spawn(
      ...
      [...loaderArgs, ...optionArgs, options.entry, ...userArgs],
      {
        ...
        env: {
          ...process.env,
          DEVTOOLS_CLI_LINKED_WORKSPACE: options.linkedWorkspace,
          DEVTOOLS_CLI_CHANNEL: '1',
