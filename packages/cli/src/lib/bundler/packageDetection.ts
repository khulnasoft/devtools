...
const DETECTED_MODULES_MODULE_NAME = '__devtools-autodetected-plugins__';
...
async function detectPackages(
  ...
  const pkg: DevtoolsPackageJson = await fs.readJson(
  ...
  return Object.keys(pkg.dependencies ?? {}).flatMap(depName => {
    ...
    try {
      const depPackageJson: DevtoolsPackageJson = require(require.resolve(
        `${depName}/package.json`,
        { paths: [targetPath] },
      ));
      if (
        ['frontend-plugin', 'frontend-plugin-module'].includes(
          depPackageJson.devtools?.role ?? '',
        )
      ) {
...
async function writeDetectedPackagesModule(
  ...
  await writeQueue.add(() =>
    fs.writeFile(
      ...
      `window['__@devtools/discovered__'] = { modules: [${requirePackageScript}] };`,
    ),
  );
}
