...
const BUILD_CACHE_ENV_VAR = 'DEVTOOLS_CLI_EXPERIMENTAL_BUILD_CACHE';
...
export async function createConfig(
  ...
  if (options.moduleFederation?.mode !== 'remote') {
    const templateOptions = {
      meta: {
        'devtools-app-mode': options?.appMode ?? 'public',
      },
    ...
    plugins.push(
      new HtmlWebpackPlugin({
        meta: {
          ...
          'devtools-public-path': '<%= publicPath %>/',
        },
        minify: false,
  ...
  return {
    ...
    ...(isDev
      ? {
          watchOptions: {
            ignored: /node_modules\/(?!__devtools-autodetected-plugins__)/,
          },
        }
      : {}),
    optimization,
    ...
    context: paths.targetPath,
    entry: [
      require.resolve('@devtools/cli/config/webpack-public-path'),
