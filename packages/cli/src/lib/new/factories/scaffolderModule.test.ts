...
describe('scaffolderModule factory', () => {
  ...
  it('should create a scaffolder backend module package', async () => {
    ...
    expectLogsToMatch(output, [
      'Creating module devtools-plugin-scaffolder-backend-module-test',
...
backend.add(import('devtools-plugin-scaffolder-backend-module-test'));
backend.start();
`);
    ...
    await expect(
      ...
      dependencies: {
        'devtools-plugin-scaffolder-backend-module-test': '^1.0.0',
      },
    });
    ...
    await expect(
      ...
      expect.objectContaining({
        name: 'devtools-plugin-scaffolder-backend-module-test',
        description: 'The test module for @devtools/plugin-scaffolder-backend',
        private: true,
        version: '1.0.0',
      }),
    );
