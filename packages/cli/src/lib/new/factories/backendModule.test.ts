...
describe('backendModule factory', () => {
  ...
  it('should create a backend plugin', async () => {
    ...
    expectLogsToMatch(output, [
      'Creating backend module devtools-plugin-test-backend-module-tester-two',
...
backend.add(import('devtools-plugin-test-backend-module-tester-two'));
backend.start();
`);
    ...
    await expect(
      ...
      dependencies: {
        'devtools-plugin-test-backend-module-tester-two': '^1.0.0',
      },
    });
