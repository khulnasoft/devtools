...
import corePluginApiPkg from '@devtools/core-plugin-api/package.json';
...
"@devtools/cli@*":
  version "1.1.5"
    ...
    const cliVersion = packageVersions['@devtools/cli'];
    expect(provider('@devtools/cli')).toBe(
      // If we're currently in pre-release we expect that to be picked instead
      cliVersion.includes('-') ? `^${cliVersion}` : '*',
    );
    expect(provider('@devtools/core-plugin-api')).toBe(
