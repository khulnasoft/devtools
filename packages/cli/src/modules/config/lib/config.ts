...
import { ConfigSources, loadConfigSchema } from '@devtools/config-loader';
import { AppConfig, ConfigReader } from '@devtools/config';
import { paths } from '../../../lib/paths';
import { getPackages } from '@manypkg/get-packages';
import { PackageGraph } from '@devtools/cli-node';
    ...
    if (packages.length) {
      ...
      localPackageNames = Array.from(
        graph.collectPackageNames([options.fromPackage], node => {
          // Workaround for Devtools main repo only, since the CLI has some artificial devDependencies
          if (node.name === '@devtools/cli') {
            return undefined;
          }
