...
import { Lockfile, PackageGraph } from '@devtools/cli-node';
...
interface TestGlobal extends Global {
  __devtoolsCli_jestSuccessCache?: {
    filterConfigs(
  ...
  __devtoolsCli_watchProjectFilter?: {
...
export async function command(opts: OptionValues, cmd: Command): Promise<void> {
  ...
  if (isSingleWatchMode && parsedArgs.length > 0) {
    testGlobal.__devtoolsCli_watchProjectFilter = {
  ...
  // the `repo test` command in the Devtools CLI.
  if (opts.successCache) {
    ...
    // Set up a bridge with the @devtools/cli/config/jest configuration file. These methods
    // are picked up by the config script itself, as well as the custom result processor.
    testGlobal.__devtoolsCli_jestSuccessCache = {
