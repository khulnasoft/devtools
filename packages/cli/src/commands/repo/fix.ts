 ...
 */
export interface FixablePackage extends DevtoolsPackage {
  changed: boolean;
}
...
export function printPackageFixHint(packages: FixablePackage[]) {
  ...
  if (changed.length > 0) {
    ...
    const fixCmd =
      rootPkg.scripts?.fix === 'devtools-cli repo fix'
        ? 'fix'
...
export function fixPackageExports(pkg: FixablePackage) {
  ...
  if (existingTypesVersions !== JSON.stringify(typesVersions)) {
    ...
    pkg.packageJson = Object.fromEntries(newPkgEntries) as DevtoolsPackageJson;
    pkg.changed = true;
  }
...
export function fixSideEffects(pkg: FixablePackage) {
  ...
  pkg.packageJson = Object.fromEntries(pkgEntries) as DevtoolsPackageJson;
  pkg.changed = true;
}
...
export function createRepositoryFieldFixer() {
  ...
  return (pkg: FixablePackage) => {
    ...
    if (!repoField || typeof repoField === 'string') {
      const pkgEntries = Object.entries(pkg.packageJson);
      pkgEntries.splice(
        // Place it just above the devtools field
        pkgEntries.findIndex(([name]) => name === 'devtools'),
        0,
        [
          'repository',
          {
...
function guessPluginId(role: PackageRole, pkgName: string): string | undefined {
  switch (role) {
    ...
    default:
      throw new Error(
        `Invalid 'devtools.role' field in "${pkgName}", got '${role}'`,
      );
  }
}

export function fixPluginId(pkg: FixablePackage) {
  const role = pkg.packageJson.devtools?.role;
  if (!role) {
    return;
  }
  ...
  const currentId = pkg.packageJson.devtools?.pluginId;
  if (currentId !== undefined) {
    if (typeof currentId !== 'string' && currentId !== null) {
      throw new Error(
        `Invalid 'devtools.pluginId' field in "${pkg.packageJson.name}", must be a string`,
      );
    }
    return;
  }
  ...
  if (
    ...
    const msg = `Failed to guess plugin ID for "${pkg.packageJson.name}", please set the 'devtools.pluginId' field manually in "${path}"`;
...
const backendPluginPackageNameByPluginId = new Map(
  [
    ...
    'signals',
    'techdocs',
  ].map(pluginId => [pluginId, `@devtools/plugin-${pluginId}-backend`]),
);
...
export function fixPluginPackages(
  ...
  const pkgDevtools = pkg.packageJson.devtools;
  const role = pkgDevtools?.role;
  if (!role) {
    return;
  }
  ...
  const pluginId = pkgDevtools.pluginId;
  if (!pluginId) {
    ...
    throw new Error(
      `Missing 'devtools.pluginId' field in "${pkg.packageJson.name}"`,
    );
  }
  ...
  if (role === 'backend-plugin-module' || role === 'frontend-plugin-module') {
    ...
    // Try to find a plugin package in the same repo, but otherwise fall back to looking up the package name by ID of @devtools/* plugins
    const pluginPkgName =
      repoPackages.find(
        p =>
          p.packageJson.devtools?.pluginId === pluginId &&
          p.packageJson.devtools?.role === targetRole,
    ...
    if (!pluginPkgName) {
      ...
      throw new Error(
        `Failed to find plugin package for "${pkg.packageJson.name}", please declare the name of the plugin package that this package is a module for in the 'devtools.pluginPackage' field in "${path}". ` +
          `It is also possible that this package is not a module, and should use the '${suggestedRole}' role instead.`,
      );
    }

    if (pkgDevtools.pluginPackage !== pluginPkgName) {
      pkgDevtools.pluginPackage = pluginPkgName;
      pkg.changed = true;
    }
  } else {
    let pluginPackages: string[] | undefined = repoPackages
      .filter(
        p =>
          p.packageJson.devtools?.pluginId === pluginId &&
          pluginPackageRoles.includes(p.packageJson.devtools?.role),
      )
    ...
    if (pkgDevtools.pluginPackages?.join(',') !== pluginPackages?.join(',')) {
      pkgDevtools.pluginPackages = pluginPackages;
      pkg.changed = true;
    }
  }
}
