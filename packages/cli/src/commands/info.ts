...
export default async () => {
  await new Promise(async () => {
    ...
    let devtoolsVersion = 'N/A';
    if (fs.existsSync(devtoolsFile)) {
      try {
        const devtoolsJson = await fs.readJSON(devtoolsFile);
        devtoolsVersion = devtoolsJson.version ?? 'N/A';
      } catch (error) {
        console.warn('The "devtools.json" file is not in the expected format');
        console.log();
      }
    }
    ...
    const deps = [...lockfile.keys()].filter(n => n.startsWith('@devtools/'));
