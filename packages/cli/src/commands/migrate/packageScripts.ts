...
import { PackageGraph, PackageRoles, PackageRole } from '@devtools/cli-node';
...
export async function command() {
  ...
  await Promise.all(
    packages.map(async ({ dir, packageJson }) => {
      ...
      const testCmd = ['test'];
      if (scripts.test?.startsWith('devtools-cli test')) {
        const args = scripts.test
          .slice('devtools-cli test'.length)
          .split(' ')
          .filter(Boolean);
      ...
      const expectedScripts = {
        ...(hasStart && {
          start: `devtools-cli package ${startCmd.join(' ')}`,
        }),
        build: `devtools-cli package ${buildCmd.join(' ')}`,
        lint: 'devtools-cli package lint',
        test: `devtools-cli package ${testCmd.join(' ')}`,
      ...
      for (const [name, value] of Object.entries(expectedScripts)) {
        ...
        const isDevtoolsScript = currentScript?.includes('devtools-cli');
        if (isMissing || (isDifferent && isDevtoolsScript)) {
          changed = true;
