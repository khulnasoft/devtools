import { AuthService, LoggerService } from '@devtools/backend-plugin-api';
import { NotFoundError } from '@devtools/errors';
import { catalogServiceRef } from '@devtools/plugin-catalog-node/alpha';
...
// https://devtools.khulnasoft.com/docs/backend-system/core-services/database
export async function createTodoListService({
  ...
  return {
    async createTodo(input, options) {
      ...
      // TEMPLATE NOTE:
      // A common pattern for Devtools plugins is to pass an entity reference
      ...
      if (input.entityRef) {
        ...
        // https://devtools.khulnasoft.com/docs/features/software-catalog/extending-the-model#adding-a-new-annotation
        //
      ...
      // https://devtools.khulnasoft.com/docs/backend-system/core-services/logger
