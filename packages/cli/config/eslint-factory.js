...
/**
 * Creates a ESLint configuration that extends the base Devtools configuration.
    ...
    case 'backend-plugin-module':
      return createConfig(dir, {
        ...
        restrictedSrcSyntax: [
          {
            message:
              "`__dirname` doesn't refer to the same dir in production builds, try `resolvePackagePath()` from `@devtools/backend-plugin-api` instead.",
...
function createPackageConfig(dir, extraConfig) {
  ...
  const role = pkg.devtools?.role;
  if (!role) {
    throw new Error(`Package ${pkg.name} does not have a devtools.role`);
  }
