...
const paths = require('@devtools/cli-common').findPaths(process.cwd());
...
const envOptions = {
  oldTests: Boolean(process.env.DEVTOOLS_OLD_TESTS),
};
...
async function getProjectConfig(targetPath, extraConfig, extraOptions) {
  ...
  const options = {
    ...
    ...getRoleConfig(pkgJson.devtools?.role),
  };
  ...
  if (
    ...
    FRONTEND_ROLES.includes(pkgJson.devtools?.role)
  ) {
  ...
  if (!config.id) {
    const configHash = crypto
      ...
      .digest('hex');
    config.id = `devtools_cli_${configHash}`;
  }
...
async function getRootConfig() {
  ...
  let projects = await Promise.all(
    projectPaths.flat().map(async projectPath => {
      ...
      // We check for the presence of "devtools-cli test" in the package test
      ...
      const isSupportedTestScript =
        testScript?.includes('devtools-cli test') ||
        testScript?.includes('devtools-cli package test');
  ...
  const cache = global.__devtoolsCli_jestSuccessCache;
  if (cache) {
    projects = await cache.filterConfigs(projects, globalRootConfig);
  }
  const watchProjectFilter = global.__devtoolsCli_watchProjectFilter;
