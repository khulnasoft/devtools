...
- e52d6ad861: Updated the `devtools-cli` so that the `list-deprecations` command is visible and also removed the "[EXPERIMENTAL]" tag.
...
- 947ae3b40e: Applied the fix from version `0.15.3` of this package, which is part of the `v0.71.1` release of Devtools.
- 19eed0edd9: Fix for `overrides` not being properly forwarded from the extra configuration passed to `@devtools/cli/config/eslint-factory`.
...
- Fixed an issue where the CLI would try and fail to require the `package.json` of other Devtools packages, like `@devtools/dev-utils/package.json`.
...
- 44cc7c3b95: Added a new ESLint configuration setup for packages, which utilizes package roles to generate the correct configuration. The new configuration is available at `@devtools/cli/config/eslint-factory`.

  Introduced a new `devtools-cli migrate package-lint-configs` command, which migrates old lint configurations to use `@devtools/cli/config/eslint-factory`.

- b1aacbf96a: Applied the fix from version `0.15.1` of this package, which was part of the `v0.70.1` release of Devtools.
- d2ecde959b: Package roles are now marked as stable and migration is encouraged. Please check out the [migration guide](https://devtools.khulnasoft.com/docs/tutorials/package-role-migration).
...
- 0b74c72987: Added a new experimental and hidden `devtools-cli repo lint` command that can be used to lint all packages in the project, similar to `lerna run lint`.
...
- 22862486de: The `versions:bump` command now also considers the root `package.json` when searching for updates. It has also received updates to its output, including a link the [Devtools upgrade helper](https://devtools.github.io/upgrade-helper) and silenced `yarn install` output.
- 7410e12268: Several changes were made to the new experimental package roles system. Unless you have been experimenting with using this new system, these changes have no affect on your project.
  ...
  Renamed the `devtools-cli migrate package-role` command to `devtools-cli migrate package-roles`.
  ...
  The `devtools-cli migrate package-scripts` received several tweaks to make it more accurate. It now tries to maintain existing script arguments, like `--config` parameters for `build` and `start` scripts.

  The `script` command category has been renamed to `package`.

  The `devtools-cli package build` command set an incorrect target directory for `app` and `backend` packages, which has been fixed.
...
- Updated dependencies
  ...
  - @devtools/release-manifests@0.0.2
...
- eaf67f0578: Introduced initial support for an experimental `devtools.role` field in package.json, as well as experimental and hidden `migrate` and `script` sub-commands. We do not recommend usage of any of these additions yet.
- aeb5c69abb: Introduces a new `--release` parameter to the `devtools-cli versions:bump` command.
...
- Updated dependencies
  - @devtools/release-manifests@0.0.1
...
- 08fa6a604a: Removed the `typescript` dependency from the Devtools CLI in order to decouple the TypeScript version in Devtools projects. To keep using a specific TypeScript version, be sure to add an explicit dependency in your root `package.json`:
...
- 6e34e2cfbf: Introduce `--deprecated` option to `config:check` to log all deprecated app configuration properties
  ...
  ```sh
  $ yarn devtools-cli config:check --lax --deprecated
...
- 2a42d573d2: Introduced a new `--experimental-type-build` option to the various package build commands. This option switches the type definition build to be executed using API Extractor rather than a `rollup` plugin. In order for this experimental switch to work, you must also have `@microsoft/api-extractor` installed within your project, as it is an optional peer dependency.
  ...
  In addition, the `prepack` command now has support for `alphaTypes` and `betaTypes` fields in the `publishConfig` of `package.json`. These optional fields can be pointed to `dist/types.alpha.d.ts` and `dist/types.beta.d.ts` respectively, which will cause `<name>/alpha` and `<name>/beta` entry points to be generated for the package. See the [`@devtools/catalog-model`](https://github.com/khulnasoft/devtools/blob/master/packages/catalog-model/package.json) package for an example of how this can be used in practice.
...
- 9534391ae4: Add --pattern option to override matching glob patterns for devtools versioning
...
- 5fdc8df0e8: The frontend configuration is now available as a `config` global during templating of the `index.html` file. This allows for much more flexibility as the values available during templating is not longer hardcoded to a fixed set of values.
  ...
  The [`@devtools/create-app` changelog](https://github.com/khulnasoft/devtools/blob/master/packages/create-app/CHANGELOG.md#049) also contains more details how to migrate existing usage.
...
- 0ebb05eee2: Add cli option to minify the generated code of a plugin or backend package
  ...
  ```
  devtools-cli plugin:build --minify
  devtools-cli backend:build --minify
  ```

- cd450844f6: Updated the frontend plugin template to put React dependencies in `peerDependencies` by default, as well as allowing both React v16 and v17. This change can be applied to existing plugins by running `yarn devtools-cli plugin:diff` within the plugin package directory.
...
- Updated dependencies
  - @devtools/config-loader@0.8.1

## 0.9.1

### Patch Changes

- dde216acf4: Switch the default test coverage provider from the jest default one to `'v8'`, which provides much better coverage information when using the default Devtools test setup. This is considered a bug fix as the current coverage information is often very inaccurate.
...
- 677bfc2dd0: Keep devtools.json in sync

  The `versions:bump` script now takes care about updating the `version` property inside `devtools.json` file. The file is created if is not present.
...
- 4ca3542fdd: Fixed a bug where calling `devtools-cli backend:bundle --build-dependencies` with no dependencies to be built would cause all monorepo packages to be built instead.
- 867ea81d15: bump `@rollup/plugin-commonjs` from 17.1.0 to 21.0.1
- 16d06f6ac3: Introduces new `devtools-cli create` command to replace `create-plugin` and make space for creating a wider array of things. The create command also adds a new template for creating isomorphic common plugin packages.
- Updated dependencies
...
- dd355bca46: Switched to dynamically determining the packages that are unsafe to repack when executing the CLI within the Devtools main repo.
...
- b486adb8c6: The Jest configuration that's included with the Devtools CLI has received several changes.
  ...
  To counteract the slowdown of the additional transforms that have been introduced, the default configuration has also been reworked to enable caching across different packages. Previously each package in a Devtools monorepo would have its own isolated Jest cache, but it is now shared between packages that have a similar enough Jest configuration.
  ...
  This change also removes the `@devtools/cli/config/jestEsmTransform.js` transform, which can be replaced by using the `@devtools/cli/config/sucraseEsmTransform.js` transform instead.
...
- 1ef9e64901: Add an experimental `install <plugin>` command.
  ...
  Given a `pluginId`, the command looks for NPM packages matching `@devtools/plugin-{pluginId}` or `devtools-plugin-{pluginId}` or `{pluginId}`. It looks for the `experimentalInstallationRecipe` in their `package.json` for the steps of installation. Detailed documentation and API Spec to follow (and to be decided as well).
...
- 185fec5c0c: The default jest configuration used by the `test` command now supports yarn workspaces. By running `devtools-cli test` in the root of a monorepo, all packages will now automatically be included in the test suite and it will run just like it does within a package. Each package in the monorepo will still use its own local jest configuration, and only packages that have `devtools-cli test` in the `test` script within `package.json` will be included.
- Updated dependencies
  - @devtools/config-loader@0.6.10
...
- 13895db37: Support importing font files in tests.
  This fixes remaining issues from [#7019](https://github.com/khulnasoft/devtools/pull/7019).
- Updated dependencies
  - @devtools/cli-common@0.1.3
  - @devtools/config-loader@0.6.8
...
- 485438a56: Fix `devtools-cli backend:dev` argument passing
- Updated dependencies
  - @devtools/config@0.1.7
  - @devtools/config-loader@0.6.7
...
- ab5cc376f: Use new `isChildPath` export from `@devtools/cli-common`
- Updated dependencies
...
- 9cd3c533c: Switch from `ts-jest` to `@sucrase/jest-plugin`, improving performance and aligning transpilation with bundling.
  ...
  "jest": {
    ...
    "transform": {
      "\\.esm\\.js$": "@devtools/cli/config/jestEsmTransform.js",
      "\\.(js|jsx|ts|tsx)$": "ts-jest",
      "\\.(bmp|gif|jpg|jpeg|png|frag|xml|svg)$": "@devtools/cli/config/jestFileTransform.js",
      "\\.(yaml)$": "yaml-jest"
    }
  }
  ```

  Note that this will override the default jest transforms included with the `@devtools/cli`.
...
- 21e8ebef5: Fix error message formatting in the packaging process.
  ...
  For instance, if you break your tsconfig.json with invalid syntax, devtools-cli will not be able to build anything and it will be very hard to find out why because the underlying error message is hidden behind a TypeError.
...
- fc79a6dd3: Added lax option to devtools-cli app:build command
...
- d4f0a1406: New config command to export the configuration schema. When running devtools-cli with yarn, consider using `yarn --silent devtools-cli config:schema` to get a clean output on `stdout`.
...
- 6266ddd11: The `yarn devtools-cli app:diff` has been broken since a couple of months. The command to perform updates `yarn devtools-cli versions:bump` prints change logs which seems to be a good replacement for this command.
...
- 19fe61c27: We have updated the default `eslint` rules in the `@devtools/cli` package.
...
- b604a9d41: Append `-credentials.yaml` to credentials file generated by `devtools-cli create-github-app` and display warning about sensitive contents.
...
- f538e2c56: Make versions:bump install new versions of dependencies that were within the specified range as well as install new versions of transitive @devtools dependencies.
...
- 1722cb53c: Added support for loading and validating configuration schemas, as well as declaring config visibility through schemas.
  ...
  The new `loadConfigSchema` function exported by `@devtools/config-loader` allows for the collection and merging of configuration schemas from all nearby dependencies of the project.

  A configuration schema is declared using the `https://devtools.khulnasoft.com/schema/config-v1` JSON Schema meta schema, which is based on draft07. The only difference to the draft07 schema is the custom `visibility` keyword, which is used to indicate whether the given config value should be visible in the frontend or not. The possible values are `frontend`, `backend`, and `secret`, where `backend` is the default. A visibility of `secret` has the same scope at runtime, but it will be treated with more care in certain contexts, and defining both `frontend` and `secret` for the same value in two different schemas will result in an error during schema merging.
...
- 72f6cda35: Adds a new `DEVTOOLS_CLI_BUILD_PARELLEL` environment variable to control
  parallelism for some build steps.

  This is useful in CI to help avoid out of memory issues when using `terser`. The
  `DEVTOOLS_CLI_BUILD_PARELLEL` environment variable can be set to
