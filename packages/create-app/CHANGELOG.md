...
- f9c7bdd899: Builtin support for cookiecutter based templates has been removed from `@devtools/plugin-scaffolder-backend`. Due to this, the `containerRunner` argument to its `createRouter` has also been removed.
  ...
  -import { DockerContainerRunner } from '@devtools/backend-common';
   ...
   import { createRouter } from '@devtools/plugin-scaffolder-backend';
  ...
  If you want to retain cookiecutter support, please use the `@devtools/plugin-scaffolder-backend-module-cookiecutter` package explicitly (see [its README](https://github.com/khulnasoft/devtools/tree/master/plugins/scaffolder-backend-module-cookiecutter) for installation instructions).
...
- 1201383b60: Updated the template to write the Devtools release version to `devtools.json`, rather than the version of `@devtools/create-app`. This change is applied automatically when running `devtools-cli versions:bump` in the latest version of the Devtools CLI.
- c543fe3ff2: Postgres-based search is now installed when PG is chosen as the desired database for Devtools.

  There is no need to make this change in an existing Devtools backend. See [supported search engines](https://devtools.khulnasoft.com/docs/features/search/search-engines) for details about production-ready search engines.

- 55150919ed: - **BREAKING**: Support for `devtools.khulnasoft.com/v1beta2` Software Templates has been removed. Please migrate your legacy templates to the new `scaffolder.devtools.khulnasoft.com/v1beta3` `apiVersion` by following the [migration guide](https://devtools.khulnasoft.com/docs/features/software-templates/migrating-from-v1beta2-to-v1beta3)
- bde30664c4: Updated template to use package roles. To apply this change to an existing app, check out the [migration guide](https://devtools.khulnasoft.com/docs/tutorials/package-role-migration).
  ...
  +    "lint": "devtools-cli repo lint --since origin/master",
  +    "lint:all": "devtools-cli repo lint",
  ```
...
- ee3d6c6f10: Update the template to reflect the renaming of `DocsResultListItem` to `TechDocsSearchResultListItem` from `@devtools/plugin-techdocs`.

  To apply this change to an existing app, make the following change to `packages/app/src/components/search/SearchPage.tsx`:

  ```diff
  -import { DocsResultListItem } from '@devtools/plugin-techdocs';
...
- 022507c860: The Devtools Search Platform's indexing process has been rewritten as a stream
  ...
  `@devtools/plugin-search-backend-node`, as well as any backend plugins whose
  ...
  -import { DefaultTechDocsCollator } from '@devtools/plugin-techdocs-backend';
  +import { DefaultCatalogCollatorFactory } from '@devtools/plugin-catalog-backend';
  +import { DefaultTechDocsCollatorFactory } from '@devtools/plugin-techdocs-backend';
  ...
  Devtools backend instance, you will need to re-implement them as readable,
  transform, and writable streams respectively (including factory classes for
  instantiating them). [A how-to guide for refactoring](https://devtools.khulnasoft.com/docs/features/search/how-to-guides#rewriting-alpha-style-collators-for-beta)
...
- a686702dbe: Update the template to reflect the renaming of `CatalogResultListItem` to `CatalogSearchResultListItem` from `@devtools/plugin-catalog`.

  To apply this change to an existing app, make the following change to `packages/app/src/components/search/SearchPage.tsx`:

  ```diff
  -import { CatalogResultListItem } from '@devtools/plugin-catalog';
...
- f39c1e6036: To reflect the updated `knex` and `@vscode/sqlite3` dependencies introduced with [v0.4.19](https://github.com/khulnasoft/devtools/blob/master/packages/create-app/CHANGELOG.md#0419), we update our example `Dockerfile`, adding `@vscode/sqlite3` build dependencies to the image. Further on, we updated it to the `node:16-bullseye-slim` base image.
  ...
  If you are using a multi-stage Docker build for your app, please refer to the [updated examples](https://github.com/khulnasoft/devtools/blob/master/docs/deployment/docker.md#multi-stage-build) in the documentation.
...
- e725bb812f: Remove SearchContextProvider from `<Root />`
  ...
  -} from '@devtools/plugin-search';
  +import { SidebarSearchModal } from '@devtools/plugin-search';
...
- 22f4ecb0e6: Switched the `file:` dependency for a `link:` dependency in the `backend` package. This makes sure that the `app` package is linked in rather than copied.
  ...
  -    "app": "file:../app",
  +    "app": "link:../app",
       "@devtools/backend-common": "^{{version '@devtools/backend-common'}}",
  ```
...
- 1dd5a02e91: **BREAKING:** Updated `knex` to major version 1, which also implies changing out
  ...
  This means that as you update to this version of Devtools, there are two
  ...
  ([example](https://github.com/khulnasoft/devtools/blob/e0506af8fc54074a160fb91c83d6cae8172d3bb3/plugins/catalog-backend/migrations/20220116144621_remove_legacy.js#L17)),
...
- f27f5197e2: Apply the fix from `0.4.16`, which is part of the `v0.65.1` release of Devtools.
...
- ba59832aed: Permission the `catalog-import` route
  ...
  ...
  + import { PermissionedRoute } from '@devtools/plugin-permission-react';
  + import { catalogEntityCreatePermission } from '@devtools/plugin-catalog-common';
...
- a0d446c8ec: Replaced EntitySystemDiagramCard with EntityCatalogGraphCard
  ...
  Add `@devtools/plugin-catalog-graph` as a `dependency` in `packages/app/package.json` or `cd packages/app && yarn add @devtools/plugin-catalog-graph`.
  ...
  + } from '@devtools/plugin-catalog-graph';
  + import {
...
- b5402d6d72: Migrated the app template to React 17.
  ...
  To apply this change to an existing app, make sure you have updated to the latest version of `@devtools/cli`, and make the following change to `packages/app/package.json`:
...
- 5e8d278f8e: Added an external route binding from the `org` plugin to the catalog index page.
  ...
  This change is needed because `@devtools/plugin-org` now has a required external route that needs to be bound for the app to start.
...
- 08fa6a604a: The app template has been updated to add an explicit dependency on `typescript` in the root `package.json`. This is because it was removed as a dependency of `@devtools/cli` in order to decouple the TypeScript versioning in Devtools projects.
  ...
  Another option is to use the utilities from `@devtools/errors` to assert the type of errors caught in `catch` blocks:

  ```ts
  import { assertError, isError } from '@devtools/errors';
...
- Updated dependencies
  ...
  - @devtools/plugin-app-backend@0.3.22-next.0
  ...
  - @devtools/backend-tasks@0.1.4-next.0
  - @devtools/catalog-client@0.5.5-next.0
  - @devtools/test-utils@0.2.3-next.0
  - @devtools/plugin-proxy-backend@0.2.16-next.0
  - @devtools/plugin-rollbar-backend@0.1.19-next.0
  - @devtools/plugin-search-backend@0.3.1-next.0
  - @devtools/plugin-techdocs-backend@0.12.4-next.0
...
- cd529c4094: Add permissions to create-app's PluginEnvironment
  ...
  [`@devtools/plugin-permission-common`](https://www.npmjs.com/package/@devtools/plugin-permission-common) and [`@devtools/plugin-permission-node`](https://www.npmjs.com/package/@devtools/plugin-permission-node) will need to be installed as dependencies:
...
- 20af5a701f: The `<SearchType />` filter in the composed `SearchPage.tsx` was replaced with the `<SearchType.Accordion />` variant.
  ...
  -import { Content, Header, Page } from '@devtools/core-components';
  +import { CatalogIcon, Content, DocsIcon, Header, Page } from '@devtools/core-components';
...
- 0dcd1dd64f: Add a `scheduler` to the plugin environment, which can schedule collaborative tasks across backends. To apply the same change in your backend, follow the steps below.
  ...
  ```shell
  # From the Devtools repository root
  cd packages/backend
  yarn add @devtools/backend-tasks
  ```
  ...
  +import { PluginTaskScheduler } from '@devtools/backend-tasks';
  ...
  ```diff
   // In packages/backend/src/index.ts
  +import { TaskScheduler } from '@devtools/backend-tasks';
...
- 5fdc8df0e8: The `index.html` template of the app has been updated to use the new `config` global provided by the Devtools CLI.
...
- 1bada775a9: TechDocs Backend may now (optionally) leverage a cache store to improve
  ...
  [cm-change]: https://github.com/khulnasoft/devtools/blob/master/packages/create-app/CHANGELOG.md#patch-changes-6
  [td-rec-arch]: https://devtools.khulnasoft.com/docs/features/techdocs/architecture#recommended-deployment
...
- 24d2ce03f3: Search Modal now relies on the Search Context to access state and state setter. If you use the SidebarSearchModal as described in the [getting started documentation](https://devtools.khulnasoft.com/docs/features/search/getting-started#using-the-search-modal), make sure to update your code with the SearchContextProvider.
...
- dcaeaac174: Cleaned out the `peerDependencies` in the published version of the package, making it much quicker to run `npx @devtools/create-app` as it no longer needs to install a long list of unnecessary.
...
- 4ebc9fd277: Create devtools.json file

  `@devtools/create-app` will create a new `devtools.json` file. At this point, the file will contain a `version` property, representing the version of `@devtools/create-app` used for creating the application. If the devtools's application has been bootstrapped using an older version of `@devtools/create-app`, the `devtools.json` file can be created and kept in sync, together with all the changes of the latest version of devtools, by running the following script:
...
- 014cbf8cb9: Migrated the app template use the new `@devtools/app-defaults` for the `createApp` import, since the `createApp` exported by `@devtools/app-core-api` will be removed in the future.

  To migrate an existing application, add the latest version of `@devtools/app-defaults` as a dependency in `packages/app/package.json`, and make the following change to `packages/app/src/App.tsx`:

  ```diff
  -import { createApp, FlatRoutes } from '@devtools/core-app-api';
...
- 5dcea2586c: Integrated `SidebarSearchModal` component into default-app to use the `SearchModal`.
  ...
  If you only want to use the `SearchModal` you can import it from `'@devtools/plugin-search'`:

  ```js
  import { SearchModal } from '@devtools/plugin-search';
  ```
...
- 5725f87e4c: Updated the app template to no longer include the `--no-private` flag for the `create-plugin` command.
  ...
  -  "create-plugin": "devtools-cli create-plugin --scope internal --no-private",
...
- 5914668655: Removed `@devtools/plugin-welcome`, no new updates to the packages will be
  published in the future.

  The welcome plugin was used by early alpha versions of Devtools, but today only
  contained a simple page with welcome instructions. It was superseded by
  `@devtools/plugin-home` which can be used to build a homepage customized to the
...
- eaca0f53fb: The scaffolder plugin has just released the beta 3 version of software templates, which replaces the handlebars templating syntax. As part of this change, the template entity schema is no longer included in the core catalog-model as with previous versions. The decoupling of the template entities version will allow us to more easily make updates in the future.
  ...
  If you're interested in learning more about creating custom kinds, please check out the [extending the model](https://devtools.khulnasoft.com/docs/features/software-catalog/extending-the-model) documentation.

## 0.3.44

### Patch Changes

- e254368371: Switched the default `test` script in the package root to use `devtools-cli test` rather than `lerna run test`. This is thanks to the `@devtools/cli` now supporting running the test command from the project root.
...
- 9325075eea: Added the default `ScmAuth` implementation to the app.
  ...
  To apply this change to an existing app, head to `packages/app/apis.ts`, import `ScmAuth` from `@devtools/integration-react`, and add a `ScmAuth.createDefaultApiFactory()` to your list of APIs:
...
- 89fd81a1ab: This change adds an API endpoint for requesting a catalog refresh at `/refresh`, which is activated if a `RefreshService` is passed to `createRouter`.
  ...
  -    CatalogBuilder,
  -    createRouter,
  -  } from '@devtools/plugin-catalog-backend';
...
- 48ea3d25b: The recommended value for a `devtools.khulnasoft.com/techdocs-ref` annotation is now
...
- 98dda80b4: Update `techdocs.generators` with the latest `techdocs.generator` config in `app-config.yaml`. See
  https://devtools.khulnasoft.com/docs/features/techdocs/configuration for reference and relevant PR
  https://github.com/khulnasoft/devtools/pull/6071/files for the changes.
...
- 224e54484: Added an `EntityProcessingErrorsPanel` component to show any errors that occurred when refreshing an entity from its source location.
  ...
  If upgrading, this should be added to your `EntityPage` in your Devtools application:
...
- 03bf17e9b: Improve the responsiveness of the EntityPage UI. With this the Header component should scale with the screen size & wrapping should not cause overflowing/blocking of links. Additionally enforce the Pages using the Grid Layout to use it across all screen sizes & to wrap as intended.
  ...
  To benefit from the improved responsive layout, the `EntityPage` in existing Devtools applications should be updated to set the `xs` column size on each grid item in the page, as this does not default. For example:
...
- eb740ee24: Moved sample software templates to the [devtools/software-templates](https://github.com/khulnasoft/software-templates) repository. If you previously referenced the sample templates straight from `scaffolder-backend` plugin in the main [khulnasoft/devtools](https://github.com/khulnasoft/devtools) repository in your `app-config.yaml`, these references will need to be updated.

  See https://github.com/khulnasoft/software-templates
...
- 60e830222: Support for `Template` kinds with version `devtools.khulnasoft.com/v1alpha1` has now been removed. This means that the old method of running templates with `Preparers`, `Templaters` and `Publishers` has also been removed. If you had any logic in these abstractions, they should now be moved to `actions` instead, and you can find out more about those in the [documentation](https://devtools.khulnasoft.com/docs/features/software-templates/writing-custom-actions)

  If you need any help migrating existing templates, there's a [migration guide](https://devtools.khulnasoft.com/docs/features/software-templates/migrating-from-v1alpha1-to-v1beta2). Reach out to us on Discord in the #support channel if you're having problems.
  ...
  -   Publishers,
  -   Templaters,
  - } from '@devtools/plugin-scaffolder-backend';
  + import { createRouter } from '@devtools/plugin-scaffolder-backend';
    import Docker from 'dockerode';
    import { Router } from 'express';
    import type { PluginEnvironment } from '../types';
...
- Updated dependencies
  - @devtools/plugin-scaffolder-backend@0.12.2

## 0.3.26

### Patch Changes

- 5db7445b4: Adding .DS_Store pattern to .gitignore in Scaffolded Devtools App. To migrate an existing app that pattern should be added manually.
...
- Updated dependencies
  ...
  - @devtools/plugin-catalog-backend@0.10.3
  - @devtools/backend-common@0.8.3
  - @devtools/cli@0.7.1
  - @devtools/plugin-api-docs@0.5.0
  - @devtools/plugin-scaffolder-backend@0.12.1
  - @devtools/plugin-techdocs@0.9.6
  - @devtools/plugin-techdocs-backend@0.8.3
  - @devtools/plugin-catalog-import@0.5.10
  - @devtools/plugin-app-backend@0.3.14
  - @devtools/plugin-proxy-backend@0.2.10
  - @devtools/plugin-rollbar-backend@0.1.12
  - @devtools/plugin-search-backend@0.2.1
  - @devtools/plugin-user-settings@0.2.11
  - @devtools/catalog-model@0.8.3
  - @devtools/plugin-auth-backend@0.3.13
...
- 4f8cf50fe: Updated the `@gitbeaker/node` dependency past the broken one without a `dist` folder.
  ...
  If you get build errors that look like the following in your Devtools instance, you may want to also bump all of your `@gitbeaker/*` dependencies to at least `^30.2.0`.
  ...
  Be aware that this is only required short term until we can release our updated versions of `@devtools/plugin-scaffolder-backend`.

- 55a253de2: Migrating old `devtools.khulnasoft.com/v1alpha1` templates to `devtools.khulnasoft.com/v1beta2`
  ...
  ```diff
  -    - type: url
  -      target: https://github.com/khulnasoft/devtools/blob/master/plugins/scaffolder-backend/sample-templates/create-react-app/template.yaml
  -      rules:
  -        - allow: [Template]
  ```

- 509b5638c: Added "out-of-the-box" alpha-milestone search features to scaffolded Devtools apps.
  ...
  import { Content, Header, Lifecycle, Page } from '@devtools/core';
  import { CatalogResultListItem } from '@devtools/plugin-catalog';
  import {
    SearchBar,
    SearchFilter,
...
- Updated dependencies [875809a59]
  ...
  - @devtools/plugin-catalog-backend@0.10.2
...
- Updated dependencies [f46a9e82d]
  ...
  - @devtools/core@0.7.12
  - @devtools/plugin-tech-radar@0.4.0
  - @devtools/plugin-scaffolder-backend@0.11.5
  - @devtools/plugin-catalog-backend@0.10.1
  - @devtools/plugin-techdocs@0.9.5
...
- 6c4bd674c: Cache management has been added to the Devtools backend.
...
- f86ab6d49: Added newer entity relationship cards to the default `@devtools/create-app` template:
  ...
  The `EntityLinksCard` was also added to the overview page. To apply these to your Devtools application, compare against the updated [EntityPage.tsx](https://github.com/khulnasoft/devtools/blob/371760ca2493c8f63e9b44ecc57cc8488131ba5b/packages/create-app/templates/default-app/packages/app/src/components/catalog/EntityPage.tsx)
...
- Updated dependencies [3a181cff1]
  - @devtools/plugin-catalog-backend@0.10.0
  - @devtools/catalog-client@0.3.12
  - @devtools/catalog-model@0.8.0
  - @devtools/core@0.7.11
  - @devtools/plugin-catalog@0.6.0
  - @devtools/cli@0.6.13
  - @devtools/plugin-techdocs@0.9.4
  - @devtools/plugin-scaffolder-backend@0.11.4
  - @devtools/plugin-api-docs@0.4.15
  - @devtools/plugin-auth-backend@0.3.12
...
- Updated dependencies [675a569a9]
  ...
  - @devtools/plugin-scaffolder-backend@0.11.1
  ...
  - @devtools/plugin-catalog-backend@0.9.0
  - @devtools/plugin-app-backend@0.3.13
  - @devtools/plugin-auth-backend@0.3.10
  - @devtools/plugin-proxy-backend@0.2.8
  - @devtools/plugin-rollbar-backend@0.1.11
  - @devtools/plugin-techdocs-backend@0.8.1
...
- e0bfd3d44: The `scaffolder-backend` and `techdocs-backend` plugins have been updated.
  ...
  - import { SingleHostDiscovery } from '@devtools/backend-common';
  + import {
  +   DockerContainerRunner,
  +   SingleHostDiscovery,
  + } from '@devtools/backend-common';
...
- Updated dependencies [d1b1306d9]
  - @devtools/plugin-scaffolder-backend@0.11.0
  - @devtools/backend-common@0.7.0
  - @devtools/plugin-techdocs-backend@0.8.0
  - @devtools/plugin-catalog-import@0.5.5
  - @devtools/plugin-github-actions@0.4.5
  - @devtools/cli@0.6.10
  - @devtools/core@0.7.8
  - @devtools/plugin-catalog-backend@0.8.2
  - @devtools/theme@0.2.7
  - @devtools/plugin-tech-radar@0.3.10
  - @devtools/plugin-scaffolder@0.9.3
  - @devtools/plugin-techdocs@0.9.1
  - @devtools/plugin-proxy-backend@0.2.7
  - @devtools/catalog-model@0.7.8
  - @devtools/config@0.1.5
  - @devtools/catalog-client@0.3.11
  - @devtools/plugin-app-backend@0.3.12
  - @devtools/plugin-auth-backend@0.3.9
  - @devtools/plugin-rollbar-backend@0.1.10
...
- Updated dependencies [1a142ae8a]
  ...
  - @devtools/plugin-catalog-backend@0.8.1
  - @devtools/plugin-search@0.3.5
  - @devtools/plugin-techdocs@0.9.0
  - @devtools/plugin-scaffolder-backend@0.10.1
...
- b49a525ab: Fixing dependency resolution for problematic library `graphql-language-service-interface`.
  ...
  /tmp/devtools-e2e-uMeycm/test-app/node_modules/graphql-language-service-interface/esm/GraphQLLanguageService.js 100:23
...
- a360f9478: Expose the catalog-import route as an external route from the scaffolder.
  ...
  when you for example are running devtools with `catalog.readonly=true`.
...
- 3e7de08af: **Fully migrated the template to the new composability API**
  ...
  Since this change is large with a lot of repeated changes, we'll describe a couple of common cases rather than the entire change. If your entity pages are unchanged from the `create-app` template, you can also just bring in the latest version directly from the [template itself](https://github.com/khulnasoft/devtools/blob/master/packages/create-app/templates/default-app/packages/app/src/components/catalog/EntityPage.tsx).
...
- Updated dependencies [676ede643]
  - @devtools/plugin-catalog-backend@0.7.0
  - @devtools/plugin-scaffolder@0.8.1
  - @devtools/plugin-scaffolder-backend@0.9.4
  - @devtools/plugin-auth-backend@0.3.7
  - @devtools/plugin-api-docs@0.4.10
  - @devtools/plugin-github-actions@0.4.3
  - @devtools/plugin-catalog@0.5.2
  - @devtools/plugin-techdocs@0.7.1
  - @devtools/catalog-client@0.3.9
  - @devtools/plugin-catalog-import@0.5.1
  - @devtools/plugin-explore@0.3.3
  - @devtools/catalog-model@0.7.5
  - @devtools/backend-common@0.6.1
  - @devtools/plugin-user-settings@0.2.9
...
- 2c525f85e: (fix) Adds locationAnalyzer to default-app template
  ...
  This resulted in 404 errors in newly bootstrapped devtools applications,
  when adding components without configuration.

  To fix this in an existing devtools application, the locationAnalyzer needs
...
- f88fe9dd9: Adds plugin-org and more capability to the default EntityPage to display Users, Groups and Systems.
  ...
  +    - allow: [Component, System, API, Group, User, Template, Location]
    locations:
      # Devtools example components
      - type: url
        target: https://github.com/khulnasoft/devtools/blob/master/packages/catalog-model/examples/all-components.yaml

  +    # Devtools example systems
  +    - type: url
  +      target: https://github.com/khulnasoft/devtools/blob/master/packages/catalog-model/examples/all-systems.yaml
  +
      # Devtools example APIs
  ```

  Additionally, the default app sidebar was updated to parity with the Devtools
  repo. You can see these changes in the template
  [App.tsx](https://github.com/khulnasoft/devtools/blob/8817a87cdd5c881fbe8a43557ba7f9df0f9e3258/packages/create-app/templates/default-app/packages/app/src/App.tsx#L70)
  referencing a new `Root` component.

  Finally, compare your `packages/app/src/components/catalog/EntityPage.tsx` to
  [EntityPage](https://github.com/khulnasoft/devtools/blob/8817a87cdd5c881fbe8a43557ba7f9df0f9e3258/packages/create-app/templates/default-app/packages/app/src/components/catalog/EntityPage.tsx)
  from the `@devtools/create-app` default template to pick up additional
  changes there.
...
- Updated dependencies [df59930b3]
  - @devtools/plugin-scaffolder-backend@0.9.3
  - @devtools/plugin-github-actions@0.4.2
  - @devtools/plugin-catalog@0.5.1
  - @devtools/plugin-techdocs@0.7.0
  - @devtools/plugin-techdocs-backend@0.7.0
  - @devtools/plugin-auth-backend@0.3.6
  - @devtools/core@0.7.3
  - @devtools/plugin-catalog-backend@0.6.7
...
- 3385b374b: Supply a `scmIntegrationsApiRef` from the new `@devtools/integration-react`.
  ...
  ```diff
     "dependencies": {
  +    "@devtools/integration-react": "^0.1.1",
  ```
...
- 028339210: Adds example groups and users to the default app template.
  ...
  ```diff
       - type: url
         target: https://github.com/khulnasoft/devtools/blob/master/packages/catalog-model/examples/all-apis.yaml

  +    # Devtools example organization groups
  +    - type: url
  +      target: https://github.com/khulnasoft/devtools/blob/master/packages/catalog-model/examples/acme/org.yaml
  +      rules:
  +        - allow: [Group, User]
  +
       # Devtools example templates
       - type: url
         target: https://github.com/khulnasoft/devtools/blob/master/plugins/scaffolder-backend/sample-templates/react-ssr-template/template.yaml
  ```
...
- Updated dependencies [8686eb38c]
  - @devtools/plugin-catalog-backend@0.6.6
  - @devtools/plugin-catalog@0.5.0
  - @devtools/catalog-client@0.3.8
  ...
  - @devtools/plugin-techdocs-backend@0.6.5
  - @devtools/plugin-scaffolder-backend@0.9.2
  ...
  - @devtools/plugin-auth-backend@0.3.5
  - @devtools/test-utils@0.1.9
  - @devtools/plugin-app-backend@0.3.10
  - @devtools/plugin-proxy-backend@0.2.6
  - @devtools/plugin-rollbar-backend@0.1.8
...
- b03fba0dc: Adds "yarn dev" command to simplify local development.
  ...
  You can then run `yarn dev` which will start both the Devtools frontend and backend in a single window.
...
- Updated dependencies [2e57922de]
  ...
  - @devtools/plugin-catalog-backend@0.6.5
  - @devtools/plugin-catalog@0.4.2
  - @devtools/backend-common@0.5.6
  - @devtools/plugin-app-backend@0.3.9
  - @devtools/plugin-scaffolder-backend@0.9.1
  - @devtools/catalog-model@0.7.4
  - @devtools/catalog-client@0.3.7
  - @devtools/core@0.7.1
  - @devtools/plugin-techdocs-backend@0.6.4
  - @devtools/plugin-techdocs@0.6.1
  - @devtools/plugin-auth-backend@0.3.4
...
- f71589800: The api-docs plugin has been migrated to use an [external route reference](https://devtools.khulnasoft.com/docs/plugins/composability#binding-external-routes-in-the-app) to dynamically link to the create component page.
  ...
  ```diff
  + import { apiDocsPlugin } from '@devtools/plugin-api-docs';
...
```diff
- import { Router as DocsRouter } from '@devtools/plugin-techdocs';
+ import { TechdocsPage } from '@devtools/plugin-techdocs';
  import { CatalogImportPage } from '@devtools/plugin-catalog-import';
- import { Router as TechRadarRouter } from '@devtools/plugin-tech-radar';
- import { SearchPage as SearchRouter } from '@devtools/plugin-search';
- import { Router as SettingsRouter } from '@devtools/plugin-user-settings';
+ import { TechRadarPage } from '@devtools/plugin-tech-radar';
+ import { SearchPage } from '@devtools/plugin-search';
+ import { UserSettingsPage } from '@devtools/plugin-user-settings';
+ import { ApiExplorerPage } from '@devtools/plugin-api-docs';
  import { EntityPage } from './components/catalog/EntityPage';
  import { scaffolderPlugin, ScaffolderPage } from '@devtools/plugin-scaffolder';
...
If you have added additional plugins with registered routes or are using `Router` components from other plugins, these should be migrated to use the `*Page` components as well. See [this commit](https://github.com/khulnasoft/devtools/commit/abd655e42d4ed416b70848ffdb1c4b99d189f13b) for more examples of how to migrate.

For more information and the background to this change, see the [composability system migration docs](https://devtools.khulnasoft.com/docs/plugins/composability).
...
- Updated dependencies [dbea11072]
  ...
  - @devtools/plugin-scaffolder-backend@0.9.0
  - @devtools/cli@0.6.3
  - @devtools/plugin-techdocs-backend@0.6.3
  - @devtools/plugin-catalog-backend@0.6.4
  - @devtools/plugin-api-docs@0.4.8
  - @devtools/plugin-catalog@0.4.1
  - @devtools/catalog-model@0.7.3
  - @devtools/backend-common@0.5.5
  - @devtools/plugin-proxy-backend@0.2.5
  - @devtools/plugin-auth-backend@0.3.3
...
- 08fa2176a: **BREAKING CHANGE**
  ...
  The default catalog client comes from the `@devtools/catalog-client`, which you need to add as a dependency in `packages/backend/package.json`.
  ...
  See the `@devtools/scaffolder-backend` changelog for more information about this change.
...
- d50e9b81e: Updated docker build to use `devtools-cli backend:bundle` instead of `devtools-cli backend:build-image`.
  ...
  -  "build-image": "devtools-cli backend:build-image --build --tag devtools",
  +  "build": "devtools-cli backend:bundle",
  +  "build-image": "docker build ../.. -f Dockerfile --tag devtools",
  ```
...
- Updated dependencies [688b73110]
  ...
  - @devtools/plugin-techdocs-backend@0.6.1
  - @devtools/plugin-catalog-backend@0.6.2
...
- 30e200d12: `@devtools/plugin-catalog-import` has been refactored, so the `App.tsx` of the devtools apps need to be updated:
...
- Updated dependencies [804502a5c]
  ...
  - @devtools/plugin-auth-backend@0.3.0
  - @devtools/plugin-catalog@0.3.1
  - @devtools/plugin-scaffolder@0.5.0
  - @devtools/plugin-scaffolder-backend@0.7.0
  - @devtools/plugin-catalog-backend@0.6.1
  - @devtools/plugin-circleci@0.2.8
  - @devtools/plugin-search@0.3.0
  - @devtools/plugin-app-backend@0.3.7
  ...
  - @devtools/plugin-techdocs-backend@0.6.0
  - @devtools/core@0.6.1
...
- 019fe39a0: **BREAKING CHANGE**: The `useEntity` hook has been moved from `@devtools/plugin-catalog` to `@devtools/plugin-catalog-react`.
  To apply this change to an existing app, add `@devtools/plugin-catalog-react` to your dependencies in `packages/app/package.json`, and update
  the import inside `packages/app/src/components/catalog/EntityPage.tsx` as well as any other places you were using `useEntity` or any other functions that were moved to `@devtools/plugin-catalog-react`.
...
- Updated dependencies [11cb5ef94]
  ...
  - @devtools/plugin-techdocs-backend@0.5.5
  - @devtools/cli@0.6.0
  - @devtools/core@0.6.0
  ...
  - @devtools/plugin-catalog-backend@0.6.0
  - @devtools/plugin-techdocs@0.5.5
  - @devtools/plugin-user-settings@0.2.5
  - @devtools/catalog-model@0.7.1
  - @devtools/plugin-scaffolder-backend@0.6.0
  - @devtools/plugin-app-backend@0.3.6
  ...
  - @devtools/plugin-auth-backend@0.2.12
  - @devtools/plugin-proxy-backend@0.2.4
  - @devtools/plugin-rollbar-backend@0.1.7
...
- Updated dependencies [4eaa06057]
  ...
  - @devtools/plugin-scaffolder-backend@0.5.2
  - @devtools/cli@0.5.0
  - @devtools/plugin-catalog@0.2.14
  - @devtools/plugin-catalog-backend@0.5.5
  ...
  - @devtools/plugin-app-backend@0.3.5
  ...
  - @devtools/plugin-techdocs-backend@0.5.4
...
- Updated dependencies [a08db734c]
  - @devtools/plugin-catalog@0.2.13
  - @devtools/plugin-scaffolder-backend@0.5.1
...
- 89278acab: Migrate to using `FlatRoutes` from `@devtools/core` for the root app routes.

  This is the first step in migrating applications as mentioned here: https://devtools.khulnasoft.com/docs/plugins/composability#porting-existing-apps.

  To apply this change to an existing app, switch out the `Routes` component from `react-router` to `FlatRoutes` from `@devtools/core`.
...
- 26d3b24f3: fix routing and config for user-settings plugin
  ...
  ```
  export { plugin as UserSettings } from '@devtools/plugin-user-settings';
  ```
...
- Updated dependencies [a93f42213]
  - @devtools/catalog-model@0.7.0
  - @devtools/plugin-catalog-backend@0.5.4
  - @devtools/plugin-github-actions@0.3.0
  - @devtools/core@0.5.0
  ...
  - @devtools/plugin-scaffolder-backend@0.5.0
  - @devtools/plugin-techdocs@0.5.4
  - @devtools/plugin-techdocs-backend@0.5.4
  - @devtools/plugin-auth-backend@0.2.11
  - @devtools/plugin-lighthouse@0.2.8
  - @devtools/plugin-circleci@0.2.6
  - @devtools/plugin-search@0.2.6
  - @devtools/plugin-explore@0.2.3
  - @devtools/plugin-tech-radar@0.3.3
  - @devtools/plugin-user-settings@0.2.4
  - @devtools/plugin-app-backend@0.3.4
  - @devtools/plugin-proxy-backend@0.2.4
  - @devtools/plugin-rollbar-backend@0.1.7
...
- 1773a5182: Removed lighthouse plugin from the default set up plugins, as it requires a separate Backend to function.
  ...
  2. The `@devtools/plugin-lighthouse` dependency from `packages/app/package.json`.
  3. The `@devtools/plugin-lighthouse` re-export from `packages/app/src/plugins.ts`.
...
- 4e0e3b1bf: Add missing `yarn clean` for app.
  ...
  For users with existing Devtools installations, add the following under the `scripts` section in `packages/app/package.json`, after the "lint" entry:
...
- a8573e53b: techdocs-backend: Simplified file, removing individual preparers and generators.
  ...
  In your Devtools app, `packages/backend/plugins/techdocs.ts` file has now been simplified,
  to remove registering individual preparers and generators.

  Please update the file when upgrading the version of `@devtools/plugin-techdocs-backend` package.
...
- 2783ec018: In the techdocs-backend plugin (`packages/backend/src/plugins/techdocs.ts`), create a publisher using
  ...
  Read more about the configs here https://devtools.khulnasoft.com/docs/features/techdocs/configuration
...
- 4a655c89d: Removed `"resolutions"` entry for `esbuild` in the root `package.json` in order to use the version specified by `@devtools/cli`.
...
- ea475893d: Add [API docs plugin](https://github.com/khulnasoft/devtools/tree/master/plugins/api-docs) to new apps being created through the CLI.

## 0.2.2

### Patch Changes

- 7d7abd50c: Add `app-backend` as a backend plugin, and make a single docker build of the backend the default way to deploy devtools.
  ...
  First, install the `@devtools/plugin-app-backend` plugin package in your backend. These changes where made for `v0.3.0` of the plugin, and the installation process might change in the future. Run the following from the root of the repo:

  ```bash
  cd packages/backend
  yarn add @devtools/plugin-app-backend
  ```
  ...
  ```ts
  import { createRouter } from '@devtools/plugin-app-backend';
  import { PluginEnvironment } from '../types';
  ...
  Since the backend image is now the only one needed for a simple Devtools deployment, the image tag name in the `build-image` script inside `packages/backend/package.json` was changed to the following:
...
- 61db1ddc6: Allow node v14 and add to master build matrix
  ...
  - Upgrade sqlite3@^5.0.0 in @devtools/plugin-catalog-backend
  - Add Node 14 to engines in @devtools/create-app
...
- 440a17b39: Bump @devtools/catalog-backend and pass the now required UrlReader interface to the plugin
