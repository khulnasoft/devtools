...
export default async (opts: OptionValues): Promise<void> => {
  ...
  const answers: Answers = await inquirer.prompt([
    {
      ...
      when: (a: Answers) => {
        const envName = process.env.DEVTOOLS_APP_NAME;
        if (envName) {
          a.name = envName;
          return false;
        }
  ...
  try {
      ...
      Task.log(
        chalk.yellow(
          [
            ...
            '         https://github.com/khulnasoft/devtools/issues for potential solutions',
          ].join('\n'),
        ),
      );
    }
    ...
    Task.log(
      '  Set up the software catalog: https://devtools.khulnasoft.com/docs/features/software-catalog/configuration',
    );
    Task.log('  Add authentication: https://devtools.khulnasoft.com/docs/auth/');
    Task.log();
    Task.exit();
  } catch (error) {
