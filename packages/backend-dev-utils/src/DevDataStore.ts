...
import { DevtoolsIpcClient, ipcClient } from './ipcClient';
...
export class DevDataStore {
   ...
   * process is being run through the `@devtools/cli` in development mode.
   *
  ...
  /** @internal */
  static forTest(client: Pick<DevtoolsIpcClient, 'request'>): DevDataStore {
    return new DevDataStore(client as DevtoolsIpcClient);
  }

  #client: DevtoolsIpcClient;

  private constructor(client: DevtoolsIpcClient) {
    this.#client = client;
  }
