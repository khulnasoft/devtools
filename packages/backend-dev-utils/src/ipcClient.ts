...
const requestType = '@devtools/cli/channel/request';
const responseType = '@devtools/cli/channel/response';
 ...
 * @internal
 */
export class DevtoolsIpcClient {
  #messageId = 0;
  #sendMessage: SendMessage;
  #handlers: Map<number, ResponseHandler> = new Map();

  /**
   * Creates a new client if we're in a child process with IPC and DEVTOOLS_CLI_CHANNEL is set.
   */
  static create(): DevtoolsIpcClient | undefined {
    const sendMessage = process.send?.bind(process);
    const client =
      sendMessage && process.env.DEVTOOLS_CLI_CHANNEL
        ? new DevtoolsIpcClient(sendMessage)
        : undefined;
...
export const ipcClient = DevtoolsIpcClient.create();
