...
import { ConflictError, stringifyError } from '@devtools/errors';
...
/**
 * Keep in sync with `@devtools/backend-plugin-api/src/services/system/types.ts`
 * @internal
 */
...
function toInternalServiceFactory<TService, TScope extends 'plugin' | 'root'>(
  ...
  if (f.$$type !== '@devtools/BackendFeature') {
