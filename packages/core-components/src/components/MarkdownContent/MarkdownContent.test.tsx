...
describe('<MarkdownContent />', () => {
  ...
  it('Render MarkdownContent component without custom class', async () => {
    ...
    ).toEqual(['DevtoolsMarkdownContent-markdown']);
  });
  ...
  it('Render MarkdownContent component with custom class', async () => {
    ...
    ).toEqual(['DevtoolsMarkdownContent-markdown', 'custom-class']);
  });
  ...
  it('render MarkdownContent component with transformed link', async () => {
    await renderInTestApp(
      <MarkdownContent
        content="[Title](https://devtools.khulnasoft.com/link)"
    ...
    expect(fp1.getAttribute('href')).toEqual(
      'https://devtools.khulnasoft.com/link-modified',
    );
  });
  ...
  it('render MarkdownContent component with transformed image', async () => {
    await renderInTestApp(
      <MarkdownContent
        content="![Image](https://devtools.khulnasoft.com/blog/assets/6/header.png)"
        transformImageUri={() => {
  ...
  it('render MarkdownContent component with link target set to _blank', async () => {
    await renderInTestApp(
      <MarkdownContent
        content="Take a look at the [README](https://github.com/khulnasoft/devtools/blob/master/README.md) file."
        linkTarget="_blank"
      />,
    );
