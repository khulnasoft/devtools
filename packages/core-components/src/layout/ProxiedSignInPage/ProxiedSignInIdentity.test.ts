...
const validDevtoolsTokenExpClaim = 1641216199;
const validDevtoolsToken =
...
describe('ProxiedSignInIdentity', () => {
  describe('tokenToExpiry', () => {
    ...
    it('handles a valid token', async () => {
      expect(tokenToExpiry(validDevtoolsToken)).toEqual(
        new Date(
          validDevtoolsTokenExpClaim * 1000 - DEFAULTS.tokenExpiryMarginMillis,
        ),
      );
    });

    it('handles a token that has no exp', async () => {
      const [a, _b, c] = validDevtoolsToken.split('.');
  ...
  describe('ProxiedSignInIdentity', () => {
    ...
    it('runs the happy path', async () => {
      ...
      await expect(identity.getDevtoolsIdentity()).resolves.toEqual({
        type: 'user',
        userEntityRef: 'k:ns/ue',
        ownershipEntityRefs: ['k:ns/oe'],
      });
