...
export const DEFAULTS = {
  ...
  // The amount of time before the actual expiry of the Devtools token, that we
...
export class ProxiedSignInIdentity implements IdentityApi {
  ...
  getUserId(): string {
    const { devtoolsIdentity } = this.getSessionSync();
    const ref = devtoolsIdentity.identity.userEntityRef;
  ...
  async getIdToken(): Promise<string | undefined> {
    ...
    return session.devtoolsIdentity.token;
  }
  ...
  async getDevtoolsIdentity(): Promise<DevtoolsUserIdentity> {
    const session = await this.getSessionAsync();
    return session.devtoolsIdentity.identity;
  }
  ...
  async getCredentials(): Promise<{ token?: string | undefined }> {
    const session = await this.getSessionAsync();
    return {
      token: session.devtoolsIdentity.token,
    };
  }
    ...
    const promise = this.fetchSession().then(
      session => {
        this.state = {
          type: 'active',
          session,
          expiresAt: tokenToExpiry(session.devtoolsIdentity.token),
        };
        return session;
      },
      error => {
