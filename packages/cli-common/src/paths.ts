...
export type Paths = {
  ...
  // Monorepo root dir of the cli itself. Only accessible when running inside Devtools repo.
  ownRoot: string;
  ...
  // Resolve a path relative to own monorepo root. Only accessible when running inside Devtools repo.
...
// Finds the root of the monorepo that the package exists in. Only accessible when running inside Devtools repo.
export function findOwnRootDir(ownDir: string) {
  const isLocal = fs.existsSync(resolvePath(ownDir, 'src'));
  if (!isLocal) {
    throw new Error(
      'Tried to access monorepo package root dir outside of Devtools repository',
    );
  }
...
export function findPaths(searchDir: string): Paths {
  ...
  // Lazy load this as it will throw an error if we're not inside the Devtools repo.
  let ownRoot = '';
  const getOwnRoot = () => {
    if (!ownRoot) {
...
/**
 * The name of the devtools's config file
 *
 * @public
 */
export const DEVTOOLS_JSON = 'devtools.json';
