 ...
 * {@link @devtools/plugin-permission-common#PermissionClient} that allows all
...
export class ServerPermissionClient implements PermissionsService {
  ...
  static fromConfig(
    ...
    if (
      ...
      throw new Error(
        'Service-to-service authentication must be configured before enabling permissions. Read more here https://devtools.khulnasoft.com/docs/auth/service-to-service-auth',
      );
    }
  ...
  async #getIncomingCredentials(
    options?: PermissionsServiceRequestOptions,
  ): Promise<DevtoolsCredentials | undefined> {
    if (options && 'credentials' in options) {
  ...
  #servicePrincipalDecision(
    ...
    credentials: DevtoolsCredentials<DevtoolsServicePrincipal>,
  ): DefinitivePolicyDecision[] {
