...
describe('createPermissionIntegrationRouter', () => {
  ...
  describe('POST /.well-known/devtools/permissions/apply-conditions', () => {
    it.each([
      {
        rule: 'test-rule-1',
        resourceType: 'test-resource',
      ...
      let response = await request(createApp())
        .post('/.well-known/devtools/permissions/apply-conditions')
        .send({
          items: [
            {
              id: '123',
  ...
  describe('GET /.well-known/devtools/permissions/metadata', () => {
    it('returns a list of permissions and rules of a single resource type', async () => {
      const response = await request(createApp()).get(
        '/.well-known/devtools/permissions/metadata',
      );
    ...
    it('returns a list of permissions and rules from multiple resource types', async () => {
      ...
      ).get('/.well-known/devtools/permissions/metadata');
