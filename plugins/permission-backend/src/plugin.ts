...
import {
  ...
  PolicyExtensionPoint,
} from '@devtools/plugin-permission-node/alpha';
  ...
  register(env) {
    ...
    env.registerInit({
      ...
      async init({
        ...
        if (!policies.policy) {
          throw new Error(
            'No policy module installed! Please install a policy module. If you want to allow all requests, use @devtools/plugin-permission-backend-module-allow-all-policy permissionModuleAllowAllPolicy',
          );
        }
