...
- c689d7a94c: **DEPRECATION**: The `FilteredEntityLayout` and related components have been moved to `@devtools/plugin-catalog-react` and renamed. The original components are now deprecated and should be replaced as follows:
...
- 37092662cb: **BREAKING** Completely removed the `EntitySystemDiagramCard` component which was deprecated in a previous release. Any remaining references to this component are now broken and should be replaced with `EntityCatalogGraphCard`, which can be imported from package `@devtools/plugin-catalog-graph`.
...
- aac316279f: Deprecated `EntitySystemDiagramCard`
  `EntitySystemDiagramCard` is replaced by `EntityCatalogGraphCard` which is imported from `@devtools/plugin-catalog-graph`. This component will be removed in an upcoming release
- Updated dependencies
...
- 3fa31ec84a: Deprecated the `CatalogClientWrapper` class.
  ...
  The default implementation of `catalogApiRef` that this plugin exposes, is now powered by the new `fetchApiRef`. The default implementation of _that_ API, in turn, has the ability to inject the user's Devtools token in requests in a similar manner to what the deprecated `CatalogClientWrapper` used to do. The latter has therefore been taken out of the default catalog API implementation.
...
- 7a4bd2ceac: Prefer using `Link` from `@devtools/core-components` rather than material-UI.
- Updated dependencies
...
- 82fbda923e: Introduce a new `StarredEntitiesApi` that is used in the `useStarredEntities` hook.
  The `@devtools/plugin-catalog` installs a default implementation that is backed by the `StorageApi`, but one can also override the `starredEntitiesApiRef`.
  ...
  A migration will convert the previously starred entities to the location on the first load of Devtools.
...
- 3ed78fca3: Added the ability to switch entity kind on the catalog index page. This is a non-breaking change, but if you created a custom `CatalogPage` and wish to use this feature, make the modifications shown on [#6895](https://github.com/khulnasoft/devtools/pull/6895).
...
- 787bc0826: **NOTE**: The entity `<AboutCard />` now uses an external route ref to link to
  ...
  [cacl]: https://github.com/khulnasoft/devtools/blob/master/packages/create-app/CHANGELOG.md
...
- Updated dependencies
  - @devtools/plugin-catalog-react@0.2.5
...
- 7028ee1ca: Expose `getEntitySourceLocation`, `getEntityMetadataViewUrl`, and
  `getEntityMetadataEditUrl` from `@devtools/plugin-catalog-react`.
...
- 17c497b81: The default `CatalogPage` has been reworked to be more composable and make
  ...
  [CatalogPage.tsx](https://github.com/khulnasoft/devtools/blob/9a4baa74509b6452d7dc054d34cf079f9997166d/plugins/catalog/src/components/CatalogPage/CatalogPage.tsx)
  ...
  See the documentation added on [Catalog Customization](https://devtools.khulnasoft.com/docs/features/software-catalog/catalog-customization)
  for instructions.
...
- Updated dependencies [8488a1a96]
  ...
  - @devtools/plugin-catalog-react@0.1.4
...
- 3385b374b: Use `scmIntegrationsApiRef` from the new `@devtools/integration-react`.
...
- 32a950409: Hide the kind of the owner if it's the default kind for the `ownedBy`
  relationship (group).
- f10950bd2: Minor refactoring of DevtoolsApp.getSystemIcons to support custom registered
...
- 0af242b6d: Introduce new cards to `@devtools/plugin-catalog` that can be added to entity pages:
  ...
  - In addition, `EntityHasApisCard` to display APIs of a system is added to `@devtools/plugin-api-docs`.

  `@devtools/plugin-catalog-react` now provides an `EntityTable` to build own cards for entities.
...
- 1df75733e: Adds an `EntityLinksCard` component to display `entity.metadata.links` on entity pages. The new component is a companion for the new [Entity Links](https://devtools.khulnasoft.com/docs/features/software-catalog/descriptor-format#links-optional) catalog model addition.
...
- 019fe39a0: `@devtools/plugin-catalog` stopped exporting hooks and helpers for other
  plugins. They are migrated to `@devtools/plugin-catalog-react`.
...
- 7fc89bae2: Display owner and system as entity page links in the tables of the `api-docs`
  ...
  Move `isOwnerOf` and `getEntityRelations` from `@devtools/plugin-catalog` to
  `@devtools/plugin-catalog-react` and export it from there to use it by other
  plugins.
...
- a08db734c: Remove the `WelcomeBanner` that links to a plugin that is not longer wired into Devtools instances

## 0.2.12

### Patch Changes

- 593632f07: Derive the list of to-delete entities in the `UnregisterEntityDialog` from the `devtools.khulnasoft.com/managed-by-origin-location` annotation.
...
- 9c09a364f: Remove the unused dependency to `@devtools/plugin-techdocs`.
...
- Updated dependencies [359f9d2d8]
  - @devtools/core@0.4.3
  - @devtools/plugin-techdocs@0.5.2
