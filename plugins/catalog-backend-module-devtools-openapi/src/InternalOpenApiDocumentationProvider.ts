...
import { getOpenApiSpecRoute } from '@devtools/backend-openapi-utils';
import type {
  OpenAPIObject,
  OperationObject,
  PathItemObject,
...
export class InternalOpenApiDocumentationProvider implements EntityProvider {
  ...
  async refresh(logger: LoggerService) {
    const pluginsToMerge = this.config.getStringArray(
      'catalog.providers.devtoolsOpenapi.plugins',
    );
    const configToMerge = this.config.getOptional(
      'catalog.providers.devtoolsOpenapi.entityOverrides',
    );

    const baseConfig = {
      metadata: {
        name: 'internal_devtools_openapi_doc',
        title: 'Devtools API',
      },
      spec: {
    ...
    const requiredConfig = {
      ...
      metadata: {
        annotations: {
          [ANNOTATION_LOCATION]:
            'internal-package:@devtools/plugin-catalog-backend-module-devtools-openapi',
          [ANNOTATION_ORIGIN_LOCATION]:
            'internal-package:@devtools/plugin-catalog-backend-module-devtools-openapi',
        },
      },
