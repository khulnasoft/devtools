...
import { entityKindSchemaValidator } from '@devtools/catalog-model';
import type {
...
describe('templateEntityV1beta3Validator', () => {
  ...
  beforeEach(() => {
    entity = {
      ...
      spec: {
        ...
        parameters: {
          required: ['owner'],
          'devtools:permissions': { tags: ['one', 'two'] },
          properties: {
            owner: {
              type: 'string',
              title: 'Owner',
  ...
  it('ignores unknown apiVersion', async () => {
    (entity as any).apiVersion = 'devtools.khulnasoft.com/v1beta0';
  ...
  it('rejects parameters with wrong devtools:permissions', async () => {
  ...
  it('rejects steps with wrong devtools:permissions', async () => {
    entity.spec.steps[0]['devtools:permissions']!.tags = true as unknown as [];
    expect(() => validator(entity)).toThrow(/must be array/);

    entity.spec.steps[0]['devtools:permissions'] = true as {};
    expect(() => validator(entity)).toThrow(/must be object/);
  });
});
