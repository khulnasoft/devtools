...
export class DevToolsBackendApi {
  ...
  public async listInfo(): Promise<DevToolsInfo> {
    ...
    /* eslint-disable-next-line no-restricted-syntax */
    const paths = findPaths(__dirname);
    const devtoolsFile = paths.resolveTargetRoot('devtools.json');
    let devtoolsJson = undefined;
    if (fs.existsSync(devtoolsFile)) {
      const buffer = await fs.readFile(devtoolsFile);
      devtoolsJson = JSON.parse(buffer.toString());
    }

    const lockfilePath = paths.resolveTargetRoot('yarn.lock');
    const lockfile = await Lockfile.load(lockfilePath);

    const prefixes = ['@devtools', '@internal'].concat(
    ...
    const info: DevToolsInfo = {
      ...
      devtoolsVersion:
        devtoolsJson && devtoolsJson.version ? devtoolsJson.version : 'N/A',
