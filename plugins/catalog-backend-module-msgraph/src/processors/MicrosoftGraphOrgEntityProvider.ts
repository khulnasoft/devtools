...
export type MicrosoftGraphOrgEntityProviderOptions =
  ...
  | {
       ...
       * {@link @devtools/backend-plugin-api#SchedulerService.createScheduledTaskRunner}
...
export class MicrosoftGraphOrgEntityProvider implements EntityProvider {
  ...
  private static fromLegacyConfig(
    ...
    options.logger.warn(
      'Deprecated msgraph config "catalog.processors.microsoftGraphOrg" used. Use "catalog.providers.microsoftGraphOrg" instead. More info at https://github.com/khulnasoft/devtools/blob/master/plugins/catalog-backend-module-msgraph/CHANGELOG.md#040-next1',
    );
  ...
  /** {@inheritdoc @devtools/plugin-catalog-backend#EntityProvider.getProviderName} */
  getProviderName() {
    return `MicrosoftGraphOrgEntityProvider:${this.options.id}`;
  }

  /** {@inheritdoc @devtools/plugin-catalog-backend#EntityProvider.connect} */
