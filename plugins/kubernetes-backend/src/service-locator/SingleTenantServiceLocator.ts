...
// Therefore it will always return specified cluster provided in devtools.khulnasoft.com/kubernetes-cluster annotation
// If devtools.khulnasoft.com/kubernetes-cluster annotation not provided will always return all cluster provided
export class SingleTenantServiceLocator implements KubernetesServiceLocator {
  ...
  getClustersByEntity(
    ...
    return this.clusterSupplier
      ...
      .then(clusters => {
        if (
          _entity.metadata?.annotations?.['devtools.khulnasoft.com/kubernetes-cluster']
        ) {
          return {
            clusters: clusters.filter(
              c =>
                ...
                _entity.metadata?.annotations?.[
                  'devtools.khulnasoft.com/kubernetes-cluster'
                ],
            ),
          };
        }
