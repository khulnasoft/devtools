...
import { ExtendedHttpServer } from '@devtools/backend-defaults/rootHttpRouter';
...
describe('API integration tests', () => {
  ...
  beforeEach(async () => {
    ...
    jest.mock('@devtools/catalog-client', () => ({
    ...
    const { server } = await startTestBackend({
      features: [
        ...
        import('@devtools/plugin-kubernetes-backend'),
        import('@devtools/plugin-permission-backend'),
...
`;
  ...
  it('serves permission integration endpoint', async () => {
    const response = await request(app)
      .get('/api/kubernetes/.well-known/devtools/permissions/metadata')
