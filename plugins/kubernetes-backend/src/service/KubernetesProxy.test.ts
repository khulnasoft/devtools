...
} from '@devtools/plugin-kubernetes-common';
...
import { MiddlewareFactory } from '@devtools/backend-defaults/rootHttpRouter';
...
describe('KubernetesProxy', () => {
  ...
  it('should default to using a strategy-provided bearer token as authorization headers to kubeapi when devtools-kubernetes-auth field is not provided', async () => {
    worker.use(
      rest.get(
  ...
  it('should append the Devtools-Kubernetes-Auth field to the requests authorization header if one is provided', async () => {
    worker.use(
      rest.get('https://localhost:9999/api/v1/namespaces', (req, res, ctx) => {
        if (!req.headers.get('Authorization')) {
          return res(ctx.status(401));
  ...
  it('should not invoke authStrategy if Devtools-Kubernetes-Authorization field is provided', async () => {
    worker.use(
      rest.get('https://localhost:9999/api/v1/namespaces', (req, res, ctx) => {
        if (!req.headers.get('Authorization')) {
          return res(ctx.status(401));
  ...
  it('should invoke AuthStrategy if Devtools-Kubernetes-Authorization-X-X are provided', async () => {
    ...
    const requestPromise = setupProxyPromise({
      ...
      headers: {
        [HEADER_KUBERNETES_CLUSTER]: 'cluster1',
        'Devtools-Kubernetes-Authorization-google': 'MY_TOKEN1',
        'Devtools-Kubernetes-Authorization-aks': 'MY_TOKEN2',
        'Devtools-Kubernetes-Authorization-oidc-okta': 'MY_TOKEN3',
        'Devtools-Kubernetes-Authorization-oidc-gitlab': 'MY_TOKEN4',
        'Devtools-Kubernetes-Authorization-pinniped-audience1': 'MY_TOKEN5',
        'Devtools-Kubernetes-Authorization-pinniped-au-b-c-d-e': 'MY_TOKEN6',
      },
    });
  ...
  it('should invoke the Auth strategy with an empty auth object when no Devtools-Kubernetes-Authorization-X-X are provided', async () => {
    worker.use(
  ...
  it('returns a 500 error if authStrategy errors out and Devtools-Kubernetes-Authorization field is not provided', async () => {
    worker.use(
      rest.get('https://localhost:9999/api/v1/namespaces', (req, res, ctx) => {
        if (!req.headers.get('Authorization')) {
          return res(ctx.status(401));
  ...
  describe('when server uses TLS', () => {
    ...
    it('should use a x509 client cert authentication strategy to consume kubeapi when devtools-kubernetes-auth field is not provided and the authStrategy enables x509 client cert authentication', async () => {
      worker.use(
        rest.get(
          'https://localhost:9999/api/v1/namespaces',
          (req: any, res: any, ctx: any) => {
  ...
  describe('Devtools running on k8s', () => {
