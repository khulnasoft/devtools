...
export class KubernetesFanOutHandler implements KubernetesObjectsProvider {
  ...
  private async fanOutRequests(
    ...
    const entityName =
      entity.metadata?.annotations?.['devtools.khulnasoft.com/kubernetes-id'] ||
    ...
    const labelSelector: string =
      entity.metadata?.annotations?.[
        'devtools.khulnasoft.com/kubernetes-label-selector'
      ] || `devtools.khulnasoft.com/kubernetes-id=${entityName}`;

    const namespace =
      entity.metadata?.annotations?.['devtools.khulnasoft.com/kubernetes-namespace'];
