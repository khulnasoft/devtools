...
describe('api', () => {
  ...
  const mockBaseUrl = 'http://devtools/api';
  ...
  beforeEach(() => {
    ...
    identityApi.getDevtoolsIdentity.mockReturnValue({});
  });
    ...
    describe('eventsource', () => {
      it('should work', async () => {
        ...
        expect(mockFetchEventSource).toHaveBeenCalledWith(
          'http://devtools/api/v2/tasks/a-random-task-id/eventstream',
          {
  ...
  describe('listTasks', () => {
    ...
    it('should list task using the current user as owner', async () => {
      ...
      identityApi.getDevtoolsIdentity.mockResolvedValueOnce({
        userEntityRef: 'user:default/foo',
      });

      const result = await apiClient.listTasks({ filterByOwnership: 'owned' });
      expect(identityApi.getDevtoolsIdentity).toHaveBeenCalled();
