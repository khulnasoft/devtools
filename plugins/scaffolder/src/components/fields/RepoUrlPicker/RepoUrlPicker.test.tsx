...
import { Form } from '@devtools/plugin-scaffolder-react/alpha';
...
describe('RepoUrlPicker', () => {
  ...
  describe('happy path rendering', () => {
    it('should render the repo url picker with minimal props', async () => {
      ...
      act(() => {
        ownerInput.focus();
        fireEvent.change(ownerInput, { target: { value: 'devtools' } });
        ownerInput.blur();
      ...
      expect(onSubmit).toHaveBeenCalledWith(
        expect.objectContaining({
          formData: 'github.com?owner=devtools&repo=repo123',
        }),
        expect.anything(),
      );
    });
    ...
    it('should render properly with allowedProject', async () => {
      ...
      expect(getByRole('option', { name: 'Devtools' })).toBeInTheDocument();
    });
