...
import { catalogEntityReadPermission } from '@devtools/plugin-catalog-common/alpha';
 ...
 *
 * @public
 * @deprecated Migrate to the {@link https://devtools.khulnasoft.com/docs/backend-system/building-backends/migrating | new backend system} and install this collator via module instead (see {@link https://github.com/khulnasoft/devtools/blob/nbs10/search-deprecate-create-router/plugins/search-backend-module-techdocs/README.md#installation | here} for more installation details).
 */
  ...
  private async *execute(): AsyncGenerator<TechDocsDocument, void, undefined> {
    ...
    while (moreEntitiesToGet) {
      ...
      const docPromises = entities
        .filter(it => it.metadata?.annotations?.['devtools.khulnasoft.com/techdocs-ref'])
        .map((entity: Entity) =>
          limit(async (): Promise<TechDocsDocument[]> => {
            ...
            try {
              ...
              // todo(@devtools/techdocs-core): remove Promise.race() when node-fetch is 3.x+
