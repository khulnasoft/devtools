...
/**
 * Parses a Devtools-issued token and decorates the
 * {@link @devtools/plugin-auth-node#DevtoolsIdentityResponse} with identity information sourced from the
 * token.
 *
 * @public
 */
export function prepareDevtoolsIdentityResponse(
  result: DevtoolsSignInResult,
): DevtoolsIdentityResponse {
  if (!result.token) {
