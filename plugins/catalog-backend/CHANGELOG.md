...
- Updated dependencies
  ...
  - @devtools/plugin-scaffolder-common@1.2.0
...
- Updated dependencies
  ...
  - @devtools/plugin-scaffolder-common@1.0.1-next.0
...
- ced3016f2a: **BREAKING**: The deprecated `CatalogEntityDocument` export has been removed, it can be imported from `@devtools/plugin-catalog-common` instead.
...
- Updated dependencies
  ...
  - @devtools/plugin-scaffolder-common@1.0.0
...
- 66ba5d9023: **BREAKING**: Removed `GithubDiscoveryProcessor`, `GithubMultiOrgReaderProcessor`, `GitHubOrgEntityProvider`, `GithubOrgReaderProcessor`, and `GithubMultiOrgConfig` which now instead should be imported from `@devtools/plugin-catalog-backend-module-github`. NOTE THAT the `GithubDiscoveryProcessor` and `GithubOrgReaderProcessor` were part of the default set of processors in the catalog backend, and if you are a user of discovery or location based org ingestion on GitLab, you MUST now add them manually in the catalog initialization code of your backend.
  ...
  **BREAKING**: Removed `GitLabDiscoveryProcessor`, which now instead should be imported from `@devtools/plugin-catalog-backend-module-gitlab`. NOTE THAT this processor was part of the default set of processors in the catalog backend, and if you are a user of discovery on GitLab, you MUST now add it manually in the catalog initialization code of your backend.
  ...
  **BREAKING**: Removed `BitbucketDiscoveryProcessor`, which now instead should be imported from `@devtools/plugin-catalog-backend-module-bitbucket`. NOTE THAT this processor was part of the default set of processors in the catalog backend, and if you are a user of discovery on Bitbucket, you MUST now add it manually in the catalog initialization code of your backend.

  ```diff
  // In packages/backend/src/plugins/catalog.ts
  +import { BitbucketDiscoveryProcessor } from '@devtools/plugin-catalog-backend-module-bitbucket';
  ...
  **BREAKING**: Removed `AzureDevOpsDiscoveryProcessor`, which now instead should be imported from `@devtools/plugin-catalog-backend-module-azure`. This processor was not part of the set of default processors. If you were using it, you should already have a reference to it in your backend code and only need to update the import.

  **BREAKING**: Removed the formerly deprecated type `BitbucketRepositoryParser`, which is instead reintroduced in `@devtools/plugin-catalog-backend-module-bitbucket`.

- f115a7f8fd: **BREAKING**: Removed `AwsS3DiscoveryProcessor`, which now instead should be imported from `@devtools/plugin-catalog-backend-module-aws`.
...
- e421d77536: **BREAKING**:
  ...
  - Removed the previously deprecated `runPeriodically` export. Please use the `@devtools/backend-tasks` package instead, or copy [the actual implementation](https://github.com/khulnasoft/devtools/blob/02875d4d56708c60f86f6b0a5b3da82e24988354/plugins/catalog-backend/src/util/runPeriodically.ts#L29) into your own code if you explicitly do not want coordination of task runs across your worker nodes.
...
- Updated dependencies
  ...
  - @devtools/plugin-scaffolder-common@0.3.0
...
- Updated dependencies
  ...
  - @devtools/plugin-scaffolder-common@0.3.0-next.0
...
- Updated dependencies
  - @devtools/backend-common@0.12.1
  - @devtools/catalog-model@0.12.1
  - @devtools/plugin-catalog-common@0.2.1

## 0.23.0

### Minor Changes

- 0c9cf2822d: **Breaking**: Mark permission-related exports as alpha. This means that the exports below should now be imported from `@devtools/plugin-catalog-backend/alpha` instead of `@devtools/plugin-catalog-backend`.
...
- 022507c860: A `DefaultCatalogCollatorFactory`, which works with the new stream-based
  ...
  [this upgrade guide](https://devtools.khulnasoft.com/docs/features/search/how-to-guides#how-to-migrate-from-search-alpha-to-beta)
...
- Updated dependencies
  ...
  - @devtools/plugin-scaffolder-common@0.2.3
...
- 25e97e7242: **BREAKING**: Removed `AwsOrganizationCloudAccountProcessor` from the default
  ...
  `@devtools/plugin-catalog-backend-module-aws`.
  ...
  First, add the `@devtools/plugin-catalog-backend-module-aws` dependency to your
  `packages/backend` package.

  Then, in `packages/backend/src/plugins/catalog.ts`:

  ```diff
  +import { AwsOrganizationCloudAccountProcessor } from '@devtools/plugin-catalog-backend-module-aws';
...
- 919cf2f836: The catalog API now returns entity relations that have three fields: The old
  ...
  **DEPRECATION**: The `target` field is hereby deprecated, both as seen from the
  catalog API as well as from the `@devtools/catalog-model` package. Both
...
- ed09ad8093: Added `LocationSpec`, which was moved over from `@devtools/catalog-model`.
...
- 7010349c9a: Added `EntityRelationSpec`, which was moved over from `@devtools/catalog-model`.
...
- Updated dependencies
  ...
  - @devtools/plugin-scaffolder-common@0.2.2
...
- e72d371296: Use `TemplateEntityV1beta2` from `@devtools/plugin-scaffolder-common` instead
  of `@devtools/catalog-model`.
...
- Updated dependencies
  - @devtools/plugin-scaffolder-common@0.2.0
...
- 9f2a8dc423: **BREAKING**: Removed all remnants of the old catalog engine implementation.
  ...
  version. If you are still on the old version, see [the relevant change log entry](https://github.com/khulnasoft/devtools/blob/master/plugins/catalog-backend/CHANGELOG.md#patch-changes-27)
...
- 2b27e49eb1: Internal update to match status field changes in `@devtools/catalog-model`.
- Updated dependencies
  ...
  - @devtools/plugin-catalog-common@0.1.1
...
- Updated dependencies
  ...
  - @devtools/plugin-catalog-common@0.1.1-next.0
...
- cd529c4094: In order to integrate the permissions system with the refresh endpoint in catalog-backend, a new AuthorizedRefreshService was created as a thin wrapper around the existing refresh service which performs authorization and handles the case when authorization is denied. In order to instantiate AuthorizedRefreshService, a permission client is required, which was added as a new field to `CatalogEnvironment`.
  ...
  The new `permissions` field in `CatalogEnvironment` should already receive the permission client from the `PluginEnvrionment`, so there should be no changes required to the catalog backend setup. See [the create-app changelog](https://github.com/khulnasoft/devtools/blob/master/packages/create-app/CHANGELOG.md) for more details.
...
- Updated dependencies
  - @devtools/backend-common@0.9.14
...
- 9fb9256e50: This continues the deprecation of classes used by the legacy catalog engine. New deprecations can be viewed in this [PR](https://github.com/khulnasoft/devtools/pull/7500) or in the API reference documentation.
...
- d6f90e934d: #### Enforcing catalog rules
  ...
  If you do use any of those `rules` sections, make sure that they are complete and list all of the kinds that are in active use in your Devtools installation.
...
- Updated dependencies
  - @devtools/catalog-client@0.3.19
...
- Updated dependencies
  - @devtools/catalog-client@0.3.18

## 0.13.0

### Minor Changes

- 8bfc0571c: Add a default catalog value for BitBucketDiscoveryProcessor. This allows to have a target like so: `https://bitbucket.mycompany.com/projects/devtools/repos/service-*`
  which will be expanded to `https://bitbucket.mycompany.com/projects/devtools/repos/service-a/catalog-info.yaml` given that repository 'service-a' exists.

  ## Migration

  If you are using a custom [Bitbucket parser](https://devtools.khulnasoft.com/docs/integrations/bitbucket/discovery#custom-repository-processing) and your `bitbucket-discovery` target (e.g. in your app-config.yaml) omits the catalog path in any of the following ways:

  - `https://bitbucket.mycompany.com/projects/devtools/repos/service-*`
  - `https://bitbucket.mycompany.com/projects/devtools/repos/*`
  - `https://bitbucket.mycompany.com/projects/devtools/repos/*/`
...
- 2d41b6993: Make use of the new `readUrl` method on `UrlReader` from `@devtools/backend-common`.
- Updated dependencies
  ...
  - @devtools/catalog-client@0.3.16
...
- 8e533f92c: Move `LdapOrgReaderProcessor` from `@devtools/plugin-catalog-backend`
  to `@devtools/plugin-catalog-backend-module-ldap`.
  ...
  1. Add dependency to `@devtools/plugin-catalog-backend-module-ldap` to the `package.json` of your backend.
  ...
  For more configuration details, see the [README of the `@devtools/plugin-catalog-backend-module-ldap` package](https://github.com/khulnasoft/devtools/blob/master/plugins/catalog-backend-module-ldap/README.md).
...
- Updated dependencies
  ...
  - @devtools/catalog-client@0.3.15
...
- 127048f92: Move `MicrosoftGraphOrgReaderProcessor` from `@devtools/plugin-catalog-backend`
  ...
  1. Add dependency to `@devtools/plugin-catalog-backend-module-msgraph` to the `package.json` of your backend.
  2. Add the processor to the catalog builder:
...
- 71416fb64: Moved installation instructions from the main [devtools.khulnasoft.com](https://devtools.khulnasoft.com) documentation to the package README file. These instructions are not generally needed, since the plugin comes installed by default with `npx @devtools/create-app`.
- Updated dependencies
  - @devtools/catalog-client@0.3.14
  - @devtools/plugin-search-backend-node@0.2.2
...
- f46a9e82d: Move dependency to `@microsoft/microsoft-graph-types` from `@devtools/plugin-catalog`
  to `@devtools/plugin-catalog-backend`.
...
- 9a207f052: Port `GithubOrgReaderProcessor` to support configuration via
  [`integrations`](https://devtools.khulnasoft.com/docs/integrations/github/locations) in
  addition to [`catalog.processors.githubOrg.providers`](https://devtools.khulnasoft.com/docs/integrations/github/org#configuration).
  ...
  A [`integrations` configuration](https://devtools.khulnasoft.com/docs/integrations/github/locations)
...
- b219821a0: Expose `BitbucketRepositoryParser` introduced in [#5295](https://github.com/khulnasoft/devtools/pull/5295)
...
- a99e0bc42: Entity lifecycle and owner are now indexed by the `DefaultCatalogCollator`. A `locationTemplate` may now optionally be provided to its constructor to reflect a custom catalog entity path in the Devtools frontend.
- Updated dependencies [e1e757569]
  - @devtools/plugin-search-backend-node@0.1.4
...
- 09b5fcf2e: GithubDiscoveryProcessor now excludes archived repositories so they won't be added to Devtools.
...
- Updated dependencies [4d248725e]
  - @devtools/plugin-search-backend-node@0.1.2
...
- 93c62c755: Move logic for generating URLs for the view, edit and source links of catalog
  ...
  existing support for the `devtools.khulnasoft.com/view-url`, `devtools.khulnasoft.com/edit-url` and
  `devtools.khulnasoft.com/source-location` annotations that are now filled by the
...
- ecdd407b1: GithubDiscoveryProcessor outputs locations as optional to avoid outputting errors for missing locations (see https://github.com/khulnasoft/devtools/issues/4730).
- 12d8f27a6: Add version `devtools.khulnasoft.com/v1beta2` schema for Template entities.
...
- 84364b35c: Added an option to scan GitHub for repositories using a new location type `github-discovery`.
  ...
  target:
     'https://github.com/khulnasoft/techdocs-*/blob/master/catalog.yaml'
  ```
...
- def2307f3: Adds a `devtools.khulnasoft.com/managed-by-origin-location` annotation to all entities. It links to the
  location that was registered to the catalog and which emitted this entity. It has a different
  semantic than the existing `devtools.khulnasoft.com/managed-by-location` annotation, which tells the direct
  parent location that created this entity.

  Consider this example: The Devtools operator adds a location of type `github-org` in the
  ...
  metadata:
    ...
    annotations:
      ...
      devtools.khulnasoft.com/managed-by-location: github-org:…
      # The entity was added because the 'bootstrap:boostrap' was added to the catalog
      devtools.khulnasoft.com/managed-by-origin-location: bootstrap:bootstrap
      # ...
  spec:
    # ...
  ```
...
- Updated dependencies [147fadcb9]
  - @devtools/catalog-model@0.6.1
  - @devtools/backend-common@0.4.3
...
- ac3560b42: Remove `implementsApis` from `Component` entities. Deprecation happened in [#3449](https://github.com/khulnasoft/devtools/pull/3449).
...
- 83b6e0c1f: Remove the deprecated fields `ancestors` and `descendants` from the `Group` entity.
  ...
  See https://github.com/khulnasoft/devtools/issues/3049 and the PRs linked from it for details.
...
- a9fd599f7: Add Analyze location endpoint to catalog backend. Add catalog-import plugin and replace import-component with it. To start using Analyze location endpoint, you have add it to the `createRouter` function options in the `\devtools\packages\backend\src\plugins\catalog.ts` file:
...
- 1185919f3: Marked the `Group` entity fields `ancestors` and `descendants` for deprecation on Dec 6th, 2020. See https://github.com/khulnasoft/devtools/issues/3049 for details.
  ...
  After Dec 6th, the fields will be removed from types and classes of the Devtools repository. At the first release after that, they will not be present in released packages either.
...
- Updated dependencies [b47dce06f]
  ...
  - @devtools/backend-common@0.3.1

## 0.2.1

### Patch Changes

- f531d307c: An entity A, that exists in the catalog, can no longer be overwritten by registering a different location that also tries to supply an entity with the same kind+namespace+name. Writes of that new entity will instead be rejected with a log message similar to `Rejecting write of entity Component:default/artist-lookup from file:/Users/freben/dev/github/devtools/packages/catalog-model/examples/components/artist-lookup-component.yaml because entity existed from github:https://github.com/khulnasoft/devtools/blob/master/packages/catalog-model/examples/components/artist-lookup-component.yaml`
...
- e6b00e3af: Remove the devtools.khulnasoft.com/definition-at-location annotation.
  ...
  metadata:
    ...
    annotations:
      ...
      devtools.khulnasoft.com/definition-at-location: 'url:https://raw.githubusercontent.com/APIs-guru/openapi-directory/master/APIs/spotify.com/v1/swagger.yaml'
  spec:
    type: openapi
...
- 002860e7a: Filters passed to the `/entities` endpoint of the catalog has changed format.
  ...
  [#2910](https://github.com/khulnasoft/devtools/issues/2910) for details.
...
- 440a17b39: The catalog backend UrlReaderProcessor now uses a UrlReader from @devtools/backend-common, which must now be supplied to the constructor.
