...
import { createConditionTransformer } from '@devtools/plugin-permission-node';
...
import { EntityFilter } from '@devtools/plugin-catalog-node';
import { mockCredentials } from '@devtools/backend-test-utils';
...
describe('AuthorizedEntitiesCatalog', () => {
  ...
  describe('entityAncestry', () => {
    it('throws error if denied access to root entity', async () => {
      ...
      await expect(() =>
        catalog.entityAncestry('devtools:default/component', {
          credentials: mockCredentials.none(),
        }),
      ).rejects.toThrow(NotAllowedError);
    });
