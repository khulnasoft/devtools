...
import { processingResult } from '@devtools/plugin-catalog-node';
...
describe('parseEntityYaml', () => {
  it('should parse a yaml', () => {
    const results = Array.from(
      ...
      metadata:
        name: devtools
        description: devtools.khulnasoft.com
        annotations:
...
---
    ...
    expect(results[1]).toEqual({
      ...
      error: expect.objectContaining({
        message: expect.stringMatching(
          /YAML error at my-loc-type:my-loc-target, YAMLParseError: Nested mappings are not allowed in compact mappings at line 9, column 19:\s+apiVersion: devtools.khulnasoft.com\/v1alpha1/,
        ),
      }),
    });
  });
