...
import { TestDatabases } from '@devtools/backend-test-utils';
import fs from 'fs';
    ...
    async databaseId => {
      ...
      await knex
        .insert({
          ...
          type: 'url',
          target:
            'https://example.com/foo-bar-test-group/very-long-group-name-that-exceeds-255-characters-just-to-test-the-limits-of-url-length-in-the-catalog-info-yaml-file-and-see-how-the-devtools-system-handles-it-making/test-this-alright-1/-/blob/main/catalog-info.yaml',
        })
        .into('locations');
