...
import { Entity, EntityPolicy } from '@devtools/catalog-model';
...
describe('transformLegacyPolicyToProcessor', () => {
  const entityToProcess: Entity = {
    apiVersion: 'devtools.khulnasoft.com/v1alpha',
    kind: 'Component',
    metadata: {
      name: 'test',
    },
  ...
  it('modifies the entity if the policy modifies the entity', async () => {
    ...
    expect(entity?.apiVersion).toBe('devtools.khulnasoft.com/v1alpha');
