...
export interface IncrementalEntityProvider<TCursor, TContext> {
   ...
   * events.
   *
   * This system acts as a wrapper for the Devtools events bus, and
   * requires the events backend to function. It does not provide its
   * own events backend. See {@link https://github.com/khulnasoft/devtools/tree/master/plugins/events-backend}.
   */
  eventHandler?: {
    /**
...
export interface IncrementalEntityProviderOptions {
   ...
   * Devtools removing all associated entities. To avoid that, set
   ...
   * improperly returned 0 assets. If set to `true`, Devtools will
