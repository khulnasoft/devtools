    ...
    async handler(ctx) {
      ...
      try {
        ...
        if (e.response?.statusCode !== 404) {
          throw new InputError(
            `Failed to check status of branch '${branchName}'. Please make sure that branch already exists or Devtools has permissions to create one. ${getErrorMessage(
              e,
            )}`,
          );
        }
      ...
      if (!branchExists) {
        ...
        try {
          ...
          throw new InputError(
            `The branch '${branchName}' was not found and creation failed with error. Please make sure that branch already exists or Devtools has permissions to create one. ${getErrorMessage(
              e,
            )}`,
          );
        }
