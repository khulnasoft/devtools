...
import { ConfigApi, DiscoveryApi, FetchApi } from '@devtools/core-plugin-api';
import {
  GithubIntegration,
...
export class CatalogImportClient implements CatalogImportApi {
  ...
  async preparePullRequest(): Promise<{
    ...
    const appTitle =
      this.configApi.getOptionalString('app.title') ?? 'Devtools';
    const appBaseUrl = this.configApi.getString('app.baseUrl');
    const catalogFilename = getCatalogFilename(this.configApi);

    return {
      title: `Add ${catalogFilename} config file`,
      body: `This pull request adds a **Devtools entity metadata file** \
...
[overview of the Devtools software catalog](https://devtools.khulnasoft.com/docs/features/software-catalog/).`,
    };
  }
