...
import { ConfigReader, UrlPatternDiscovery } from '@devtools/core-app-api';
import { ScmIntegrations } from '@devtools/integration';
import { ScmAuthApi } from '@devtools/integration-react';
import { catalogApiMock } from '@devtools/plugin-catalog-react/testUtils';
import { MockFetchApi, registerMswTestHooks } from '@devtools/test-utils';
...
describe('CatalogImportClient', () => {
  ...
  const mockBaseUrl = 'http://devtools:9191/api/catalog';
  ...
  beforeEach(() => {
    catalogImportClient = new CatalogImportClient({
      ...
      configApi: new ConfigReader({
        app: {
          baseUrl: 'https://demo.devtools.khulnasoft.com/',
        },
      }),
    });
  });
  ...
  describe('analyzeUrl', () => {
    ...
    it('should reject for integrations that are not github or azure', async () => {
      await expect(
        catalogImportClient.analyzeUrl(
          'https://registered-but-not-github.com/khulnasoft/devtools',
        ),
      ).rejects.toThrow(
        new Error(
    ...
    it('should find locations from github', async () => {
              ...
              existingEntityFiles: [
                {
                  ...
                  location: {
                    type: 'url',
                    target:
                      'https://github.com/khulnasoft/devtools/blob/main/simple/path/catalog-info.yaml',
                  },
                  entity: {
                    apiVersion: '1',
                    kind: 'k',
                ...
                },
                {
                  ...
                  location: {
                    type: 'url',
                    target:
                      'https://github.com/khulnasoft/devtools/blob/main/co/mple/x/path/catalog-info.yaml',
                  },
                  entity: {
                    apiVersion: '1',
                    kind: 'k',
                ...
                },
                {
                  ...
                  location: {
                    type: 'url',
                    target:
                      'https://github.com/khulnasoft/devtools/blob/main/catalog-info.yaml',
                  },
                  entity: {
                    apiVersion: '1',
                    kind: 'k',
      ...
      await expect(
        catalogImportClient.analyzeUrl(
          'https://github.com/khulnasoft/devtools',
        ),
      ).resolves.toEqual({
        locations: [
          {
  ...
  describe('submitPullRequest', () => {
    it('should create GitHub pull request', async () => {
      ...
      await expect(
        catalogImportClient.submitPullRequest({
          repositoryUrl: 'https://github.com/khulnasoft/devtools',
          fileContent: `
            {
                ...
                "metadata": {
                  "name": "valid-name",
                  "annotations": {
                      "github.com/project-slug": "devtools/example-repo"
                }
              },
              "spec": {
                  "type": "other",
      ...
      expect(
        ...
        owner: 'devtools',
        repo: 'devtools',
        ref: 'refs/heads/devtools-integration',
        sha: 'any',
      });
      expect(
    ...
    it('should create AzureDevops pull request', async () => {
      ...
      azureMock.mockResolvedValueOnce({
        repository: {
          name: 'devtools',
          webUrl: 'https://dev.azure.com/spotify/devtools/_git/devtools',
        },
        pullRequestId: '01',
      ...
      const expectedPrOptions: AzurePrOptions = {
        ...
        branchName: 'devtools-integration',
        token: 'token',
        fileContent: `
            {
                "apiVersion": "devtools.khulnasoft.com/v1alpha1",
      ...
      await expect(
        catalogImportClient.submitPullRequest({
          repositoryUrl:
            'https://dev.azure.com/spotify/devtools/_git/devtools',
          fileContent: expectedPrOptions.fileContent,
          title: expectedPrOptions.title,
          body: expectedPrOptions.description,
        }),
      ).resolves.toEqual({
        link: 'https://dev.azure.com/spotify/devtools/_git/devtools/pullrequest/01',
        location:
          'https://dev.azure.com/spotify/devtools/_git/devtools?path=/catalog-info.yaml',
      });
    ...
    it('Submit Pull Request with invalid component name', async () => {
      const ErrorMessage =
        'Policy check failed for component:default/invalid name; caused by Error: "metadata.name" is not valid; expected a string that is sequences of [a-zA-Z0-9] separated by any of [-_.], at most 63 characters in total but found "invalid name". To learn more about catalog file format, visit: https://github.com/khulnasoft/devtools/blob/master/docs/architecture-decisions/adr002-default-catalog-file-format.md';
    ...
    it('should create GitHub pull request and validate all documents inside the YAML file', async () => {
      ...
      await expect(
        catalogImportClient.submitPullRequest({
          repositoryUrl: 'https://github.com/khulnasoft/devtools',
          fileContent: `apiVersion: devtools.khulnasoft.com/v1alpha1
kind: Component
metadata:
