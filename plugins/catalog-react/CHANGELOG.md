...
- c689d7a94c: Added `CatalogFilterLayout`, which replaces `FilteredEntityLayout` from `@devtools/plugin-catalog`, as well as `FilterContainer` and `EntityListContainer`. It is used like this:
...
- 7ffb2c73c9: **BREAKING**: Removed the deprecated `loadCatalogOwnerRefs` function. Usages of this function can be directly replaced with `ownershipEntityRefs` from `identityApi.getDevtoolsIdentity()`.
...
- fc6290a76d: **BREAKING**: Removed the deprecated `useOwnUser` hook. Existing usage can be replaced with `identityApi.getDevtoolsIdentity()`, followed by a call to `catalogClient.getEntityByRef(identity.userEntityRef)`.
...
- 72431d7bed: - **BREAKING**: The `isOwnerOf` function has been marked as `@alpha` and is now only available via the `@devtools/plugin-catalog-react/alpha` import. The limitations of this function with regards to only supporting direct relations have also been documented.
- 03ec06bf7f: **BREAKING**: Moved **DefaultStarredEntitiesApi** to `@devtools/plugin-catalog`. If you were using this in tests, you can use the new `MockStarredEntitiesApi` from `@devtools/plugin-catalog-react` instead.
...
- c4276915c0: **BREAKING**: Deleted the deprecated `loadIdentityOwnerRefs` function which is replaced by `ownershipEntityRefs` from `identityApi.getDevtoolsIdentity()`.

  Deprecated the `loadCatalogOwnerRefs` hook as membership references should be added as `ent` inside `claims` sections of the `SignInResolver` when issuing tokens. See https://devtools.khulnasoft.com/docs/auth/identity-resolver for more details on how to prepare your `SignInResolver` if not done already. Usage of the `loadCatalogOwnerRefs` hook should be replaced by `ownershipEntityRefs` from `identityApi.getDevtoolsIdentity()` instead.
...
- 3d87019269: The `entityRouteRef` is now a well-known route that should be imported directly from `@devtools/plugin-catalog-react`. It is guaranteed to be globally unique across duplicate installations of the `@devtools/plugin-catalog-react`, starting at this version.
...
- 2916a83b9c: Deprecated `loadIdentityOwnerRefs`, since they can now be retrieved as `ownershipEntityRefs` from `identityApi.getDevtoolsIdentity()` instead.
...
- fe2a6532ff: Add Override Components for Components in @devtools/plugin-catalog-react
...
- 4c0f0b2003: Removed dependency on `@devtools/core-app-api`.
...
- ea81a1f19c: Deprecated EntityContext in favor of using `useEntity`, `EntityProvider` and the new `AsyncEntityProvider` instead. This update also brings cross-version compatibility to `@devtools/catalog-react`, meaning that future versions can be used in parallel with this one.
- Updated dependencies
...
- Updated dependencies [1cf1d351f]
  - @devtools/core-plugin-api@0.1.1
...
- 10a0124e0: Expose `useRelatedEntities` from `@devtools/plugin-catalog-react` to retrieve
