...
import { ApiRef, identityApiRef } from '@devtools/core-plugin-api';
...
jest.spyOn(mockIdentityApi, 'getDevtoolsIdentity');

jest.mock('@devtools/core-plugin-api', () => {
  const actual = jest.requireActual('@devtools/core-plugin-api');
  return {
    ...actual,
...
describe('useOwnedEntitiesCount', () => {
  ...
  it(`shouldn't invoke queryEntities when filters are loading`, async () => {
    ...
    await waitFor(() =>
      expect(mockIdentityApi.getDevtoolsIdentity).toHaveBeenCalled(),
    );
