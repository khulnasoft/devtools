...
import { isChildPath, LoggerService } from '@devtools/backend-plugin-api';
import { Entity } from '@devtools/catalog-model';
import { assertError, ForwardedError } from '@devtools/errors';
...
/**
 * Return the source url for MkDocs based on the devtools.khulnasoft.com/techdocs-ref annotation.
...
export const getRepoUrlFromLocationAnnotation = (
  ...
  if (locationType === 'url') {
    ...
    if (
      ...
      // handle the case where a user manually writes url:https://github.com/khulnasoft/devtools i.e. without /blob/...
