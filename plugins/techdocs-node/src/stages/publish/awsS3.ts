 ...
 */
import { Entity, CompoundEntityRef } from '@devtools/catalog-model';
...
export class AwsS3Publish implements PublisherBase {
  ...
  static async fromConfig(
    ...
    const storageClient = new S3Client({
      customUserAgent: 'devtools-aws-techdocs-s3-publisher',
  ...
  private static async buildCredentials(
    ...
    if (roleArn) {
      return fromTemporaryCredentials({
        ...
        params: {
          RoleSessionName: 'devtools-aws-techdocs-s3-publisher',
          RoleArn: roleArn,
        },
        clientConfig: { region },
      });
    ...
    try {
      ...
      this.logger.error(
        `Could not retrieve metadata about the AWS S3 bucket ${this.bucketName}. ` +
          ...
          'by using environment variables. Refer to https://devtools.khulnasoft.com/docs/features/techdocs/using-cloud-storage',
      );
