...
} from '@devtools/integration-aws-node';
...
describe('AwsS3Publish', () => {
  ...
  describe('publish', () => {
    it('should publish a directory', async () => {
      ...
      expect(await publisher.publish({ entity, directory })).toMatchObject({
        objects: expect.arrayContaining([
          'default/component/devtools/404.html',
          `default/component/devtools/index.html`,
          `default/component/devtools/assets/main.css`,
        ]),
      });
    });
    ...
    it('should publish a directory when root path is specified', async () => {
      const publisher = await createPublisherFromConfig({
        bucketRootPath: 'devtools-data/techdocs',
      });
      expect(await publisher.publish({ entity, directory })).toMatchObject({
        objects: expect.arrayContaining([
          'devtools-data/techdocs/default/component/devtools/404.html',
          `devtools-data/techdocs/default/component/devtools/index.html`,
          `devtools-data/techdocs/default/component/devtools/assets/main.css`,
        ]),
      });
    });
