 ...
 */
import { Entity, DEFAULT_NAMESPACE } from '@devtools/catalog-model';
import {
  getStaleFiles,
...
describe('lowerCaseEntityTriplet', () => {
  it('returns lower-cased entity triplet path', () => {
    const originalPath = 'default/Component/devtools/index.html';
    const actualPath = lowerCaseEntityTriplet(originalPath);
    expect(actualPath).toBe('default/component/devtools/index.html');
  });
});

describe('lowerCaseEntityTripletInStoragePath', () => {
  it('does not lowercase beyond the triplet', () => {
    const originalPath = 'default/Component/devtools/assets/IMAGE.png';
    const actualPath = lowerCaseEntityTripletInStoragePath(originalPath);
    expect(actualPath).toBe('default/component/devtools/assets/IMAGE.png');
  });
...
describe('normalizeExternalStorageRootPath', () => {
  ...
  it('returns normalized path from path with leading and trailing sep', () => {
    const originalPath = '/devtools-data/techdocs/';
    const normalPath = normalizeExternalStorageRootPath(originalPath);
    expect(normalPath).toBe('devtools-data/techdocs');
  });
...
describe('getStaleFiles', () => {
  const defaultFiles = [
    'default/Component/devtools/index.html',
    'default/Component/devtools/techdocs_metadata.json',
    'default/Component/devtools/assests/javascripts/bundle.7f4f3c92.min.js',
    'default/Component/devtools/assets/stylesheets/main.fe0cca5b.min.css',
  ];
  ...
  it('should not return directories as stale files if they are parent directories of new files', () => {
    const oldFiles = [...defaultFiles, 'default/Component/devtools/foo'];
    const newFiles = [
      ...defaultFiles,
      'default/Component/devtools/foo/bar/index.html',
    ];
    const staleFiles = getStaleFiles(newFiles, oldFiles);
    expect(staleFiles).toHaveLength(0);
  });
...
describe('getCloudPathForLocalPath', () => {
  ...
  it('should compose a remote filename including entity information', () => {
    ...
    expect(remoteBucket).toBe(`custom/component/devtools/${localPath}`);
  });
  ...
  it('should use the default namespace when it is undefined', () => {
    ...
    expect(remoteBucket).toBe(
      `${DEFAULT_NAMESPACE}/component/devtools/${localPath}`,
    );
  });
  ...
  it('should prepend root directory to destination', () => {
    const localPath = 'index/html';
    const rootPath = 'devtools-data/techdocs/';
    const remoteBucket = getCloudPathForLocalPath(
      entity,
      localPath,
      false,
    ...
    expect(remoteBucket).toBe(
      `devtools-data/techdocs/custom/component/devtools/${localPath}`,
    );
  });
