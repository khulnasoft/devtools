...
describe('AzureDevOpsDiscoveryProcessor', () => {
  describe('parseUrl', () => {
    it('parses well formed URLs', () => {
      ...
      expect(
        parseUrl(
          'https://dev.azure.com/spotify/engineering/_git/devtools?path=/catalog.yaml',
        ),
      ).toEqual({
        baseUrl: 'https://dev.azure.com',
        org: 'spotify',
      ...
      expect(
        parseUrl(
          'https://azuredevops.mycompany.com/spotify/engineering/_git/devtools?path=/src/*/catalog.yaml',
        ),
      ).toEqual({
      ...
      expect(
        parseUrl(
          'https://azuredevops.mycompany.com/spotify/engineering/_git/devtools?path=/src/*/catalog.yaml&version=GBtopic/catalog-info',
        ),
      ).toEqual({
        baseUrl: 'https://azuredevops.mycompany.com',
        org: 'spotify',
      ...
      expect(
        parseUrl(
          'https://azuredevops.mycompany.com/spotify/engineering/_git/devtools?path=/src/*/catalog.yaml&version=GBtopic%2Fcatalog-info',
        ),
      ).toEqual({
        baseUrl: 'https://azuredevops.mycompany.com',
        org: 'spotify',
  ...
  describe('handles repositories', () => {
    ...
    it('output all locations found on from code search', async () => {
      ...
      expect(emitter).toHaveBeenCalledWith({
        ...
        location: {
          type: 'url',
          target:
            'https://dev.azure.com/shopify/engineering/_git/devtools?path=/catalog-info.yaml',
          presence: 'optional',
        },
      });
      expect(emitter).toHaveBeenCalledWith({
    ...
    it('output single locations from code search', async () => {
      const location: LocationSpec = {
        type: 'azure-discovery',
        target: 'https://dev.azure.com/shopify/engineering/_git/devtools',
      };
      mockCodeSearch.mockResolvedValueOnce([
        {
          fileName: 'catalog-info.yaml',
    ...
    it('output locations with branch if specified in target', async () => {
      const location: LocationSpec = {
        type: 'azure-discovery',
        target:
          'https://dev.azure.com/shopify/engineering/_git/devtools?path=/catalog-info.yaml&version=GBtopic/catalog-info',
      };
      mockCodeSearch.mockResolvedValueOnce([
        {
          fileName: 'catalog-info.yaml',
    ...
    it('output single locations with different file name from code search', async () => {
      ...
      expect(emitter).toHaveBeenCalledWith({
        ...
        location: {
          type: 'url',
          target:
            'https://dev.azure.com/shopify/engineering/_git/devtools?path=/src/main/catalog.yaml',
          presence: 'optional',
        },
      });
    });
