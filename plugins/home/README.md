...
```tsx
import { createCardExtension } from '@devtools/plugin-home-react';
...
import { RandomJokeHomePageComponent } from '@devtools/plugin-home';
...
import {
  ...
  CustomHomepageGrid,
} from '@devtools/plugin-home';
import { Content, Header, Page } from '@devtools/core-components';
import { HomePageSearchBar } from '@devtools/plugin-search';
import { HomePageCalendar } from '@devtools/plugin-gcalendar';
import { MicrosoftCalendarCard } from '@devtools/plugin-microsoft-calendar';
...
```ts
// packages/app/src/App.tsx
import { VisitListener } from '@devtools/plugin-home';
// ...
...
In order to validate the config you can use `devtools/cli config:check`
...
We are hoping that we together can build up a collection of Homepage templates. We therefore put together a place where we can collect all the templates for the Home Plugin in the [storybook](https://devtools.khulnasoft.com/storybook/?path=/story/plugins-home-templates).
