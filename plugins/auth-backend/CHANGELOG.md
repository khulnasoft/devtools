...
- 3884bf0348: **BREAKING**: The default sign-in resolvers for all providers, if you choose to
  ...
  result is that you may for example see your Devtools token `sub` claim now
...
- 6bc86fcf2d: The following breaking changes were made, which may imply specifically needing
  ...
  - **BREAKING**: Moved `IdentityClient`, `DevtoolsSignInResult`,
    `DevtoolsIdentityResponse`, and `DevtoolsUserIdentity` to
    `@devtools/plugin-auth-node`.
  - **BREAKING**: Removed deprecated type `DevtoolsIdentity`, please use
    `DevtoolsSignInResult` from `@devtools/plugin-auth-node` instead.
  ...
  ````diff
  -import { IdentityClient } from '@devtools/plugin-auth-backend';
  +import {
  +  IdentityClient,
...
- 648606b3ac: Added support for storing static GitHub access tokens in cookies and using them to refresh the Devtools session.
- Updated dependencies
...
- 67349916ac: The `sub` claim in Devtools tokens generated by the default Google and OIDC sign-in resolvers are now full entity references of the format `<kind>:<namespace>/<name>`.
...
- f8496730ab: Switched the handling of the `DevtoolsIdentityResponse` so that the returned `identity.userEntityRef` is always a full entity reference. If `userEntityRef` was previously set to `jane`, it will now be `user:default/jane`. The `userEntityRef` in the response is parsed from the `sub` claim in the payload of the Devtools token.
...
- da9c59d6e0: Removed `@devtools/test-utils` dependency.
...
- Updated dependencies
  - @devtools/backend-common@0.10.2
...
- a036b65c2f: **BREAKING CHANGE** The `idToken` field of `DevtoolsIdentity` has been removed, with the `token` taking its place. This means you may need to update existing `signIn.resolver` implementations to return an `token` rather than an `idToken`. This also applies to custom auth providers.

  The `DevtoolsIdentity` type has been deprecated and will be removed in the future. Taking its place is the new `DevtoolsSignInResult` type with the same shape.

  This change also introduces the new `DevtoolsIdentityResponse` that mirrors the type with the same name from `@devtools/core-plugin-api`. The `DevtoolsIdentityResponse` type is different from the `DevtoolsSignInResult` in that it also has a `identity` field which is of type `DevtoolsUserIdentity` and is a decoded version of the information within the token.

  When implementing a custom auth provider that is not based on the `OAuthAdapter` you may need to convert `DevtoolsSignInResult` into a `DevtoolsIdentityResponse`, this can be done using the new `prepareDevtoolsIdentityResponse` function.
...
- 0cfeea8f8f: AWS-ALB: update provider to the latest changes described [here](https://devtools.khulnasoft.com/docs/auth/identity-resolver).
...
- 2567c066d: TokenIssuer is now exported so it may be used by auth providers that are not bundled with Devtools
- Updated dependencies
...
- c467cc4b9: Adds support for custom sign-in resolvers and profile transformations for the
  ...
  Adds an `ent` claim in Devtools tokens, with a list of
  [entity references](https://devtools.khulnasoft.com/docs/features/software-catalog/references)
  ...
  Updates `DevtoolsIdentity` so that

  - `idToken` is deprecated in favor of `token`
  - An optional `entity` field is added which represents the entity that the user is represented by within Devtools.

  More information:

  - [The identity resolver documentation](https://devtools.khulnasoft.com/docs/auth/identity-resolver)
    explains the concepts and shows how to implement your own.
  - The [From Identity to Ownership](https://github.com/khulnasoft/devtools/issues/4089)
...
- 0289a059c: Add support for the majority of the Core configurations for Passport-SAML.
  ...
  As part of this change, there is also a fix to the redirection behaviour when doing load balancing and HTTPS termination - the application's baseUrl is used to generate the callback URL. For properly configured Devtools installations, no changes are necessary, and the baseUrl is respected.
...
- 819a70229: Add SAML login to devtools
...
- 6f1768c0f: Initial implementation of catalog user lookup
  ...
  - Added "@devtools/catalog-model@^0.1.1-alpha.23" as a dependency
