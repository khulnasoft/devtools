...
import { AuthOwnershipResolver } from '@devtools/plugin-auth-node';
import {
  TokenManager,
...
export async function createRouter(
  ...
  const devtoolsTokenExpiration = readDevtoolsTokenExpiration(config);
  ...
  if (keyStore instanceof StaticKeyStore) {
    tokenIssuer = new StaticTokenIssuer(
      {
        ...
        issuer: authUrl,
        sessionExpirationSeconds: devtoolsTokenExpiration,
      },
    ...
    tokenIssuer = new TokenFactory({
      issuer: authUrl,
      keyStore,
      keyDurationSeconds: devtoolsTokenExpiration,
      logger: logger.child({ component: 'token-factory' }),
      algorithm:
        tokenFactoryAlgorithm ??
