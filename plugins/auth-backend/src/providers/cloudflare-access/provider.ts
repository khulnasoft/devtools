...
} from '@devtools/plugin-auth-backend-module-cloudflare-access-provider';
import {
  SignInResolver,
 ...
 *
 * @public
 * @deprecated import from `@devtools/plugin-auth-backend-module-cloudflare-access-provider` instead
 */
...
export const cfAccess = createAuthProviderIntegration({
  create(options: {
    ...
    signIn: {
      /**
       * Maps an auth result to a Devtools identity for the user.
       */
      resolver: SignInResolver<CloudflareAccessResult>;
    };

    /**
     * Cache service object that was configured for the Devtools backend,
