...
import {
  ...
  DevtoolsIdentityResponse,
  DevtoolsSignInResult,
  CookieConfigurer,
  OAuthState,
...
import { prepareDevtoolsIdentityResponse } from '../../providers/prepareDevtoolsIdentityResponse';
...
/**
 * @public
 * @deprecated Use `createOAuthRouteHandlers` from `@devtools/plugin-auth-node` instead
 */
...
export class OAuthAdapter implements AuthProviderRouteHandlers {
  ...
  async frameHandler(
    ...
    try {
      ...
      const identity = await this.populateIdentity(response.devtoolsIdentity);

      const responseObj: WebMessageResponse = {
        type: 'authorization_response',
        response: { ...response, devtoolsIdentity: identity },
      };
  ...
  async refresh(req: express.Request, res: express.Response): Promise<void> {
    ...
    try {
      ...
      const devtoolsIdentity = await this.populateIdentity(
        response.devtoolsIdentity,
      );
      ...
      res.status(200).json({ ...response, devtoolsIdentity });
    } catch (error) {
  ...
  /**
   * If the response from the OAuth provider includes a Devtools identity, we
   * make sure it's populated with all the information we can derive from the user ID.
   */
  private async populateIdentity(
    identity?: DevtoolsSignInResult,
    ...
    return prepareDevtoolsIdentityResponse(identity);
  }
