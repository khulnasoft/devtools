...
import { TokenParams, tokenTypes } from '@devtools/plugin-auth-node';
...
/**
 * The payload contents of a valid Devtools JWT token
 */
export interface DevtoolsTokenPayload {
  /**
...
/**
 * The payload contents of a valid Devtools user identity claim token
 *
 * @internal
 */
interface DevtoolsUserIdentityProofPayload {
  /**
   * The entity ref of the user
   */
  sub: string;
...
export class TokenFactory implements TokenIssuer {
  ...
  async issueToken(params: TokenParams): Promise<string> {
    ...
    const claims: DevtoolsTokenPayload = {
      ...additionalClaims,
      iss,
      sub,
      ent,
  ...
  private async createUserIdentityClaim(options: {
    ...
    payload: DevtoolsUserIdentityProofPayload;
    ...
    // the signature part of these to reduce duplication within the Devtools
