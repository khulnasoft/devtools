...
1. Set Application Name to `devtools-dev` or something along those lines.
1. You can set the Homepage URL to whatever you want to.
1. The Authorization Callback URL should match the redirect URI set in Devtools.
...
- Give the app a name. e.g. `devtools-dev`
- Select `Accounts in this organizational directory only` under supported account types.
- Enter the callback URL for your devtools backend instance:
...
We also need to generate a client secret so Devtools can authenticate as this app.
...
[How to add an auth provider](https://github.com/khulnasoft/devtools/blob/master/docs/auth/add-auth-provider.md)

## Token issuers

[Configuring token issuers](https://github.com/khulnasoft/devtools/blob/master/docs/auth/index.md)
...
If you need to change Devtools token expiration from the default value of one hour you can do so through configuration. Note that this is **not** the session duration, but rather the duration that the short-term cryptographic tokens are valid for. The expiration can not be set lower than 10 minutes or above 24 hours.

This is what the configuration looks like:

```
auth:
   devtoolsTokenExpiration: { minutes: <user_defined_value> }
```
