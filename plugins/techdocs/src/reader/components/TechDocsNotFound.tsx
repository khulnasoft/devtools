...
import { useApi, configApiRef, useAnalytics } from '@devtools/core-plugin-api';
import { ErrorPage } from '@devtools/core-components';
import { useTechDocsReaderPage } from '@devtools/plugin-techdocs-react';
...
export const TechDocsNotFound = ({ errorMessage }: Props) => {
  ...
  if (![undefined, 'local'].includes(techdocsBuilder)) {
    additionalInfo =
      "Note that techdocs.builder is not set to 'local' in your config, which means this Devtools app will not " +
      "generate docs if they are not found. Make sure the project's docs are generated and published by some external " +
      "process (e.g. CI/CD pipeline). Or change techdocs.builder to 'local' to generate docs from this Devtools " +
      'instance.';
  }
