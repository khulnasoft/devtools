...
import type { ActionContext } from '@devtools/plugin-scaffolder-node';
...
describe('confluence:transform:markdown', () => {
  ...
  beforeEach(() => {
    ...
    mockContext = createMockActionContext({
      input: {
        ...
        repoUrl:
          'https://notreal.github.com/space/devtools/blob/main/mkdocs.yml',
      },
      workspacePath,
      logger,
    });
  ...
  it('should call confluence to markdown action successfully with results array', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith(
      `Fetching the mkdocs.yml catalog from https://notreal.github.com/space/devtools/blob/main/mkdocs.yml`,
    );
