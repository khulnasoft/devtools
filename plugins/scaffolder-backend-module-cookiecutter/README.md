...
```bash
# From your Devtools root directory
yarn --cwd packages/backend add @devtools/plugin-scaffolder-backend-module-cookiecutter
```
...
backend.add(import('@devtools/plugin-scaffolder-backend-module-cookiecutter'));
```
...
If you are running Devtools from a Docker container and you want to avoid calling a container inside a container, you can set up `cookiecutter` in your own image, this will use the local installation instead.
