...
import { StructuredMetadataTable } from '@devtools/core-components';
...
function processSchema(
  ...
  const name =
    definitionInSchema?.['ui:devtools']?.review?.name ??
    definitionInSchema?.title ??
    key;

  if (definitionInSchema) {
    const devtoolsReviewOptions = definitionInSchema['ui:devtools']?.review;
    if (devtoolsReviewOptions) {
      if (devtoolsReviewOptions.mask) {
        return [[name, devtoolsReviewOptions.mask]];
      }
      if (devtoolsReviewOptions.show === false) {
        return [];
      }
    }
    ...
    if (devtoolsReviewOptions?.explode !== false && isJsonObject(value)) {
