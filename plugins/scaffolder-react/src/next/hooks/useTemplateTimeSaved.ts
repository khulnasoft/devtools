...
/**
 * Returns the devtools.khulnasoft.com/time-saved annotation (as a number of minutes) for
...
export const useTemplateTimeSavedMinutes = (templateRef: string) => {
  ...
  const { value: timeSavedMinutes } = useAsync(async () => {
    ...
    const timeSaved = entity?.metadata.annotations?.['devtools.khulnasoft.com/time-saved'];
