 ...
 */
import { featureFlagsApiRef, useApi } from '@devtools/core-plugin-api';
import { TemplatePresentationV1beta3 } from '@devtools/plugin-scaffolder-common';
import { JsonObject } from '@devtools/types';
import { UiSchema } from '@rjsf/utils';
import { TemplateParameterSchema } from '@devtools/plugin-scaffolder-react';
...
export const useTemplateSchema = (
  ...
  const returningSteps = steps
    ...
    .filter(step => {
      const stepFeatureFlag = step.uiSchema['ui:devtools']?.featureFlag;
          ...
          Object.entries((step.schema?.properties ?? {}) as JsonObject).filter(
            ([key]) => {
              const stepFeatureFlag =
                step.uiSchema[key]?.['ui:devtools']?.featureFlag;
