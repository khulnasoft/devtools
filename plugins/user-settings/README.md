...
This plugin provides two components, `<UserSettings />` is intended to be used within the [`<Sidebar>`](https://devtools.khulnasoft.com/storybook/?path=/story/sidebar--sample-sidebar) and displays the signed-in users profile picture and name. The second component is a settings page where the user can control different settings across the App.
...
Please see [the backend README](https://github.com/khulnasoft/devtools/tree/master/plugins/user-settings-backend)
...
```tsx
import { Settings as SidebarSettings } from '@devtools/plugin-user-settings';
...
[the example extra tab](https://github.com/khulnasoft/devtools/blob/master/packages/app/src/components/advancedSettings/AdvancedSettings.tsx)
we have created in Devtools's example app.
