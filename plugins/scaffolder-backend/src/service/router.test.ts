...
import { TaskBroker } from '@devtools/plugin-scaffolder-node';
...
import { AutocompleteHandler } from '@devtools/plugin-scaffolder-node/alpha';
...
describe('createRouter', () => {
  ...
  const getMockTemplate = (): TemplateEntityV1beta3 => ({
    ...
    metadata: {
      ...
      annotations: {
        'devtools.khulnasoft.com/managed-by-location': 'url:https://dev.azure.com',
      },
    },
    spec: {
  ...
  describe('not providing an identity api', () => {
    ...
    describe('POST /v2/tasks', () => {
      it('rejects template values which do not match the template schema definition', async () => {
        const response = await request(app)
          ...
          .send({
            ...
            values: {
              storePath: 'https://github.com/khulnasoft/devtools',
            },
          });
      ...
      it('should emit auditlog containing user identifier when devtools auth is passed', async () => {
        const mockToken = mockCredentials.user.token();
