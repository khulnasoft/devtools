...
import { JsonArray, JsonObject, JsonValue } from '@devtools/types';
import { InputError, NotAllowedError, stringifyError } from '@devtools/errors';
...
import {
  ...
  TaskSpecV1beta3,
  TaskStep,
} from '@devtools/plugin-scaffolder-common';
...
import { createConditionAuthorizer } from '@devtools/plugin-permission-node';
...
import { DevtoolsLoggerTransport, WinstonLogger } from './logger';
...
const isValidTaskSpec = (taskSpec: TaskSpec): taskSpec is TaskSpecV1beta3 => {
  return taskSpec.apiVersion === 'scaffolder.devtools.khulnasoft.com/v1beta3';
};
...
const createStepLogger = ({
  ...
  const taskLogger = WinstonLogger.create({
    ...
    transports: [new DevtoolsLoggerTransport(rootLogger, task, step.id)],
  });
