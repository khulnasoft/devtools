...
import { TaskSpec } from '@devtools/plugin-scaffolder-common';
import {
...
import { RESOURCE_TYPE_SCAFFOLDER_ACTION } from '@devtools/plugin-scaffolder-common/alpha';
import {
  createMockDirectory,
  mockCredentials,
  mockServices,
...
describe('NunjucksWorkflowRunner', () => {
  ...
  describe('validation', () => {
    ...
    it('should pass token through', async () => {
      ...
      expect(fakeActionHandler.mock.calls[0][0].secrets).toEqual(
        expect.objectContaining({ devtoolsToken: token }),
      );
    });
  });
  ...
  describe('conditionals', () => {
    it('should execute steps conditionally', async () => {
      ...
      expect(output.result).toBe('devtools');
    });
      ...
      const task = createMockTaskWithSpec({
        ...
        steps: [
          { id: 'test', name: 'test', action: 'output-action' },
          {
            ...
            name: 'conditional',
            action: 'output-action',
            if: '${{ steps.test.output.mock !== "devtools"}}',
          },
        ],
        output: {
          result: '${{ steps.conditional.output.mock }}',
  ...
  describe('templating', () => {
    it('should template the input to an action', async () => {
      ...
      expect(fakeActionHandler).toHaveBeenCalledWith(
        expect.objectContaining({ input: { foo: 'devtools' } }),
      );
    });
    ...
    it('should template complex values into the action', async () => {
      ...
      expect(fakeActionHandler).toHaveBeenCalledWith(
        expect.objectContaining({ input: { foo: { bar: 'DEVTOOLS' } } }),
      );
    });
    ...
    it('should template the output from simple actions', async () => {
      ...
      expect(output.foo).toEqual('DEVTOOLS');
    });
