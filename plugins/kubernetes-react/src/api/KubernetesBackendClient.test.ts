...
import { UrlPatternDiscovery } from '@devtools/core-app-api';
      ...
      worker.use(
        rest.get(
          ...
          (req, res, ctx) =>
            res(
              req.headers.get(
                'Devtools-Kubernetes-Authorization-oidc-okta',
              ) === 'k8-token3'
                ? ctx.json(nsResponse)
                : ctx.status(403),
            ),
    ...
    it('hits /proxy api when signed in as a guest', async () => {
      ...
      worker.use(
        rest.get(
          ...
          (req, res, ctx) =>
            res(
              req.headers.get('Devtools-Kubernetes-Authorization-aws') ===
                'k8-token'
                ? ctx.json(nsResponse)
                : ctx.status(403),
            ),
    ...
    it('responds with an 403 error when invalid k8 token is provided', async () => {
      ...
      worker.use(
        rest.get(
          ...
          (req, res, ctx) =>
            res(
              req.headers.get('Devtools-Kubernetes-Authorization') ===
                'Bearer k8-token'
                ? ctx.json(nsResponse)
                : ctx.status(403),
            ),
