...
import { wrapInTestApp, TestApiProvider } from '@devtools/test-utils';
...
import { TechDocsReaderPage, techdocsPlugin } from '@devtools/plugin-techdocs';
import { entityRouteRef } from '@devtools/plugin-catalog-react';
import { searchApiRef } from '@devtools/plugin-search-react';
...
export class TechDocsAddonTester {
  ...
  build() {
    ...
    const discoveryApi = {
      getBaseUrl: jest
        .fn()
        .mockResolvedValue('https://devtools.example.com/api/techdocs'),
    };
    ...
    techdocsStorageApi.getApiOrigin.mockResolvedValue(
      'https://devtools.example.com/api/techdocs',
    );
    techdocsStorageApi.getBaseUrl.mockResolvedValue(
      `https://devtools.example.com/api/techdocs/${entityName.namespace}/${entityName.kind}/${entityName.name}/${this.options.path}`,
    );
