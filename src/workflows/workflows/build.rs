...
use walkdir::WalkDir;
use khulnasoft_workflows_types::Workflow;
...
fn main() -> Result<()> {
    ...
    Command::new("cargo")
        .args(["fmt", "-p", "khulnasoft-workflows"])
        .output()?;
...
/// ```ignore
/// use khulnasoft_workflows_metadata::*;
...
fn write_workflows_function(
    ...
    writeln!(parent_module, "\nuse khulnasoft_workflows_types::*;")?;
...
fn write_workflow(workflow: Workflow, file_name: &str) -> Result<()> {
    ...
    writeln!(&module, "use khulnasoft_workflows_types::*;")?;
