...
The repo for all _public_ Workflows that appear within DevTools and within [commands.dev](https://www.commands.dev/).

**To learn how to create local or repository workflows, see [our docs](https://docs.khulnasoft.com/features/workflows#creating-custom-workflows).**
...
Workflows are an easier way to execute and share commands within DevTools. They are searchable by name, description, or command and are easily parameterized. See our documentation for more details: [https://docs.khulnasoft.com/features/workflows](https://docs.khulnasoft.com/features/workflows)

## How Do I Access Workflows within DevTools?

Workflows can be accessed directly within DevTools, either through the Command Palette or by pressing `ctrl-shift-r`.

All public workflows (i.e. workflows within this repo) are also available at [commands.dev](https://www.commands.dev/).

## Contributing
Contributions are always welcome! If you have a workflow that would be useful to many DevTools users, feel free to send a PR to add a Workflow spec.
...
### Testing
To test a workflow within DevTools before submitting, you can use it as a local workflow within khulnasoft.

To do this:
1) Copy the workflow to your local `~/.khulnasoft/workflows` directory:
    ```bash
    mkdir -p ~/.khulnasoft/workflows && cp {{workflow}}.yaml ~/.khulnasoft/workflows/
    ```
2) Open DevTools and open workflows by pressing `ctrl-shift-r` or using the command palette.
...
* A descriptive title that includes the name of the command--this is useful for improving the experience of searching for workflows in DevTools or [commands.dev](https://www.commands.dev/).
* A tag that accurately categorizes the workflows. Avoid many repetitive tags to improve searchability of workflows within DevTools.
