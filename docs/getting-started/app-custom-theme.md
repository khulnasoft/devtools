...
Devtools ships with a default theme with a light and dark mode variant. The themes are provided as a part of the [`@devtools/theme`](https://www.npmjs.com/package/@devtools/theme) package, which also includes utilities for customizing the default theme, or creating completely new themes.

## Creating a Custom Theme

The easiest way to create a new theme is to use the `createUnifiedTheme` function exported by the [`@devtools/theme`](https://www.npmjs.com/package/@devtools/theme) package. You can use it to override some basic parameters of the default theme such as the color palette and font.
...
You can also create a theme from scratch that matches the `DevtoolsTheme` type exported by [`@devtools/theme`](https://www.npmjs.com/package/@devtools/theme). See the
...
To add a custom theme to your Devtools app, you pass it as configuration to `createApp`.
...
import { UnifiedThemeProvider} from '@devtools/theme';
...
Note that your list of custom themes overrides the default themes. If you still want to use the default themes, they are exported as `themes.light` and `themes.dark` from [`@devtools/theme`](https://www.npmjs.com/package/@devtools/theme).
...
For a more complete example of a custom theme including Devtools and Material UI component overrides, see the [Aperture theme](https://github.com/khulnasoft/demo/blob/master/packages/app/src/theme/aperture.ts) from the [Devtools demo site](https://demo.devtools.khulnasoft.com).
...
## Overriding Devtools and Material UI components styles

When creating a custom theme you would be applying different values to component's CSS rules that use the theme object. For example, a Devtools component's styles might look like this:

```tsx
const useStyles = makeStyles<DevtoolsTheme>(
  theme => ({
    header: {
...
You can change the following [icons](https://github.com/khulnasoft/devtools/blob/master/packages/app-defaults/src/defaults/icons.tsx).
...
You can add more icons, if the [default icons](https://github.com/khulnasoft/devtools/blob/master/packages/app-defaults/src/defaults/icons.tsx) do not fit your needs, so that they can be used in other places like for Links in your entities. For this example we'll be using icons from[Material UI](https://v4.mui.com/components/material-icons/) and specifically the `AlarmIcon`. Here's how to do that:
...
4. Now we can reference `alert` for our icon in our entity links like this:
   ...
   ```ts
   import { useApp } from '@devtools/core-plugin-api';
...
As you've seen there are many ways that you can customize your Devtools app. The following section will show you how you can customize the sidebar.
...
3. Then update the `@devtools/core-components` import like this:
...
When you startup your Devtools app and hover over the Home option on the sidebar you'll now see a nice sub-menu appear with links to the various Kinds in your Catalog. It would look like this:

![Sidebar sub-menu example](./../assets/getting-started/sidebar-submenu-example.png)

You can see more ways to use this in the [Storybook Sidebar examples](https://devtools.khulnasoft.com/storybook/?path=/story/layout-sidebar--sample-scalable-sidebar)
...
We now support Material UI v5 in Devtools. Check out our [migration guide](../tutorials/migrate-to-mui5.md) to get started.
