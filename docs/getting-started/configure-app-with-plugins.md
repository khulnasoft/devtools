...
:::note Note
Devtools plugins are primarily written using [TypeScript](https://www.typescriptlang.org), [Node.js](https://nodejs.org) and [React](https://reactjs.org). Having an understanding of these technologies will be beneficial on your journey to customizing Devtools!
:::

## Summary

Devtools plugins customize the app for your needs. There is a
[plugin directory](https://devtools.khulnasoft.com/plugins) with plugins for many common
infrastructure needs - CI/CD, monitoring, auditing, and more.

## Adding existing plugins to your app

The following steps assume that you have
[created a Devtools app](./index.md) and want to add an existing plugin
to it.

We are using the
[CircleCI](https://github.com/CircleCI-Public/devtools-plugin/tree/main/plugins/circleci)
...
1. Add the plugin's npm package to the repo:
   ...
   yarn --cwd packages/app add @circleci/devtools-plugin
   ```

   Note the plugin is added to the `app` package, rather than the root
   `package.json`. Devtools Apps are set up as monorepos with
...
2. Add the `EntityCircleCIContent` extension to the entity pages in the app:
   ...
   This is just one example, but each Devtools instance may integrate content or
...
In a standard Devtools app created with
[@devtools/create-app](./index.md), the sidebar is managed inside
