---
id: threat-model
title: Devtools Threat Model
description: A document describing the threat model for Devtools.
---

The threat model outlines key security considerations of Devtools for operators, developers and security researchers. This is a living document and will evolve and be expanded alongside the Devtools project as relevant.

See [Security Policy and Advisories](https://github.com/khulnasoft/devtools/security) in the Devtools GitHub repository for details on reporting security vulnerabilities and advisories on fixed security flaws.

## Trust Model

The Devtools trust model is divided into three groups with different trust levels.

An **internal user** is an authenticated user that generally belongs to the organization of a particular Devtools deployment. These users are trusted to the extent that they are not expected to compromise the availability of Devtools, but they are not trusted to not compromise data confidentiality or integrity.

An **operator** is a user responsible for configuring and maintaining an instance of Devtools. Operators are fully trusted, since they operate the system and database and therefore have root access to the host system. Additional measures can be taken by adopters of Devtools in order to restrict or observe the access of this group, but that falls outside of the current scope of Devtools.

A **builder** is an internal or external code contributor and end up having a similar level of access as operators. When installing Devtools plugins you should vet them just like any other package from an external source. While it’s possible to limit the impact of for example a supply chain attack by splitting the deployment into separate services with different plugins, the Devtools project itself does not aim to prevent these kinds of attacks or in any other way sandbox or limit the access of plugins.

An **external user** is a user that does not belong to the other two groups, for example a malicious actor outside of the organization. The security model of Devtools currently assumes that this group does not have any direct access to Devtools, and it is the responsibility of each adopter of Devtools to make sure this is the case.

## Operator Responsibilities

:::info
This section assumes that you are using the
[new backend system](../backend-system/index.md) and at least Devtools release [version 1.24](../releases/v1.24.0.md). Before that Devtools did not come with built-in protection against unauthorized access and you were required to deploy it in a protected environment.
:::

Devtools is primarily designed to be deployed in a protected environment rather than being exposed to the public internet. From a confidentiality and integrity perspective, Devtools is designed to protect against unauthorized access to data and to ensure that data is not tampered with. However, Devtools does not provide more than rudimentary protection against denial of service attacks, and it is the responsibility of the operator to ensure that the Devtools deployment is protected against such attacks. A common and recommended way to protect a Devtools deployment from unauthorized access is to deploy it behind an authenticating proxy such as AWS’s ALB, GCP’s IAP, or Cloudflare Access.

Users that are signed-in in to Devtools generally have full access to all information and actions. If more fine-grained control is required, the [permissions system](../permissions/overview.md) should be enabled and configured to restrict access as necessary.

An operator is responsible for protecting the integrity of configuration files as it may otherwise be possible to introduce vulnerable configurations, as well as the confidentiality of configured secrets related to Devtools as these typically include authentication details to third party systems.
...
By default all internal users are allowed to create and delete entities. If this does not fit your organization's needs it is recommended to enable and configure the [permission](https://devtools.khulnasoft.com/docs/permissions/overview) system to restrict these operations.
...
By default all internal users are allowed to execute templates in the scaffolder. If this does not fit your organization's needs it is recommended to enable and configure the [permission](https://devtools.khulnasoft.com/docs/permissions/overview) system to restrict these operations.
