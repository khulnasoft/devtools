...
Plugins provide the actual base features of a Devtools backend. Each plugin operates completely independently of all other plugins and they only communicate with each other through network calls. This means that there is a strong degree of isolation between plugins, and that each plugin can be considered a separate microservice. While a default Devtools project has all plugins installed within a single backend, it is also possible to split this setup into multiple backends, with each backend housing one or more plugins.
...
The following rules apply to the production setup of Devtools plugins in the broader Devtools plugin ecosystem. Any plugin that is maintained under the `@devtools` package namespace should follow these rules, and it is recommended that all widely distributed plugins follow these rules as well.
