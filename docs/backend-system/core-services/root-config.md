...
backend.add(
  rootConfigServiceFactory({
    argv: [
      '--config',
      '/devtools/app-config.development.yaml',
      '--config',
      '/devtools/app-config.yaml',
    ],
...
For more advanced customization, there are several APIs from the `@devtools/config-loader` package that allow you to customize the implementation of the config service. The default implementation uses the `ConfigSources.default` method, which has the same options as the `rootConfigServiceFactory` function. You can use these to create your own config service implementation:
