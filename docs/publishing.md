...
[`.github/workflows/deploy_packages.yml`](https://github.com/khulnasoft/devtools/blob/master/.github/workflows/deploy_packages.yml)
...
- Check [`Version Packages (next)` Pull Request](https://github.com/khulnasoft/devtools/pulls?q=is%3Aopen+is%3Apr+in%3Atitle+%22Version+Packages+%28next%29%22)
  ...
  - Check `pre.json` in [the `.changeset` folder](https://github.com/khulnasoft/devtools/blob/master/.changeset) - it should have `"mode": "pre"` near the top. If you encounter `"mode": "exit"` or if the file doesn't exist, it indicates a mainline release.
- Verify that there are no active/unfinished `sync_version-packages` actions running (https://github.com/khulnasoft/devtools/actions/workflows/sync_version-packages.yml)
...
- Check [`Version Packages (next)` Pull Request](https://github.com/khulnasoft/devtools/pulls?q=is%3Aopen+is%3Apr+in%3Atitle+%22Version+Packages+%28next%29%22) for sufficient approval to be merged
...
Merging the `Version Packages (next)` Pull Request will trigger the deployment workflows. Follow along the [deployment workflow](https://github.com/khulnasoft/devtools/actions/workflows/deploy_packages.yml). If you notice flakiness (e.g. if the build is flaky or if the release step runs into an error with releasing to npm) just restart the workflow.

When the release is ready to be published, maintainers will be notified in the private `#maintainers` channel on the Devtools Discord Server. They will then start the publishing workflow on the publishing workflows page linked in the notification message. On the publishing workflows page there is a dropdown menu called "Run workflow", this opens a form where the person responsible for the release should paste the SHA received in the Discord notification and click to run the workflow.
...
- Check [`Version Packages` Pull Request](https://github.com/khulnasoft/devtools/pulls?q=is%3Aopen+is%3Apr+in%3Atitle+%22Version+Packages)
...
- Create Release Notes PR
  ...
  - Add an entry to [`/microsite/sidebar.js`](https://github.com/khulnasoft/devtools/blob/master/microsite/sidebars.js) for the release note
  - Update the navigation bar item in [`/microsite/docusaurus.config.ts`](https://github.com/khulnasoft/devtools/blob/master/microsite/docusaurus.config.ts) to point to the new release note
  - Finally copy the content, without the metadata header, into the description of the [`Version Packages` Pull Request](https://github.com/khulnasoft/devtools/pulls?q=is%3Aopen+is%3Apr+in%3Atitle+%22Version+Packages)

Once the release has been published edit the newly created release in the [GitHub repository](https://github.com/khulnasoft/devtools/releases) and replace the text content with the release notes.
...
**This emergency release process is intended only for the Devtools
maintainers.**
...
For this example we will be using the `@devtools/plugin-foo` package as an
...
`@devtools/plugin-foo` released in the `v1.18.0` Devtools release, the following
...
- [ ] Once fixes have been created for each release, the fix should be applied to the
  ...
  - [ ] A changeset with the message "Applied the fix from version `6.5.1` of this package, which is part of the `v1.18.1` release of Devtools."

## Troubleshooting

### When the release workflow is not triggered for some reason, such as a GitHub incident

Ask one of the maintainers to trigger [the Deploy packages](https://github.com/khulnasoft/devtools/actions/workflows/deploy_packages.yml) workflow with the "Unconditionally trigger the release job to run" checkbox set, on the `master` branch. Please validate first that nothing substantial has been pushed to master since the original failed release attempt! For this reason, it is wise to have the master branch locked until each release has gone through.
...
- Manually create a [new GitHub release](https://github.com/khulnasoft/devtools/releases/new).
- Trigger the repository dispatch workflow using the following request, replace `<VERSION>` with the release version **without** the `v` prefix:
  ...
  curl -L \
    ...
    https://api.github.com/repos/khulnasoft/devtools/dispatches \
