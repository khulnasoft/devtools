...
description: Importing users and groups from GitLab into Devtools
---
...
yarn --cwd packages/backend add @devtools/plugin-catalog-backend-module-gitlab @devtools/plugin-catalog-backend-module-gitlab-org
```
...
// optional - event router for gitlab. See.: https://github.com/khulnasoft/devtools/blob/master/plugins/events-backend-module-gitlab/README.md
...
backend.add(import('@devtools/plugin-catalog-backend-module-gitlab-org'));
```

You need to decide how you want to receive events from external sources like

- [via HTTP endpoint](https://github.com/khulnasoft/devtools/blob/master/plugins/events-backend/README.md#configuration)
...
- [Events Plugin](https://github.com/khulnasoft/devtools/tree/master/plugins/events-backend/README.md)
- [GitLab Module for the Events Plugin](https://github.com/khulnasoft/devtools/blob/master/plugins/events-backend-module-gitlab/README.md)
...
import { GitlabOrgDiscoveryEntityProvider } from '@devtools/plugin-catalog-backend-module-gitlab';
...
To use the entity provider, you'll need a [Gitlab integration set up](https://devtools.khulnasoft.com/docs/integrations/gitlab/locations).
  ...
  providers:
    gitlab:
      yourProviderId:
        ...
        orgEnabled: true
        group: org/teams # Required for gitlab.com when `orgEnabled: true`. Optional for self managed. Must not end with slash. Accepts only this group and groups under the provided path (which will be stripped)
        restrictUsersToGroup: true # Optional: Devtools will ingest only users directly assigned to org/teams.
...
into Devtools entities. You can do this on the user and group requests to
...
The example below uses the groupNameTransformer option to change the metadata.name property of the Devtools Group Entity. Instead of populating it with the usual `group.full_path` data that comes from GitLab, it uses the `group.id`:
...
} from '@devtools/plugin-catalog-backend-module-gitlab';
