...
# prettier-ignore
description: Importing users and groups from Microsoft Entra ID into Devtools
---
...
The package is not installed by default, therefore you have to add `@devtools/plugin-catalog-backend-module-msgraph` to your backend package.
...
Entities can also be excluded from devtools by returning `undefined`.
...
} from '@devtools/plugin-catalog-backend-module-msgraph';
import { GroupEntity, UserEntity } from '@devtools/catalog-model';
...
export async function myUserTransformer(
  ...
  const devtoolsUser = await defaultUserTransformer(graphUser, userPhoto);

  if (devtoolsUser) {
    devtoolsUser.metadata.description = 'Loaded from Microsoft Entra ID';
  }
