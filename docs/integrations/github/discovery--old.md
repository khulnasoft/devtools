...
dependency on `@devtools/plugin-catalog-backend-module-github` to your backend
package.

```bash title="From your Devtools root directory"
yarn --cwd packages/backend add @devtools/plugin-catalog-backend-module-github
```
...
import { GithubEntityProvider } from '@devtools/plugin-catalog-backend-module-github';
...
[set up](locations.md) with either a [Personal Access Token](../../getting-started/config/authentication.md) or [GitHub Apps](./github-apps.md). For Personal Access Tokens you should pay attention to the [required scopes](https://devtools.khulnasoft.com/docs/integrations/github/locations/#token-scopes), where you will need at least the `repo` scope for reading components. For GitHub Apps you will need to grant it the [required permissions](https://devtools.khulnasoft.com/docs/integrations/github/github-apps#app-permissions) instead, where you will need at least the `Contents: Read-only` permissions for reading components.
    ...
    github:
      ...
      topicProviderId:
        ...
        filters:
          branch: 'main' # string
          repository: '.*' # Regex
          topic: 'devtools-exclude' # optional string
      topicFilterProviderId:
        ...
        filters:
          ...
          repository: '.*' # Regex
          topic:
            include: ['devtools-include'] # optional array of strings
...
- **`filters`** _(optional)_:
  ...
  - **`topic`** _(optional)_:
    ...
    In the example above, a repository with the `devtools-include` topic would still be excluded
...
accounts). The snippet below refreshes the Devtools catalog data every 35 minutes, which issues an API request for each discovered location.
...
More information about scheduling can be found on the [SchedulerServiceTaskScheduleDefinition](https://devtools.khulnasoft.com/docs/reference/backend-plugin-api.schedulerservicetaskscheduledefinition) page.
...
backend. They are not installed by default, therefore you have to add a
dependency on `@devtools/plugin-catalog-backend-module-github` to your backend
package, plus `@devtools/integration` for the basic credentials management:

```bash title="From your Devtools root directory"
yarn --cwd packages/backend add @devtools/integration @devtools/plugin-catalog-backend-module-github
```
...
catalog:
  locations:
    ...
    - type: github-discovery
      target: https://github.com/*/blob/devtools-docs/catalog-info.yaml
```
...
GitHub [rate limits](https://docs.github.com/en/rest/overview/resources-in-the-rest-api#rate-limiting) API requests to 5,000 per hour (or more for Enterprise
accounts). The default Devtools catalog backend refreshes data every 100
