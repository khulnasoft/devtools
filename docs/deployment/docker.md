...
description: How to build a Devtools Docker image for deployment
---
...
This section describes how to build a Devtools App into a deployable Docker
...
This section assumes that an [app](https://devtools.khulnasoft.com/docs/getting-started/)
has already been created with `@devtools/create-app`, in which the frontend is
bundled and served from the backend. This is done using the
`@devtools/plugin-app-backend` plugin, which also injects the frontend
configuration into the app. This means that you only need to build and deploy a
single container in a minimal setup of Devtools. If you wish to separate the
serving of the frontend out from the backend, see the
[separate frontend](#separate-frontend) topic below.

## Host Build

This section describes how to build a Docker image from a Devtools repo with
...
`Dockerfile` is included when creating a new app with `@devtools/create-app`:
...
# Install isolate-vm dependencies, these are needed by the @devtools/plugin-scaffolder-backend.
...
The `@devtools/create-app` command adds the following `.dockerignore` in the
...
```bash
docker image build . -f packages/backend/Dockerfile --tag devtools
```

To try out the image locally you can run the following:

```sh
docker run -it -p 7007:7007 devtools
```
...
WORKDIR /app
COPY devtools.json package.json yarn.lock ./
COPY .yarn ./.yarn
COPY .yarnrc.yml ./
...
COPY --from=packages --chown=node:node /app/devtools.json ./
...
COPY --from=build --chown=node:node /app/devtools.json ./
...
Note that a newly created Devtools app will typically not have a `plugins/`
folder, so you will want to comment that line out. This build also does not work
in the main repo, since the `devtools-cli` which is used for the build doesn't
...
`@devtools/plugin-app-backend` plugin. Most notably the frontend configuration
...
3. Remove the `@devtools/plugin-app-backend` and the app package dependency
...
[contrib folder in the main repo](https://github.com/khulnasoft/devtools/blob/master/contrib/docker/frontend-with-nginx)
...
```sh
docker image build . -f packages/backend/Dockerfile --tag devtools --progress=plain --no-cache
```

## Community Contributed Dockerfile Alternatives

The `Dockerfile` mentioned above located in `packages/backend` is maintained by the maintainers of Devtools, however there are also community contributed Dockerfile alternatives located in `contrib/docker`. The `Dockerfile`s in `contrib/docker` are not maintained by the maintainers of Devtools and are not necessarily updated when the `Dockerfile` located in `packages/backend` is updated.

### Minimal Hardened Image

A contributed `Dockerfile` exists within the directory of `contrib/docker/minimal-hardened-image` which uses the [`wolfi-base`](https://github.com/wolfi-dev) image to reduce vulnerabilities. When this was contributed, this alternative `Dockerfile` reduced 98.2% of vulnerabilities in the built Devtools docker image when compared with the image built from `packages/backend/Dockerfile`.
