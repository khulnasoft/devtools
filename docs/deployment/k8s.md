...
description: How to deploy Devtools to a Kubernetes cluster
---

[Kubernetes](https://kubernetes.io/) is a system for deploying, scaling and
managing containerized applications. Devtools is designed to fit this model and
...
This guide covers basic Kubernetes definitions needed to get Devtools up and
running in a typical cluster. The object definitions might look familiar, since
the Devtools software catalog
...
Devtools in production uses PostgreSQL as a database. To isolate the database
from Devtools app deployments, we can create a separate Kubernetes deployment
for PostgreSQL.

### Creating a PostgreSQL secret

First, create a Kubernetes Secret for the PostgreSQL username and password. This
will be used by both the PostgreSQL database and Devtools deployments:
...
$ kubectl get pods --namespace=devtools
...
```shell
$ kubectl exec -it --namespace=devtools postgres-56c86b8bbc-66pt2 -- /bin/bash
...
$ kubectl get services --namespace=devtools
NAME       TYPE        CLUSTER-IP    EXTERNAL-IP   PORT(S)    AGE
postgres   ClusterIP   10.96.5.103   <none>        5432/TCP   29s
```

## Creating the Devtools instance

Now that we have PostgreSQL up and ready to store data, we can create the
Devtools instance. This follows similar steps as the PostgreSQL deployment.

### Creating a Devtools secret

For any Devtools configuration secrets, such as authorization tokens, we can
create a similar Kubernetes Secret as we did
[for PostgreSQL](#creating-a-postgresql-secret), remembering to base64 encode
the values:

```yaml
# kubernetes/devtools-secrets.yaml
apiVersion: v1
kind: Secret
metadata:
...
```shell
$ kubectl apply -f kubernetes/devtools-secrets.yaml
secret/devtools-secrets created
```

### Creating a Devtools deployment

To create the Devtools deployment, first create a [Docker image](docker.md).
...
# https://devtools.khulnasoft.com/docs/plugins/observability#health-checks
#          readinessProbe:
#            httpGet:
#              port: 7007
#              path: /healthcheck
...
$ yarn build-image --tag devtools:1.0.0
```
...
`POSTGRES_PORT` environment variables into our Devtools container.
These can be used in the Devtools `app-config.yaml` along with the secrets. Apply this to `app-config.production.yaml` as well if you have one:
...
deployment.apps/devtools created

$ kubectl get deployments --namespace=devtools
...
$ kubectl get pods --namespace=devtools
NAME                                 READY   STATUS    RESTARTS   AGE
devtools-54bfcd6476-n2jkm           1/1     Running   0          58s
...
$ kubectl logs --namespace=devtools -f devtools-54bfcd6476-n2jkm -c devtools
```

### Creating a Devtools service

Like the [PostgreSQL service](#creating-a-postgresql-service) above, we need to
create a Kubernetes Service for Devtools to handle connecting requests to the
correct pods.

Create the Kubernetes Service descriptor:

```yaml
# kubernetes/devtools-service.yaml
apiVersion: v1
kind: Service
metadata:
  name: devtools
...
service/devtools created
```

Now we have a fully operational Devtools deployment! 🎉 For a _**grand
reveal**_, you can forward a local port to the service:

```shell
$ sudo kubectl port-forward --namespace=devtools svc/devtools 80:80
...
browse your Kubernetes-deployed Devtools instance. 🚢🚢🚢

## Further steps

This is most of the way to a full production deployment of Devtools on
...
Devtools Docker image, update the image tag reference in `devtools.yaml` and
then apply the changes with `kubectl apply -f kubernetes/devtools.yaml`.
