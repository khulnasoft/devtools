...
# prettier-ignore
description: Configuring the Kubernetes integration for Devtools expose your entity's objects
---

Configuring the Devtools Kubernetes integration involves two steps:
...
```js
import { createTranslationMessages } from '@devtools/core-plugin-api/alpha';
import { kubernetesReactTranslationRef } from '@devtools/plugin-kubernetes-react/alpha';
...
[Resources](https://devtools.khulnasoft.com/docs/features/software-catalog/system-model#resource)
of
[type](https://devtools.khulnasoft.com/docs/features/software-catalog/descriptor-format#spectype-required-4)
...
[annotations](https://devtools.khulnasoft.com/docs/features/software-catalog/descriptor-format#annotations-optional)
(as seen
[here](https://github.com/khulnasoft/devtools/blob/86baccb2d7d378baed74eaebf017c60b410986e5/plugins/kubernetes-backend/src/cluster-locator/CatalogClusterLocator.ts#L51-L61)
in the code):

- [`kubernetes.io/api-server`](https://devtools.khulnasoft.com/docs/reference/plugin-kubernetes-common.annotation_kubernetes_api_server/),
  denoting the base URL of the Kubernetes control plane
- [`kubernetes.io/api-server-certificate-authority`](https://devtools.khulnasoft.com/docs/reference/plugin-kubernetes-common.annotation_kubernetes_api_server_ca/),
  containing a base64-encoded certificate authority bundle in PEM format;
  Devtools will check that the control plane presents a certificate signed by
  this authority.
- [`kubernetes.io/auth-provider`](https://devtools.khulnasoft.com/docs/reference/plugin-kubernetes-common.annotation_kubernetes_auth_provider/),
...
configure the way Devtools communicates, documented
[here](https://devtools.khulnasoft.com/docs/reference/plugin-kubernetes-common#variables)
...
procedure like the
[`GkeEntityProvider`](https://devtools.khulnasoft.com/docs/reference/plugin-catalog-backend-module-gcp.gkeentityprovider/)
(installation documented
[here](https://github.com/khulnasoft/devtools/tree/master/plugins/catalog-backend-module-gcp#installation))
or the
[`AwsEKSClusterProcessor`](https://devtools.khulnasoft.com/docs/reference/plugin-catalog-backend-module-aws.awseksclusterprocessor/)
to automatically update the set of clusters tracked by Devtools.
...
| Value                  | Description                                                                                                                                                                                                                                                                                                                               |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `aks`                  | This will use a user's AKS access token from the [Microsoft auth provider](https://devtools.khulnasoft.com/docs/auth/microsoft/provider) to access the Kubernetes API on AKS clusters.                                                                                                                                                               |
...
| `google`               | This will use a user's Google access token from the [Google auth provider](https://devtools.khulnasoft.com/docs/auth/google/provider) to access the Kubernetes API on GKE clusters.                                                                                                                                                                  |
...
| `serviceAccount`       | This will use a Kubernetes [service account](https://kubernetes.io/docs/reference/access-authn-authz/service-accounts-admin/) to access the Kubernetes API. When this is used the `serviceAccountToken` field should also be set, or else Devtools should be running in-cluster.                                                         |
...
in place or have a single Kubernetes cluster running both Devtools and all your
...
field is omitted, Devtools will ignore the configured URL and certificate data,
...
from a configured [devtools auth provider](https://devtools.khulnasoft.com/docs/auth/) to
...
```ts
import { clusterLinksFormatters } from '@devtools/plugin-kubernetes';
clusterLinksFormatters.myDashboard = (options) => ...;
```

See also
https://github.com/khulnasoft/devtools/tree/master/plugins/kubernetes/src/utils/clusterLinks/formatters
for real examples.
...
Filesystem path (on the host where the Devtools process is running) to a
...
on the Devtools backend to the path of the service account key file.
...
### Common `devtools.khulnasoft.com/kubernetes-id` label

#### Adding the entity annotation

In order for Devtools to detect that an entity has Kubernetes components, the
...
Entities can have the `devtools.khulnasoft.com/kubernetes-namespace` annotation, this will cause the entity's Kubernetes resources
...
```yaml
'devtools.khulnasoft.com/kubernetes-id': <DEVTOOLS_ENTITY_NAME>
```

### Label selector query annotation

You can write your own custom label selector query that Devtools will use to
...
In the example above, we configured the "devtools.khulnasoft.com/kubernetes-cluster" annotation on the entity `catalog-info.yaml` file to specify that the current component is running in a single cluster called "dice-cluster", so this cluster must have been specified in the `app-config.yaml`, under the Kubernetes clusters configuration (for more details, see [`Configuring Kubernetes clusters`](#configuring-kubernetes-clusters)).

If you do not specify the annotation, by default Devtools fetches from all defined Kubernetes clusters.
...
[4]: https://devtools.khulnasoft.com/docs/features/kubernetes/authentication
