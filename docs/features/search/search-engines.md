...
description: Choosing and configuring your search engine for Devtools
---

Devtools supports 3 search engines by default, an in-memory engine called [Lunr](#lunr), [Postgres](#postgres)
and [Elasticsearch](#elasticsearch).

## Lunr

Lunr search engine is enabled by default for your Devtools instance if you have not done additional changes to the scaffolded app.

As Lunr is built into the Search backend plugin it can be added like this:

```bash title="From your Devtools root directory"
yarn --cwd packages/backend add @devtools/plugin-search-backend
```
...
other parts of Devtools locally, however its use is highly discouraged when
running Devtools in production. When deploying Devtools, use one of the
...
the database engine for Devtools. Therefore it targets setups that want to
...
```bash title="From your Devtools root directory"
yarn --cwd packages/backend add @devtools/plugin-search-backend-module-pg
```
...
Devtools supports Elasticsearch (and OpenSearch) search engine connections,
...
```bash title="From your Devtools root directory"
yarn --cwd packages/backend add @devtools/plugin-search-backend-module-elasticsearch
```
...
[configuration schema definition file.](https://github.com/khulnasoft/devtools/blob/master/plugins/search-backend-module-elasticsearch/config.d.ts)
...
[Devtools documentation.](https://devtools.khulnasoft.com/docs/conf/writing#includes-and-dynamic-data)
