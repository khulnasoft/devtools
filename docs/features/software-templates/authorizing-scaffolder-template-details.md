...
The scaffolder plugin integrates with the Devtools [permission framework](../../permissions/overview.md), which allows you to control access to certain parameters and steps in your templates based on the user executing the template. It also allows you to control access to scaffolder tasks.

### Authorizing parameters and steps

To mark specific parameters or steps as requiring permission, add the `devtools:permissions` property to the parameter or step with one or more tags. For example:
...
} from '@devtools/plugin-scaffolder-backend/alpha';
/* highlight-add-end */
...
```ts title="packages/backend/src/plugins/permission.ts"
/* highlight-add-start */
import { actionExecutePermission } from '@devtools/plugin-scaffolder-common/alpha';
import {
  createScaffolderActionConditionalDecision,
