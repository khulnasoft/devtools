        ...
        inputObject:
          ...
          properties:
            first:
              type: string
              title: first
              devtools:featureFlag: nested-experimental-feature
            second:
              type: number
              title: second
```
...
spec. Make sure to use the key `devtools:featureFlag` in your templates if
...
Devtools. The `OwnerPicker` is a custom field that generates a searchable list
...
The `catalogFilter` allow you to filter the list entities using any of the [catalog api filters](https://devtools.khulnasoft.com/docs/features/software-catalog/software-catalog-api#filtering):
...
This section introduces the built-in filters provided by Devtools and offers examples of
how to use them in the Scaffolder templates. It's important to mention that Devtools also leverages the
...
- **Input**: `github.com?repo=devtools&org=devtools`
- **Output**: [RepoSpec](https://github.com/khulnasoft/devtools/blob/v1.17.2/plugins/scaffolder-backend/src/scaffolder/actions/builtin/publish/util.ts#L39)
...
- **Output**: [CompoundEntityRef](https://github.com/khulnasoft/devtools/blob/v1.17.2/packages/catalog-model/src/types.ts#L23)
...
/* highlight-add-start */
import { scaffolderTemplatingExtensionPoint } from '@devtools/plugin-scaffolder-node/alpha';
...
The custom filed explorer allows you to select any custom field loaded on the devtools instance and test different values and configurations.
