...
title: Devtools Software Templates
sidebar_label: Overview
# prettier-ignore
description: The Software Templates part of Devtools is a tool that can help you create Components inside Devtools
---

The Software Templates part of Devtools is a tool that can help you create
Components inside Devtools. By default, it has the ability to load skeletons of
...
> [Getting Started with Devtools](../../getting-started) before proceeding.

:::note Note

If you're running Devtools with Node 20 or later, you'll need to pass the flag `--no-node-snapshot` to Node in order to
use the templates feature.
One way to do this is to specify the `NODE_OPTIONS` environment variable before starting Devtools:
...
are required for Devtools usage: the owner (which is a `user` in the devtools
system), and the `storePath` which is a destination URL to create for the
provider, for instance `https://github.com/khulnasoft/my-new-repository`, or
`https://gitlab.com/myorg/myrepo`.

![Enter Devtools vars](../../assets/software-templates/template-picked-2.png)
...
This can be done in `devtools/packages/app/src/App.tsx`:
