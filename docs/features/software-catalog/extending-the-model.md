...
The Devtools catalog [entity data model](descriptor-format.md) is based on the
...
Devtools comes with a number of catalog concepts out of the box:
...
> `devtools.khulnasoft.com`."

The `devtools.khulnasoft.com` apiVersion space is reserved for use by the Devtools
...
family, or an idea if you will, of entities that also share a schema. Devtools
comes with a number of builtin ones that we believe are useful for a large
variety of needs that one may want to model in Devtools. The primary ambition
...
`@devtools/catalog-model` package and have expectations that kinds align with
that.
...
impact. The very foundation of Devtools is to attach behavior and views and
...
builtin kinds, feel free to reach out to the Devtools maintainers to discuss
how to best proceed.
...
may affect what operations and views are enabled in Devtools for that entity.
...
Secondly, Devtools helps your engineers manage their software by integrating
the infrastructure tooling through plugins. Different plugins are used for
managing different types of components.

For example, the
[Lighthouse plugin](https://github.com/khulnasoft/community-plugins/tree/main/workspaces/lighthouse/plugins/lighthouse)
...
Devtools maintainers or a support partner to discuss your use case.
...
the data and import the typescript data type from the `@devtools/catalog-model`
package.
...
free to contact the Devtools maintainers or a support partner to discuss your
...
> to be able to show the ongoing alerts for our services in Devtools so it'd be
...
- The `devtools.khulnasoft.com` annotation prefix is reserved for use by the Devtools
...
- Annotations that have no prefix at all, are considered local to your Devtools
...
your use case fits the labels best, we would appreciate if you let the Devtools
maintainers know.
...
- The `devtools.khulnasoft.com` label prefix is reserved for use by the Devtools
  maintainers. Reach out to us if you feel that you would like to make an
  addition to that prefix.
...
- Labels that have no prefix at all, are considered local to your Devtools
  instance and can be used freely as such, but you should not make use of them
  outside of your organization. For example, if you were to open source a plugin
...
offering, reach out to the Devtools maintainers or a support partner.
...
an effect outside of your organization, reach out to the Devtools maintainers
...
> Devtools, and it would be useful if the entity's status field contained the
...
organization be namespaced to avoid collisions. Statuses emitted by Devtools
core processes will for example be prefixed with `devtools.khulnasoft.com/`, your
...
[This issue](https://github.com/khulnasoft/devtools/issues/2292) also contains
more context.
...
where something like Devtools can offer the most value and effectiveness of
...
want to have an isomorphic package that houses these types. Within the Devtools
...
using the `@devtools/cli`. Perhaps the simplest way to get started right now is
...
[scaffolder-common](https://github.com/khulnasoft/devtools/tree/master/plugins/scaffolder-common/src/index.ts)
...
[ScaffolderEntitiesProcessor](https://github.com/khulnasoft/devtools/tree/master/plugins/catalog-backend-module-scaffolder-entity-model/src/processor/ScaffolderEntitiesProcessor.ts).
...
```ts
import { CatalogProcessor, CatalogProcessorEmit, processingResult } from '@devtools/plugin-catalog-node';
import { LocationSpec } from '@devtools/plugin-catalog-common'
import { Entity, entityKindSchemaValidator } from '@devtools/catalog-model';

// For an example of the JSONSchema format and how to use $ref markers to the
// base definitions, see:
// https://github.com/khulnasoft/devtools/tree/master/packages/catalog-model/src/schema/kinds/Component.v1alpha1.schema.json
