...
and then presented through an API for quick and easy access by Devtools and
...
feature of Devtools. It's rather the table that backs the ability to filter
...
First, the catalog backend will emit events using the [events backend plugin](https://github.com/khulnasoft/devtools/tree/master/plugins/events-node). You can subscribe to the events. The events should contain
...
tool for Devtools operators who want to debug problems either with statically
registered entities that are under their control, or to help end users find
problems.

> Prior to Devtools version v1.26.0 and `@devtools/plugin-catalog-backend` v1.21.9 catalog errors were logged by default.
...
of your entity page in Devtools, if you have placed the corresponding error
...
- The stitching process injects a `devtools.khulnasoft.com/orphan: 'true'` annotation on
  the child entity.
...
- The catalog page in Devtools for the child entity detects the new annotation
...
As an end user of the Devtools interface, you may now want to delete one of the
