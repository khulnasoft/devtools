...
for this API can be found [here](https://github.com/khulnasoft/devtools/blob/master/plugins/catalog-backend/src/schema/openapi.yaml).
A UI visualizing the OpenAPI endpoints including the ability to try them out in the browser can be found [here](https://www.postman.com/devtools-io/workspace/catalog-api/overview).
...
with a `Bearer` token, which should then be the Devtools token returned by the
[`identity API`](https://devtools.khulnasoft.com/docs/reference/core-plugin-api.identityapiref).
...
- Return all orphaned entities:
  ...
  `/entities/by-query?filter=metadata.annotations.devtools.khulnasoft.com/orphan=true`
...
- Return all orphaned entities:
  ...
  `/entities?filter=metadata.annotations.devtools.khulnasoft.com/orphan=true`
