...
```json
{
  ...
  "metadata": {
    "annotations": {
      "devtools.khulnasoft.com/managed-by-location": "file:/tmp/catalog-info.yaml",
      "example.com/service-discovery": "artistweb",
      "circleci.com/project-slug": "github/example-org/artist-website"
    },
    "description": "The place to be, for great artists",
...
organization using Devtools is free to also add entities of other kinds to the
catalog.
...
`devtools.khulnasoft.com/`, to distinguish them from other types of object that share the
...
additional contextual information that may be located outside of Devtools
...
additional functionality within Devtools, which is best left to `annotations`
...
one of `[-_.]`. Devtools may support some basic icons out of the box such as those [defined in app-defaults](https://github.com/khulnasoft/devtools/blob/master/packages/app-defaults/src/defaults/icons.tsx), but the
Devtools integrator will ultimately be left to provide the appropriate icon
...
Types emitted by Devtools core processes will for example be prefixed with
`devtools.khulnasoft.com/` as in the example above.
...
Exactly equal to `devtools.khulnasoft.com/v1alpha1` and `Component`, respectively.
...
great care to establish a proper taxonomy for these. Tools including Devtools
itself may read this field and behave differently depending on its value. For
example, a website type component may present tooling in the Devtools interface
...
In Devtools, the owner of a component is the singular entity (commonly a team)
...
this field is for display purposes in Devtools, so that people looking at
...
metadata:
  ...
  annotations:
    devtools.khulnasoft.com/time-saved: PT4H
spec:
...
Exactly equal to `devtools.khulnasoft.com/v1beta2` and `Template`, respectively.
...
### `metadata.annotations.[devtools.khulnasoft.com/time-saved]` [optional]
...
Can be used in combination with the `devtools.khulnasoft.com/source-template` annotation,
...
In Devtools, the owner of a Template is the singular entity (commonly a team)
...
Exactly equal to `devtools.khulnasoft.com/v1alpha1` and `API`, respectively.
...
In Devtools, the owner of an API is the singular entity (commonly a team) that
...
display purposes in Devtools, so that people looking at catalog items can get
...
Exactly equal to `devtools.khulnasoft.com/v1alpha1` and `Group`, respectively.
...
This field is an
[entity reference](https://devtools.khulnasoft.com/docs/features/software-catalog/references).
...
the Devtools ecosystem works. See the [auth](https://devtools.khulnasoft.com/docs/auth)
...
Exactly equal to `devtools.khulnasoft.com/v1alpha1` and `User`, respectively.
...
Exactly equal to `devtools.khulnasoft.com/v1alpha1` and `Resource`, respectively.
...
In Devtools, the owner of a resource is the singular entity (commonly a team)
...
Exactly equal to `devtools.khulnasoft.com/v1alpha1` and `System`, respectively.
...
In Devtools, the owner of a system is the singular entity (commonly a team)
...
Exactly equal to `devtools.khulnasoft.com/v1alpha1` and `Domain`, respectively.
...
In Devtools, the owner of a domain is the singular entity (commonly a team)
...
Exactly equal to `devtools.khulnasoft.com/v1alpha1` and `Location`, respectively.
