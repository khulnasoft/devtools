...
# prettier-ignore
description: Documentation on External integrations to integrate systems with Devtools
---

Devtools natively supports importing catalog data through the use of
...
leverage those systems by integrating them with Devtools. This article shows
...
[example backend here](https://github.com/khulnasoft/devtools/blob/master/packages/backend-legacy/src/plugins/catalog.ts).
We will create a new
[`EntityProvider`](https://github.com/khulnasoft/devtools/blob/master/plugins/catalog-node/src/api/provider.ts)
...
`plugins` folder of your Devtools repo:
...
5. The method translates the foreign data model to the native `Entity` form, as
   ...
   `devtools.khulnasoft.com/managed-by-location` and
   `devtools.khulnasoft.com/managed-by-origin-location annotations`; otherwise, it will not
   appear in the Catalog and will generate warning logs. The
   [Well-known Annotations](./well-known-annotations.md#devtoolsiomanaged-by-location)
...
`https://github.com/khulnasoft/devtools/blob/master/catalog-info.yaml`, or a
...
registered location and you'll see your entities start to appear in Devtools.
...
site admins or by individual Devtools users. Their purpose is to reference some
...
# Example location
type: url
target: https://github.com/khulnasoft/devtools/blob/master/catalog-info.yaml
```
...
added by the organization that adopts Devtools.
...
[example backend here](https://github.com/khulnasoft/devtools/blob/master/packages/backend-legacy/src/plugins/catalog.ts).
We will create a new
[`CatalogProcessor`](https://github.com/khulnasoft/devtools/blob/master/plugins/catalog-node/src/api/processor.ts)
subclass that can be added to this catalog builder.
...
Sometimes you might already have files in GitHub or some provider that Devtools already supports but the metadata format that you use is not the same as `catalog-info.yaml` files. In this case you can implement a custom parser that can read the files and convert them on-the-fly to the `Entity` format that Devtools expects, and it will integrate seamlessly into Catalog so that you can use things like the `GithubEntityProvider` to read these files.
...
id: my-service
type: service
author: user@devtools.com
```

We need to build a custom parser that can read this format and convert it to the `Entity` format that Devtools expects.
...
import { catalogModelExtensionPoint } from '@devtools/plugin-catalog-node/alpha';
import { customEntityDataParser } from '../lib/customEntityDataParser';
...
You can find more details about [why it was created](https://github.com/khulnasoft/devtools/tree/master/plugins/catalog-backend-module-incremental-ingestion#why-did-we-create-it) and its [requirements](https://github.com/khulnasoft/devtools/tree/master/plugins/catalog-backend-module-incremental-ingestion#requirements).

### Installation

1. Install `@devtools/plugin-catalog-backend-module-incremental-ingestion` with `yarn --cwd packages/backend add @devtools/plugin-catalog-backend-module-incremental-ingestion` from the Devtools root directory.
...
backend.add(
  import(
    '@devtools/plugin-catalog-backend-module-incremental-ingestion/alpha'
  ),
);
/* highlight-add-end */
...
For more information on compatibility, refer to the [requirements](https://github.com/khulnasoft/devtools/tree/master/plugins/catalog-backend-module-incremental-ingestion#requirements).
...
```ts
import { IncrementalEntityProvider } from '@devtools/plugin-catalog-backend-module-incremental-ingestion';
...
import { incrementalIngestionProvidersExtensionPoint } from '@devtools/plugin-catalog-backend-module-incremental-ingestion/alpha';
...
For a deep dive into the technical details of the Incremental Entity Provider, see [the README](https://github.com/khulnasoft/devtools/tree/master/plugins/catalog-backend-module-incremental-ingestion).
