...
`@devtools/create-app` template, is to declaratively add locations pointing to
...
You are also not able to use placeholders in them like `$text`, `$json` or `$yaml`. You can however reference other files relative to the current file. See the full [catalog example data set here](https://github.com/khulnasoft/devtools/tree/master/packages/catalog-model/examples) for an extensive example.
...
Devtools's descriptor format. This is documented in
...
source that should be mirrored into Devtools. To make Devtools a mirror of
...
Catalog errors are published to the [events plugin](https://github.com/khulnasoft/devtools/tree/master/plugins/events-node): `@devtools/plugin-events-node`. You can subscribe to events and respond to errors, for example you may wish to log them.

The first step is to add the events backend plugin to your Devtools application. Navigate to your Devtools application directory and add the plugin package.
...
If you want to log catalog errors you can install the `@devtools/plugin-catalog-backend-module-logs` module.

Install the catalog logs module.

```bash title="From your Devtools root directory"
yarn --cwd packages/backend add @devtools/plugin-catalog-backend-module-logs
```
...
```
[1] 2024-06-07T00:00:28.787Z events warn Policy check failed for user:default/guest; caused by Error: Malformed envelope, /metadata/tags must be array entity=user:default/guest location=file:/Users/foobar/code/devtools-demo-instance/examples/org.yaml
```
...
```ts title="packages/backend/src/index.ts"
import { CATALOG_ERRORS_TOPIC } from '@devtools/plugin-catalog-backend';
import {
  coreServices,
  createBackendModule,
} from '@devtools/backend-plugin-api';
import { eventsServiceRef, EventParams } from '@devtools/plugin-events-node';
