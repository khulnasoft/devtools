...
files from here to serve documentation in Devtools. Note that the bucket names
are globally unique.
...
key TechDocs will use to make API calls. Make it available in your Devtools
...
Your Devtools app is now ready to use Google Cloud Storage for TechDocs, to
...
[Terraform example](https://github.com/khulnasoft/devtools/blob/master/contrib/terraform/techdocs-s3-storage/terraform.tf).

TechDocs will publish documentation to this bucket and will fetch files from
here to serve documentation in Devtools. Note that the bucket names are
globally unique.
...
If you are deploying Devtools to Amazon EC2, Amazon ECS, or Amazon EKS, you do
...
techdocs:
  publisher:
    ...
    awsS3:
      ...
      region: ${AWS_REGION}
      credentials:
        roleArn: arn:aws:iam::123456789012:role/my-devtools-role
```
...
Your Devtools app is now ready to use AWS S3 for TechDocs, to store and read
...
here to serve documentation in Devtools. Note that the container names are
globally unique.
...
Your Devtools app is now ready to use Azure Blob Storage for TechDocs, to store
...
Your Devtools app is now ready to use OpenStack Swift Storage for TechDocs, to
