...
TechDocs functions as a plugin to Devtools, so you will need to use Devtools
to use TechDocs.

If you haven't setup Devtools already, start
[here](../../getting-started/index.md).

> If you used `npx @devtools/create-app`, TechDocs may already be present.
>
...
The first step is to add the TechDocs plugin to your Devtools application.
Navigate to your new Devtools application directory. And then to your
`packages/app` directory, and install the `@devtools/plugin-techdocs` package.

```bash title="From your Devtools root directory"
yarn --cwd packages/app add @devtools/plugin-techdocs
```
...
With the [TechDocs Addon framework](https://devtools.khulnasoft.com/docs/features/techdocs/addons#installing-and-using-addons), you can render React components in documentation pages and these Addons can be provided by any Devtools plugin. The framework is exported by the [@devtools/plugin-techdocs-react](https://www.npmjs.com/package/@devtools/plugin-techdocs-react) package and there is a `<ReportIssue />` Addon in the [@devtools/plugin-techdocs-module-addons-contrib](https://www.npmjs.com/package/@devtools/plugin-techdocs-module-addons-contrib) package for you to use once you have these two dependencies installed:
...
Navigate to `packages/backend` of your Devtools app, and install the
`@devtools/plugin-techdocs-backend` package.

```bash title="From your Devtools root directory"
yarn --cwd packages/backend add @devtools/plugin-techdocs-backend
```
...
The final step is to import the techdocs backend plugin in Devtools app
...
That's it! TechDocs frontend and backend have now been added to your Devtools
...
Navigate to `packages/backend` of your Devtools app, and install the `@devtools/plugin-techdocs-backend` package.
...
generating documentation sites. If set to `'external'`, Devtools will assume
...
Amazon S3, or local filesystem of Devtools server.

It is okay to use the local filesystem in a "basic" setup when you are trying
out Devtools for the first time. At a later time, review
...
deploying Devtools using Docker, this will mean that your Devtools Docker
...
[`Dockerfile`](https://github.com/khulnasoft/techdocs-container/blob/main/Dockerfile)
...
[changelog](https://github.com/khulnasoft/mkdocs-techdocs-core#changelog). In
