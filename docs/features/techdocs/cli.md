...
title: TechDocs CLI
# prettier-ignore
description: TechDocs CLI - a utility command line interface for managing TechDocs sites in Devtools.
---

Utility command line interface for managing TechDocs sites in
[Devtools](https://github.com/khulnasoft/devtools).

https://devtools.khulnasoft.com/docs/features/techdocs/

## Features

- Supports local development/preview of a TechDocs site in a Devtools app.
...
Commands:
  ...
  serve [options]           Serve a documentation project locally in a Devtools app-like
                            environment
...
### Preview TechDocs site locally in a Devtools like environment
...
[techdocs-container](https://github.com/khulnasoft/techdocs-container) is used to
...
a Devtools app server on port 3000. The Devtools app has a custom TechDocs API
implementation, which uses the MkDocs preview server as a proxy to fetch the
generated documentation files and assets.

Devtools instances might differ from the provided preview app in appearance and
...
Serve a documentation project locally in a Devtools app-like environment
...
[`@devtools/plugin-techdocs-node`](https://github.com/khulnasoft/devtools/tree/master/plugins/techdocs-node)
package from Devtools for consistency. A Devtools app can also generate and
publish TechDocs sites if `techdocs.builder` is set to `'local'` in
`app-config.yaml`. See
[configuration reference](https://devtools.khulnasoft.com/docs/features/techdocs/configuration).

By default, this command uses Docker and
[techdocs-container](https://github.com/khulnasoft/techdocs-container) to make
...
Options:
  ...
  --techdocs-ref <HOST_TYPE:URL>  The repository hosting documentation source files e.g.
                                  ...
                                  This value is same as the devtools.khulnasoft.com/techdocs-ref annotation of the corresponding
                                  Devtools entity.
...
or (Azure) container which your Devtools app can read from.

The value for `--entity` must be the Devtools entity which the generated
...
Options:
  --publisher-type <TYPE>                                       (Required always) awsS3 | googleGcs | azureBlobStorage | openStackSwift - same as techdocs.publisher.type in Devtools app-config.yaml
...
case was required in the Devtools URL in order to read/render TechDocs content.
...
Devtools instances created with TechDocs `v[0.11.0]` or later do not need this
...
Once migrated and the upgraded version of the Devtools plugin has been
...
- [Google Cloud Storage](https://devtools.khulnasoft.com/docs/features/techdocs/using-cloud-storage#configuring-google-gcs-bucket-with-techdocs)
- [AWS S3](https://devtools.khulnasoft.com/docs/features/techdocs/using-cloud-storage#configuring-aws-s3-bucket-with-techdocs)
- [Azure Blob Storage](https://devtools.khulnasoft.com/docs/features/techdocs/using-cloud-storage#configuring-azure-blob-storage-container-with-techdocs)

## Development

You are welcome to contribute to TechDocs CLI to improve it and support new
features! See the project
[README](https://github.com/khulnasoft/devtools/blob/main/src/packages/techdocs-cli/README.md)
