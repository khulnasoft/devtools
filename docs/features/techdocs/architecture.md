...
When you deploy Devtools (with TechDocs enabled by default), you get a basic
...
When you open a TechDocs site in Devtools, the
...
etc.) to render on the page in TechDocs/Devtools.

The static files consist of HTML, CSS and Images generated by MkDocs. We remove
all the JavaScript before adding them to Devtools for security reasons. And
...
[techdocs-cli](https://github.com/khulnasoft/techdocs-cli) or
[techdocs-container](https://github.com/khulnasoft/techdocs-container) to
...
Currently, we use the Devtools server's (or techdocs-backend's) local file
...
cache store
[supported by Devtools](../../overview/architecture-overview.md#cache).
...
This would also allow us to use the access control management Devtools when
that is ready.
[Track progress here.](https://github.com/khulnasoft/devtools/issues/3218)
...
or do a `git clone` of the Devtools repository. We want the first experience to
_just work magically_ so that you can have your first experience with TechDocs
which is smooth. However, if you decide to deploy Devtools/TechDocs for
...
A: It makes scaling a Devtools instance harder. Think about the case where we
have distributed Devtools deployments (e.g. multiple Kubernetes pods of your
Devtools app). Using a separate/central file storage system for TechDocs is
...
like any other Devtools plugin with a frontend and its backend (catalog,
scaffolder, etc.). If you set your Devtools instance to generate docs on the
