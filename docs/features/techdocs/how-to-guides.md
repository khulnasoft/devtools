...
setup, docs are generated and stored at the server running your Devtools
...
Devtools instance should turn into read-only mode. Read more details and the
...
In your Devtools instance's `app-config.yaml`, set `techdocs.builder` from
...
based on the value of the `devtools.khulnasoft.com/techdocs-ref` annotation defined in the
Entity's `catalog-info.yaml` file. This is also called the
[Prepare](./concepts.md#techdocs-preparer) step.

We strongly recommend that the `devtools.khulnasoft.com/techdocs-ref` annotation in each
...
`mkdocs.yml` file in a subdirectory and update the `devtools.khulnasoft.com/techdocs-ref`
...
e.g.
`url:https://github.com/khulnasoft/devtools/tree/master/plugins/techdocs-backend/examples/documented-component`
...
in Devtools. While a default table experience, similar to the one provided by
...
```tsx
import { TechDocsCustomHome } from '@devtools/plugin-techdocs';
//...
...
import { Content } from '@devtools/core-components';
import {
  CatalogFilterLayout,
  EntityOwnerPicker,
  EntityTagPicker,
...
import { EntityListDocsGrid } from '@devtools/plugin-techdocs';
...
3. **Deploy the updated versions of the TechDocs plugins**: Once the migration
   ...
   and frontend plugins to your Devtools instance.
...
[beta-migrate-bug]:
https://github.com/khulnasoft/devtools/issues/new?assignees=&labels=bug&template=bug_template.md&title=[TechDocs]%20Unable%20to%20run%20beta%20migration
...
[TechDocsStorageApi](https://github.com/khulnasoft/devtools/blob/55114cfeb7045e3e5eeeaf67546b58964f4adcc7/plugins/techdocs/src/api.ts#L33),
which is responsible for talking to TechDocs storage to fetch files to render,
and
[TechDocsApi](https://github.com/khulnasoft/devtools/blob/55114cfeb7045e3e5eeeaf67546b58964f4adcc7/plugins/techdocs/src/api.ts#L49),
...
2. Override the API refs `techdocsStorageApiRef` and `techdocsApiRef` with your
   ...
   [Read more about App APIs](https://devtools.khulnasoft.com/docs/api/utility-apis#app-apis).
...
in Devtools is a tool that can help your users to create new components out of
already configured templates. It comes with a set of default templates to use,
but you can also
[add your own templates](https://devtools.khulnasoft.com/docs/features/software-templates/adding-templates).
...
the
[react-ssr-template](https://github.com/khulnasoft/software-templates/tree/main/scaffolder-templates/react-ssr-template)
...
If your Devtools instance does not have internet access, the generation will fail. TechDocs tries to download the Roboto font from Google. You can disable it by adding the following lines to mkdocs.yaml:
...
This way, custom element like `<devtools-element attribute1="value"></devtools-element>` will be allowed in the result HTML.
...
Note: To refer external diagram files, we need to include the diagrams directory in the path. Please refer [`Dockerfile`](https://github.com/khulnasoft/techdocs-container/blob/main/Dockerfile) for details.

## How to add Mermaid support in TechDocs

There are a few options for adding Mermaid support in TechDocs: using [Kroki](https://kroki.io) or [markdown-inline-mermaid](https://github.com/johanneswuerbach/markdown-inline-mermaid) to generate the diagrams at build time, or the [`devtools-plugin-techdocs-addon-mermaid`](https://github.com/johanneswuerbach/devtools-plugin-techdocs-addon-mermaid) plugin to generate the diagram in the browser. We currently use `devtools-plugin-techdocs-addon-mermaid` plugin for the [Mermaid example on the Demo site](https://demo.devtools.khulnasoft.com/docs/default/component/devtools-demo/examples/mermaid/).
...
### Using the `devtools-plugin-techdocs-addon-mermaid` plugin

Please follow the [Getting Started](https://github.com/johanneswuerbach/devtools-plugin-techdocs-addon-mermaid?tab=readme-ov-file#getting-started) instructions in the plugin's README.
...
to entry for onboarding users to Devtools. However, a purely local TechDocs
build restricts TechDocs creators to using the tooling provided in Devtools,
as well as the plugins and features provided in the Devtools-included `mkdocs`
installation.
...
1. In your Devtools instance's `app-config.yaml`, set `techdocs.builder` to
   `'local'`. This ensures that Devtools will build docs for users who want the
   'out-of-the-box' experience.
2. Configure external storage of TechDocs as normal for a production deployment.
   This allows Devtools to publish documentation to your storage, as well as
...
} from '@devtools/plugin-techdocs-node';
        ...
        const docsBuildStrategy: DocsBuildStrategy = {
          shouldBuild: async params =>
            params.entity.metadata?.annotations?.[
              'demo.devtools.khulnasoft.com/techdocs-builder'
            ] === 'local',
        };
...
You may need to add the `@devtools/plugin-techdocs-node` package to your backend `package.json` if it's not been imported already.

:::

## How to use other mkdocs plugins?

The default plugin [mkdocs-techdocs-core](https://github.com/khulnasoft/mkdocs-techdocs-core) provides a set of plugins that can be viewed as the minimum required plugins to enable TechDocs. Your organization might have needs beyond the core set though, here is the recommended way to enable other plugins.
...
Create a new Docker image that extends [spotify/techdocs](https://github.com/khulnasoft/techdocs-container), roughly:
...
Then publish the image and use it in your config under the `techdocs.generator.dockerImage` [key](https://github.com/khulnasoft/techdocs-container).
...
To make a mkdocs plugin available for all your TechDocs components you can either list it in the `techdocs.generator.mkdocs.defaultPlugins` [config](https://github.com/khulnasoft/devtools/blob/master/plugins/techdocs-backend/config.d.ts#L64C14-L64C14), or use the `--defaultPlugin` [cli option](https://devtools.khulnasoft.com/docs/features/techdocs/cli#generate-techdocs-site-from-a-documentation-project) depending on your setup.
...
In this case you can add the `devtools.khulnasoft.com/techdocs-entity` annotation and point to the owners `entityRef` and use its TechDocs. This allows the Subcomponents to read the parents docs, filling the TechDocs link on the `AboutCard` element and the Techdocs tab
...
metadata:
  ...
  annotations:
    devtools.khulnasoft.com/techdocs-entity: system:default/example
```
