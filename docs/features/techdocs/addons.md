...
and render data using normal Devtools or native hooks, APIs, and components.
...
- `Component`: A [proposed-but-not-yet-implemented](https://github.com/khulnasoft/devtools/issues/11109)
...
The installation and configuration of Addons happens within a Devtools app's
...
To start using Addons you need to add the `@devtools/plugin-techdocs-module-addons-contrib` package to your app. You can do that by running this command from the root of your project: `yarn --cwd packages/app add @devtools/plugin-techdocs-module-addons-contrib`

Addons can be installed and configured in much the same way as extensions for
other Devtools plugins: by adding them underneath an extension registry
...
import { TechDocsReaderPage } from '@devtools/plugin-techdocs';
...
import { EntityLayout } from '@devtools/plugin-catalog';
...
| ---------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`<ExpandableNavigation />`](https://devtools.khulnasoft.com/docs/reference/plugin-techdocs-module-addons-contrib.expandablenavigation) | `@devtools/plugin-techdocs-module-addons-contrib` | Allows TechDocs users to expand or collapse the entire TechDocs main navigation, and keeps the user's preferred state between documentation sites.                                                                                                                                                                                                                  |
| [`<ReportIssue />`](https://devtools.khulnasoft.com/docs/reference/plugin-techdocs-module-addons-contrib.reportissue)                   | `@devtools/plugin-techdocs-module-addons-contrib` | Allows TechDocs users to select a portion of text on a TechDocs page and open an issue against the repository that contains the documentation, populating the issue description with the selected text according to a configurable template.                                                                                                                        |
| [`<TextSize />`](https://devtools.khulnasoft.com/docs/reference/plugin-techdocs-module-addons-contrib.textsize)                         | `@devtools/plugin-techdocs-module-addons-contrib` | This TechDocs addon allows users to customize text size on documentation pages, they can select how much they want to increase or decrease the font size via slider or buttons. The default value for font size is 100% and this setting is kept in the browser's local storage whenever it is changed.                                                             |
| [`<LightBox />`](https://devtools.khulnasoft.com/docs/reference/plugin-techdocs-module-addons-contrib.lightbox)                         | `@devtools/plugin-techdocs-module-addons-contrib` | This TechDocs addon allows users to open images in a light-box on documentation pages, they can navigate between images if there are several on one page. The image size of the light-box image is the same as the image size on the document page. When clicking on the zoom icon it zooms the image to fit in the screen (similar to `background-size: contain`). |
...
import { useShadowRootElements } from '@devtools/plugin-techdocs-react';
...
Install `@devtools/plugin-techdocs-addons-test-utils` as a `devDependency` in
...
import { TechDocsAddonTester } from '@devtools/plugin-techdocs-addons-test-utils';
