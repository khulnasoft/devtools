...
A core feature of Devtools is the extensibility via plugins. Even most of the core features
...
([some examples](https://github.com/khulnasoft/devtools/issues/3655#issuecomment-758166746)).
...
`packages/` directory is reserved for core package of Devtools.
...
We prefix the package names with `@devtools/plugin-`.

This structure is based on a
[suggestion in issue #3655](https://github.com/khulnasoft/devtools/issues/3655#issuecomment-758166746).
...
- `packages/techdocs-common` which should be moved to `plugins/techdocs-node`
  and named `@devtools/plugin-techdocs-node`.
- `packages/catalog-client` which will be part of a future
  `plugins/catalog-common` and named `@devtools/plugin-catalog-common`.
