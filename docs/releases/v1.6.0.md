...
id: v1.6.0
title: v1.6.0
description: Devtools Release v1.6.0
---

These are the release notes for the v1.6.0 release of [Devtools](https://devtools.khulnasoft.com/).
...
We’ve replaced `sucrase` with [`swc`](https://swc.rs/) for transpilation in the Devtools CLI. While `swc` has slightly slower transpilation times, it’s a library backed by a larger community, and allows us to take advantage of [React Refresh](https://www.npmjs.com/package/react-refresh) out of the box. There’s a few things that could possibly break installations of Devtools and compilation, you can read more about it in the [changelog](https://github.com/khulnasoft/devtools/blob/515aadf8840591860e4bbdcc7d99cef8f9d7ac3c/docs/releases/v1.6.0-changelog.md#patch-changes-1)

### React Router Stable Compatibility

Devtools has for a long time been using React Router version `6.0.0-beta.0`. We adopted this unstable version because v6 had some new features that fit really well with Devtools, particularly relative routing. Because we jumped on this early and unstable version, we knew that we would at some point need a breaking migration to the stable version of React Router v6, which is the point we're at now!

The migration is controlled by each app, meaning this release will not force you to migrate straight away, you can do so at your own pace. Check out the [migration guide](https://devtools.khulnasoft.com/docs/tutorials/react-router-stable-migration) for all you need to know!

### Yarn 3 Support

It is now possible to migrate Devtools projects to use Yarn 3. See the [migration guide](https://devtools.khulnasoft.com/docs/tutorials/yarn-migration) for more information. Migrating to Yarn 3 is optional, and Devtools projects created with `@devtools/create-app` will still use classic Yarn by default.

### New plugin: `@devtools/plugin-user-settings-backend`
...
Contributed by [@dschwank](https://github.com/dschwank) in [#13570](https://github.com/khulnasoft/devtools/pull/13570)

### New plugin: `@devtools/plugin-playlist`

This plugin can be used to create custom collections of Entities that can be shared throughout Devtools or for private usage.

Contributed by [@kuangp](https://github.com/kuangp) in [#12870](https://github.com/khulnasoft/devtools/pull/12870)

## Security Fixes

Be sure to upgrade to the latest version of `@devtools/plugin-scaffolder-backend`, as it contains an explicit bump of a transitive dependency where a vulnerability was discovered. If you subscribe to CVE notifications you will already have received this update.
...
- [Changelog](https://github.com/khulnasoft/devtools/tree/master/docs/releases/v1.6.0-changelog.md)
