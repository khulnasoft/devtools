---
id: using-devtools-proxy-within-plugin
title: Using the Devtools Proxy from Within a Plugin
# prettier-ignore
description: Guide on how to create a set of API bindings that interface with a backend via the devtools proxy
---

This guide walks you through setting up a simple proxy to an existing API that
is deployed externally to devtools and sending requests to that API from within
a devtools frontend plugin.

If your plugin requires access to an API, devtools offers
...
1. you can configure devtools to proxy to an already existing API.

**Table of Contents**

- [Setting up the devtools proxy](#setting-up-the-devtools-proxy)
- [Calling an API using the devtools proxy](#calling-an-api-using-the-devtools-proxy)
...
# Setting up the devtools proxy

Let's say your plugin's API is hosted at _https://api.myawesomeservice.com/v1_,
and you want to be able to access it within devtools at
...
`backend-url` is referenced is because the devtools backend creates and runs
the proxy. Devtools is structured in such a way that you could run the
devtools frontend independently of the backend. So when calling your API you
...
- register an [`ApiFactory`](../reference/core-plugin-api.apifactory.md) with
  ...
  implementation and tell devtools how to instantiate it
...
Here is an example definition for this API following devtools's `apiRef` style:
...
The `myAwesomeApiRef` is what you will use within devtools to reference the API
...
import { DiscoveryApi, FetchApi } from '@devtools/core-plugin-api';
...
Now you should be able to access your API using the devtools hook
