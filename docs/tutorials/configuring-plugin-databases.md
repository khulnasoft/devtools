...
# prettier-ignore
description: Guide on how to configure Devtools databases.
---

This guide covers a variety of production persistence use cases which are
supported out of the box by Devtools. The database manager allows the developer
...
By default, Devtools uses automatically created databases for each plugin whose
names follow the `devtools_plugin_<pluginId>` pattern, e.g.
`devtools_plugin_auth`. You can configure a different database name prefix for
...
Devtools supports all of these use cases with the `DatabaseManager` provided by
`@devtools/backend-common`. We will now cover how to use and configure
Devtools's databases.
...
generated one which would have been `devtools_plugin_auth`.
...
instead of `devtools_plugin_`. Plugins such as `auth` and `catalog` will use
...
As Devtools attempts to check if the database exists, you may need to grant
