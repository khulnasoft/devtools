...
This page lists all commands provided by the Devtools CLI, what they're for,
and where to use them.

## help

This command displays a help summary or detailed help screens for each command.
Below is a cleaned up output of `yarn devtools-cli --help`:
...
repo [command]                                 Command that run across an entire Devtools project
package [command]                              Lifecycle scripts for individual packages
migrate [command]                              Migration utilities
versions:bump [options]                        Bump Devtools packages to the latest versions
...
The `package` command category, `yarn devtools-cli package --help`:
...
The `repo` command category, `yarn devtools-cli repo --help`:
...
The `migrate` command category, `yarn devtools-cli migrate --help`:
...
package-lint-configs  Migrates all packages to use @devtools/cli/config/eslint-factory
...
Options:
  ...
  --successCacheDir <path>  Set the success cache location, (default: node_modules/.cache/devtools-cli)
...
If you want to avoid re-running tests that have not changed since the last successful run in CI, you can use the `--successCache` flag. By default this cache is stored in `node_modules/.cache/devtools-cli`, but you can choose a different directory with the `--successCacheDir <path>`.
...
```text
Usage: devtools-cli package lint [options]
...
that includes all packages that have `devtools-cli test` in their package
`test` script.
...
```text
Usage: devtools-cli package test [options]

Run tests, forwarding args to Jest, defaulting to watch mode

Options:
  --devtools-cli-help    display help for command
```
...
```bash
devtools-cli new --select plugin --option id=foo
```
...
{
  "scripts": {
    "new": "devtools-cli new --scope internal --no-private --npm-registry https://acme.org/npm"
  }
}
```

```text
Usage: devtools-cli create [options]
...
```bash
yarn devtools-cli config:print --frontend --package my-app
```
...
Bump all `@devtools` packages to the latest versions. This checks for updates
in the package registry, and will update entries `package.json` files when necessary. See more how this command can be configured and used [for keeping Devtools updated](../../getting-started/keeping-devtools-updated.md).
...
Options:
  ...
  --release <version|next|main> Bump to a specific Devtools release line or version (default: "main")
```
...
```text
Usage: devtools-cli build-workspace [options] <workspace-dir>
```
...
[GitHub Apps for Devtools Authentication](../../integrations/github/github-apps.md).
...
```text
Usage: devtools-cli create-github-app <github-org>
```

## info

Outputs debug information which is useful when opening an issue. Outputs system
information, node.js and npm versions, CLI version and type (inside devtools
repo or a created app), all `@devtools/*` package dependency versions.

```text
Usage: devtools-cli info
```
