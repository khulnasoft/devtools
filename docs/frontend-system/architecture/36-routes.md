...
import React from 'react';
import { useRouteRef } from '@devtools/frontend-plugin-api';
...
import React from 'react';
import { useRouteRefParams } from '@devtools/frontend-plugin-api';
import { detailsRouteRef } from '../routes';
...
```tsx title="plugins/catalog/src/routes.ts"
import { createExternalRouteRef } from '@devtools/frontend-plugin-api';
...
import catalog from '@devtools/plugin-catalog';
import scaffolder from '@devtools/plugin-scaffolder';
...
Another thing to note is that this indirection in the routing is particularly useful for open source plugins that need to provide flexibility in how they are integrated. For plugins that you build internally for your own Devtools application, you can choose to use direct imports or even concrete route path strings directly. Although there can be some benefits to using the full routing system even in internal plugins: it can help you structure your routes, and as you will see further down it also helps you manage route parameters.
