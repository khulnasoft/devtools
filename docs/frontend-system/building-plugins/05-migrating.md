...
In order to migrate the actual definition of the plugin you need to recreate the plugin using the new `createFrontendPlugin` utility exported by `@devtools/frontend-plugin-api`.
...
Pages that were previously created using the `createRoutableExtension` extension function can be migrated to the new Frontend System using the `PageBlueprint` [extension blueprint](../architecture/23-extension-blueprints.md), exported by `@devtools/frontend-plugin-api`.
...
```tsx
import { PageBlueprint } from '@devtools/frontend-plugin-api';
import {
  compatWrapper,
...
Note at the top of the file that it uses the updated import from `@devtools/frontend-plugin-api` that we migrated in the previous section, instead of the old `@devtools/core-plugin-api`.
...
import { storageApiRef, createApiFactory } from '@devtools/core-plugin-api';
...
- Change the old `@devtools/core-plugin-api` imports to the new `@devtools/frontend-plugin-api` package as per the top section of this guide
