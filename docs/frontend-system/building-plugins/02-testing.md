...
Utilities for testing frontend features and components are available in `@devtools/frontend-test-utils`.
...
Use the `renderInTestApp` helper to render a given component inside a Devtools test app:

```tsx
import React from 'react';
import { screen } from '@testing-library/react';
import { renderInTestApp } from '@devtools/frontend-test-utils';
...
import { stringifyEntityRef } from '@devtools/catalog-model';
...
This pattern also works for many other context providers. An important example is the `EntityProvider` from the `@devtools/plugin-catalog-react` package, which you can use to provide a mocked entity context to the component.

## Testing extensions

To facilitate testing of frontend extensions, the `@devtools/frontend-test-utils` package provides a tester class which starts up an entire frontend harness, complete with a number of default features. You can then provide overrides for extensions whose behavior you need to adjust for the test run.
...
If there's anything else you think needs to be covered in the docs or that you think isn't covered by the test utilities, please create an issue in the Devtools repository. You are always welcome to contribute as well!
