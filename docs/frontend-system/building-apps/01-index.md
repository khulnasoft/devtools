...
To get set up quickly with your own Devtools project you can create a Devtools App.

A Devtools App is a monorepo setup that includes everything you need to run Devtools in your own environment.

## Creating a new app

To create a new Devtools app we recommend using the `@devtools/create-app` command line, and the easiest way to run this package is with `npx`:
...
```sh
# The command bellow creates a Devtools App inside the current folder.
...
The starting point of a frontend app is the `createApp` function, which accepts a single options object as its only parameter. It is imported from `@devtools/frontend-defaults`, which is where you will find most of the common APIs for building apps.
...
import catalogPlugin from '@devtools/plugin-catalog/alpha';
...
Visit the [built-in extensions](#customize-or-override-built-in-extensions) section to see what is installed by default in a Devtools application.

## Configure your app

### Bind external routes

Linking routes from different plugins requires this configuration. You can do this either through a configuration file or by coding, visit [this](https://devtools.khulnasoft.com/docs/frontend-system/architecture/routes#binding-external-route-references) page for instructions.

### Enable feature discovery

Use this setting to enable experimental feature discovery when building your app with `@devtools/cli`. With this configuration your application tries to discover and install package extensions automatically, check [here](../architecture/10-app.md#feature-discovery) for more details.
...
In case you need to perform asynchronous operations before passing features to the `createApp` function, define a [feature loader](https://devtools.khulnasoft.com/docs/reference/frontend-defaults.createappfeatureloader/) object and pass it to the `features` option:
