...
description: An introduction to Devtools user identities and sign-in resolvers in the old backend system
---
...
be mapped to user identities within Devtools.
...
Devtools projects created with `npx @devtools/create-app` come configured with a
...
of this page to understand the Devtools identity system once you need to install
...
A user identity within Devtools is built up from two pieces of information, a
user [entity reference](../features/software-catalog/references.md), and a
set of ownership entity references.
When a user signs in, a Devtools token is generated with these two pieces of information,
which is then used to identify the user within the Devtools ecosystem.
...
The Devtools token that encapsulates the user identity is a JWT. The user entity
...
in with multiple providers always end up with the same Devtools identity.
