...
description: Adding AWS ALB as an authentication provider in Devtools
---

Devtools can be deployed behind [AWS Application Load Balancer](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/introduction.html)
...
auth:
  providers:
    awsalb:
      ...
      signIn:
        resolvers:
          # See https://devtools.khulnasoft.com/docs/auth/aws-alb/provider#resolvers for more resolvers
...
yarn --cwd packages/backend add @devtools/plugin-auth-backend-module-aws-alb-provider
```
...
/* highlight-add-start */
backend.add(import('@devtools/plugin-auth-backend-module-aws-alb-provider'));
/* highlight-add-end */
```
