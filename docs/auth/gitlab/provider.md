...
description: Adding GitLab OAuth as an authentication provider in Devtools
---

The Devtools `core-plugin-api` package comes with a GitLab authentication
...
should point to your Devtools backend auth handler.

1. Set Application Name to `devtools-dev` or something along those lines.
2. The Authorization Callback URL should match the redirect URI set in Devtools.
  ...
  providers:
    gitlab:
      development:
        ...
        signIn:
          resolvers:
            # See https://devtools.khulnasoft.com/docs/auth/gitlab/provider#resolvers for more resolvers
...
- `callbackUrl` (optional): The URL matching the Redirect URI registered when creating your GitLab OAuth App, e.g.
  `https://$devtools.acme.corp/api/auth/gitlab/handler/frame`
...
yarn --cwd packages/backend add @devtools/plugin-auth-backend-module-gitlab-provider
```
