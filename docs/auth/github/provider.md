...
description: Adding GitHub OAuth as an authentication provider in Devtools
---

The Devtools `core-plugin-api` package comes with a GitHub authentication
...
should point to Devtools's frontend, while the `Authorization callback URL`
...
- `callbackUrl` (optional): The callback URL that GitHub will use when
  ...
  `https://your-intermediate-service.com/handler`. Only needed if Devtools is
  not the immediate receiver (e.g. one OAuth app for many devtools instances).
- `signIn`: The configuration for the sign-in process, including the **resolvers**
  ...
  entity in the Devtools catalog (typically a single resolver is sufficient).
