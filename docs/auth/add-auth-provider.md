...
The Devtools application can use various external authentication providers for
...
- [Google](https://github.com/khulnasoft/devtools/blob/master/plugins/auth-backend-module-google-provider/src/authenticator.ts)
- [GitHub](https://github.com/khulnasoft/devtools/blob/master/plugins/auth-backend-module-github-provider/src/authenticator.ts)
- [Okta](https://github.com/khulnasoft/devtools/blob/master/plugins/auth-backend-module-okta-provider/src/authenticator.ts)

### Creating proxy auth based provider

A proxy auth provider is a provider that uses another provider to authenticate for example Google IAP or AWS ALB, please note that those providers are already supported by Devtools.

The implementation is similar to the OAuth provider, but the `authenticator` function is different. There are already some examples on how to implement a proxy provider in the codebase, for example [auth-backend-module-gcp-iap-provider](https://github.com/khulnasoft/devtools/tree/master/plugins/auth-backend-module-gcp-iap-provider) and [auth-backend-module-aws-alb-provider](https://github.com/khulnasoft/devtools/tree/master/plugins/auth-backend-module-aws-alb-provider)
...
But if this module is contributed directly to Devtools the module would be imported as

```ts
backend.add(import('@devtools/plugin-auth-backend-module-foobar-provider'));
```
