...
sidebar_label: Okta
description: Adding Okta OAuth as an authentication provider in Devtools
---

The Devtools `core-plugin-api` package comes with a Okta authentication
...
4. Fill out the New Web App Integration form:
   - `App integration name`: `Devtools` (or your custom app name)
...
The configuration examples provided above are suitable for local development. For a production deployment, substitute `http://localhost:7007` with the url that your Devtools instance is available at.
  ...
  providers:
    okta:
      development:
        ...
        signIn:
          resolvers:
            # See https://devtools.khulnasoft.com/docs/auth/okta/provider#resolvers for more resolvers
...
yarn --cwd packages/backend add @devtools/plugin-auth-backend-module-okta-provider
```
