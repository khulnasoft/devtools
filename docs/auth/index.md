---
id: index
title: Authentication in Devtools
description: Introduction to authentication in Devtools
---

The authentication system in Devtools serves two distinct purposes: sign-in and
identification of users, as well as delegating access to third-party resources. It is possible to
configure Devtools to have any number of authentication providers, but only
one of these will typically be used for sign-in, with the rest being used to provide
access to external resources.

:::note Note

Identity management and the Sign-In page in Devtools will only block external access when using the new backend system, without setting `backend.auth.dangerouslyDisableDefaultAuthPolicy` in configuration. Even so, the frontend bundle is not protected from external access, protecting it requires the use of the [experimental public entry point](https://devtools.khulnasoft.com/docs/tutorials/enable-public-entry/). You can learn more about this in the [Threat Model](../overview/threat-model.md#operator-responsibilities).

:::

## Built-in Authentication Providers

Devtools comes with many common authentication providers in the core library:
...
components, or just blank space with logic running in the background. In the end, however, it must provide a valid Devtools user identity through the `onSignInSuccess`
callback prop, at which point the rest of the app is rendered.

If you want to, you can use the `SignInPage` component that is provided by `@devtools/core-components`,
...
import { SignInPage } from '@devtools/core-components';
/* highlight-add-end */
...
[Amazon Application Load Balancer](https://github.com/khulnasoft/devtools/blob/master/contrib/docs/tutorials/aws-alb-aad-oidc-auth.md),
...
The Devtools frontend core APIs provide a set of Utility APIs for plugin developers
...
`IdentityApi` exported by `@devtools/core-plugin-api` via the `identityApiRef`.
...
a Devtools token that identifies the user when making authenticated calls within Devtools.

When making calls to backend plugins, we recommend that the `FetchApi` is used, which
is exported via the `fetchApiRef` from `@devtools/core-plugin-api`. The `FetchApi` will
automatically include a Devtools token in the request, meaning there is no need
to interact directly with the `IdentityApi`.

### Accessing Third Party Resources

A common pattern for talking to third-party services in Devtools is
...
backend plugins or separate services. A goal of Devtools is to make it as easy
...
[@devtools/core-plugin-api](../reference/core-plugin-api.md#variables) reference.
...
Devtools uses [Passport](http://www.passportjs.org/) under the hood, which has
...
```ts
import { providers } from '@devtools/plugin-auth-backend';
...
By default, the Devtools authentication backend generates and manages its own signing keys automatically for any issued
Devtools tokens. However, these keys have a short lifetime and do not persist after instance restarts.
