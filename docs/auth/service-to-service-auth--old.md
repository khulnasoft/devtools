...
# prettier-ignore
description: This section describes how to use service to service authentication, both internally within Devtools plugins and towards external services.
---
...
a legitimate Devtools plugin (or other external caller), by requiring a special
type of service-to-service token which is signed with a shared secret.

When enabling this protection on your Devtools backend plugins, for example the
...
In a newly created Devtools app, the backend is setup up to not require any
...
Devtools service-to-service tokens are currently always signed with a single
...
a new token for any outgoing requests towards other Devtools backend plugins.
...
tokens in code that is external to Devtools itself. External callers may even
...
- `sub`: "devtools-server" (only this value supported currently)
