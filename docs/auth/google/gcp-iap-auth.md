...
# prettier-ignore
description: Adding Google Identity-Aware Proxy as an authentication provider in Devtools
---

Devtools allows offloading the responsibility of authenticating users to the
...
It is assumed an IAP is already serving traffic in front of a Devtools instance
...
auth:
  providers:
    gcpIap:
      ...
      signIn:
        resolvers:
          # See https://devtools.khulnasoft.com/docs/auth/google/gcp-iap-auth#resolvers for more resolvers
...
yarn --cwd packages/backend add @devtools/plugin-auth-backend-module-gcp-iap-provider
```
