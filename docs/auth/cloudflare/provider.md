...
description: Adding Cloudflare Access as an authentication provider in Devtools
---
...
Devtools instance configured to serve the frontend app from the backend and is
...
auth:
  providers:
    cfaccess:
      ...
      signIn:
        resolvers:
          # See https://devtools.khulnasoft.com/docs/auth/cloudflare/provider#resolvers for more resolvers
...
yarn --cwd packages/backend add @devtools/plugin-auth-backend-module-cloudflare-access-provider
```
...
/* highlight-add-start */
backend.add(
  import('@devtools/plugin-auth-backend-module-cloudflare-access-provider'),
);
/* highlight-add-end */
```
