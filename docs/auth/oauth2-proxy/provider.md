...
description: Adding OAuth2Proxy as an authentication provider in Devtools
---

The Devtools `@devtools/plugin-auth-backend` package comes with an
...
actual Devtools instance. This enables to reuse existing authentications within
...
OAuth2 Proxy does not provide a way to authenticate requests, you must instead ensure that your Devtools instance is only accessible through the OAuth2 Proxy. If you need more strict validation, consider using a different provider.
...
auth:
  ...
  providers:
    oauth2Proxy:
      signIn:
        resolvers:
          # See https://devtools.khulnasoft.com/docs/auth/oauth2-proxy/provider#resolvers for more resolvers
...
yarn --cwd packages/backend add @devtools/plugin-auth-backend-module-oauth2-proxy-provider
```
