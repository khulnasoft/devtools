...
# prettier-ignore
description: This section describes service to service authentication works, both internally within Devtools plugins and when external callers want to make requests.
---
...
This article describes how _service-to-service auth_ works in Devtools, both
between Devtools backend plugins and for external callers who want to make
...
Devtools plugins that use the new backend system and handle credentials using
...
requests to other Devtools backend plugins, and the receivers use the caller's
...
external callers who want to make requests to Devtools backend plugins. This is
...
header when calling Devtools plugins:
...
instance of Devtools with third-party tools, such as Auth0.
...
deployment if you are using split deployments of Devtools. Then each deployment
...
For legacy Devtools backend plugins, the above configuration is enough. But
...
- `sub`: the exact string "devtools-server"
...
} from '@devtools/backend-defaults/auth';
