...
# prettier-ignore
description: This section describes how Devtools allows plugins to request OAuth Access Tokens and OpenID Connect ID Tokens on behalf of the user, to be used for auth to various third party APIs
---

This section describes how Devtools allows plugins to request OAuth Access
...
services that require authorization via OAuth. Devtools provides standardized
[Utility APIs](../api/utility-apis.md) such as the
[GoogleAuthApi](https://github.com/khulnasoft/devtools/blob/master/packages/core-plugin-api/src/apis/definitions/auth.ts)
for that use-case. Devtools also includes a set of implementations of these
APIs that integrate with the
[auth-backend](https://github.com/khulnasoft/devtools/tree/master/plugins/auth-backend)
...
## OAuth in @devtools/core-app-api and auth-backend

The default OAuth implementation in Devtools is based on an OAuth server-side
...
[auth-backend](https://github.com/khulnasoft/devtools/tree/master/plugins/auth-backend)
and
[DefaultAuthConnector](https://github.com/khulnasoft/devtools/blob/master/packages/core-app-api/src/lib/AuthConnector/DefaultAuthConnector.ts)
in `@devtools/core-app-api`.
...
Devtools page via `postMessage`. If the OAuth provider implements offline
