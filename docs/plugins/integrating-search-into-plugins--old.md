...
We will need the interface `DocumentCollatorFactory` from package `@devtools/plugin-search-common`, so let's add it to your plugins dependencies:

```sh
# navigate to the plugin directory
# (for this tutorial our plugin lives in the devtools repo, if your plugin lives in a separate repo you need to clone that first)
...
yarn add @devtools/plugin-search-common
```
...
export class FaqCollatorFactory implements DocumentCollatorFactory {
  ...
  static fromConfig(config: Config, options: FaqCollatorFactoryOptions) {
    const baseUrl =
      ...
      'https://devtools.example.biz/faq-snippets';
