...
description: Measuring usage of your Devtools instance.
---

Setting up, maintaining, and iterating on an instance of Devtools can be a
large investment. To help measure return on this investment, Devtools comes
with an event-based Analytics API that grants app integrators the flexibility to
collect and analyze Devtools usage in the analytics tool of their choice, while
...
used plugin in my Devtools instance") to be answered.
...
[ga]: https://github.com/khulnasoft/community-plugins/blob/main/workspaces/analytics/plugins/analytics-module-ga/README.md
[ga4]: https://github.com/khulnasoft/community-plugins/blob/main/workspaces/analytics/plugins/analytics-module-ga4/README.md
[newrelic-browser]: https://github.com/khulnasoft/community-plugins/blob/main/workspaces/analytics/plugins/analytics-module-newrelic-browser/README.md
[qm]: https://github.com/quantummetric/analytics-module-qm/blob/main/README.md
[matomo]: https://github.com/khulnasoft/community-plugins/blob/main/workspaces/analytics/plugins/analytics-module-matomo/README.md
[add-tool]: https://github.com/khulnasoft/devtools/issues/new?assignees=&labels=plugin&template=plugin_template.md&title=%5BAnalytics+Module%5D+THE+ANALYTICS+TOOL+TO+INTEGRATE
[int-howto]: #writing-integrations
[analytics-api-type]: https://devtools.khulnasoft.com/docs/reference/core-plugin-api.analyticsapi
[generic-http]: https://github.com/pfeifferj/devtools-plugin-analytics-generic/blob/main/README.md
...
| `create`    | The `name` of the software being created; if no `name` property is requested by the given Software Template, then the string `new {templateName}` is used instead. | The context holds an `entityRef`, set to the template's ref (e.g. `template:default/template-name`). The `value` represents the number of minutes saved by running the template (based on the template's `devtools.khulnasoft.com/time-saved` annotation, if available).                                       |
...
If there is an event you'd like to see captured, please [open an issue](https://github.com/khulnasoft/devtools/issues/new?assignees=&labels=enhancement&template=feature_template.md&title=[Analytics%20Event]:%20THE+EVENT+TO+CAPTURE) describing the event you want to see and the questions it
...
Analytics event forwarding is implemented as a Devtools utility API. Just as
...
`@devtools/analytics-module-[name]`, and any configuration should be keyed
...
- Use the `userEntityRef` resolved by `identityApi`'s `getDevtoolsIdentity()`
...
class AcmeAnalytics implements AnalyticsApi {
  private constructor(accountId: number, identityApi?: IdentityApi) {
    if (identityApi) {
      identityApi.getDevtoolsIdentity().then(identity => {
        AcmeAnalytics.init(accountId, {
...
using the `useAnalytics()` hook provided by `@devtools/core-plugin-api`. The
...
```tsx
import { AnalyticsContext, useAnalytics } from '@devtools/core-plugin-api';
...
Devtools core (`pluginId`, `extension`, and `routeRef`) have been omitted. In
...
The `@devtools/test-utils` package includes a `MockAnalyticsApi` implementation
...
import { analyticsApiRef } from '@devtools/core-plugin-api';
import {
  MockAnalyticsApi,
  TestApiProvider,
  wrapInTestApp,
