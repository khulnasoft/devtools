...
# prettier-ignore
description: Documentation for the Devtools plugin composability APIs.
---

## Summary

This page describes the composability system that helps bring together content
from a multitude of plugins into one Devtools application.
...
export type Extension<T> = {
  expose(plugin: DevtoolsPlugin): T;
};
```
...
creation function, the Devtools core is able to hook into the process of
exposing the extension outside the plugin, and in the end the app controls the
usage of the extension.

The Devtools core API currently provides two different types of extension
...
`createScaffolderFieldExtension` from `@devtools/plugin-scaffolder`. Extensions
...
The routing system of Devtools relies heavily on the composability system. It
...
integrated. For plugins that you build internally for your own Devtools
...
Following the `1.28` release of Devtools you can now define default targets for
...
to render in different scenarios, the `@devtools/catalog` plugin provides an
...
The `@devtools/catalog` plugin provides a couple of built-in conditions,
