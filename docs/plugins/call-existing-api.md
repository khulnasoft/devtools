...
# prettier-ignore
description: Describes the various options that Devtools frontend plugins have, in communicating with service APIs that already exist
---

This article describes the various options that Devtools frontend plugins have,
...
- The request/response patterns of the API match real world usage needs in
  Devtools frontend plugins. For example, if the end use case is to show a
  small summary in Devtools, but the only available API endpoint gives a 30
...
## Using The Devtools Proxy

Devtools has an optional proxy plugin for the backend, that can be used to
...
a matter of just adding a few lines of Devtools config to get the benefit of
...
- You already have the Devtools backend itself exposed through your perimeter
  and find it practical to have only one entry point to deal with, governing
  ingress with just the Devtools config.

## Creating a Devtools Backend Plugin

Much like the Devtools frontend, the Devtools backend also has a plugin
...
[the user-settings plugin backend](https://github.com/khulnasoft/devtools/tree/master/plugins/user-settings-backend)
...
for a purpose, and when to make a Devtools backend plugin that adapts something
