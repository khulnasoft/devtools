...
Devtools plugins provide features to a Devtools App.
...
- Prefer using the [Devtools components](https://devtools.khulnasoft.com/storybook),
  otherwise go with [Material UI](https://material-ui.com/).
- Check out the shared Devtools APIs before building a new one.
...
```tsx
/* src/routes.ts */
import { createRouteRef } from '@devtools/core-plugin-api';
...
extension using `createRoutableExtension` from `@devtools/core-plugin-api`:

```tsx
/* src/plugin.ts */
import { createPlugin, createRouteRef } from '@devtools/core-plugin-api';
