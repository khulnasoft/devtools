...
id: utility-apis
title: Utility APIs
description: Devtools Utility APIs
---

## Introduction

Devtools plugins strive to be self-contained, with as much functionality as possible residing within the plugin itself and its backend APIs. There will, however, always be a need for plugins to communicate outside of their boundaries, both with other plugins and with the app itself.

Devtools provides two primary methods for plugins to communicate across their
...
by [`@devtools/core-plugin-api`](../reference/core-plugin-api.md). There are also
many predefined Utility APIs in
[`@devtools/core-plugin-api`](../reference/core-plugin-api.md), and they're all
...
[`@devtools/core-plugin-api`](../reference/core-plugin-api.md), or the
...
import { useApi, errorApiRef } from '@devtools/core-plugin-api';
...
from any component inside Devtools, including the ones in
[`@devtools/core-plugin-api`](../reference/core-plugin-api.md). The only
...
The responsibility for adding Utility APIs to a Devtools app lies in three
different locations: the Devtools core library, each plugin included in the
app, and the app itself.

### Core APIs

Starting with the Devtools core library, it provides implementations for all of
the core APIs. The core APIs are the ones exported by
[`@devtools/core-plugin-api`](../reference/core-plugin-api.md), such as the
...
[`@devtools/core-plugin-api`](../reference/app-defaults.md), which means that
...
`App.tsx`. See the
[example app in this repo](https://github.com/khulnasoft/devtools/blob/master/packages/app/src/apis.ts)
for an example.
...
[`@devtools/core-plugin-api`](../reference/core-plugin-api.md). Also, be sure to
...
[`@devtools/test-utils`](../reference/test-utils.md). It will provide mocked
...
[`@devtools/dev-utils`](../reference/dev-utils.md), where the exported
