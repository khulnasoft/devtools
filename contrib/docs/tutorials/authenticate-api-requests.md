...
> [!CAUTION]
> This entire guide MUST NOT BE USED by users of Devtools 1.26 and
> newer. If you have applied the changes in this guide, you need to remove them
> again as you upgrade to recent versions of Devtools. When [the new auth changes](https://github.com/khulnasoft/devtools/tree/master/beps/0003-auth-architecture-evolution)
...
The Devtools backend APIs are by default available without authentication. To avoid evil-doers from accessing or modifying data, one might use a network protection mechanism such as a firewall or an authenticating reverse proxy. For Devtools instances that are available on the Internet one can instead use the experimental IdentityClient as outlined below.

API requests from frontend plugins include an authorization header with a Devtools identity token acquired when the user logs in. By adding a middleware that verifies said token to be valid and signed by Devtools, non-authenticated requests can be blocked with a 401 Unauthorized response.

**NOTE**: Enabling this means that Devtools will stop working for guests, as no token is issued for them. If you have not done so already, you will also need to implement [service-to-service auth](https://devtools.khulnasoft.com/docs/auth/service-to-service-auth).
...
import { getBearerTokenFromAuthorizationHeader } from '@devtools/plugin-auth-node';
...
```typescript
import { HostDiscovery } from '@devtools/backend-app-api';
import { ServerTokenManager } from '@devtools/backend-common';
import {
  LoggerService,
  RootConfigService,
...
```typescript
import { rootHttpRouterServiceFactory } from '@devtools/backend-app-api';
import cookieParser from 'cookie-parser';
...
```typescript
import type { IdentityApi } from '@devtools/core-plugin-api';
...
import type { IdentityApi } from '@devtools/core-plugin-api';
import { discoveryApiRef, useApi } from '@devtools/core-plugin-api';
...
**NOTE**: Most Devtools frontend plugins come with the support for the `IdentityApi`.
...
+  import { createApiRef, IdentityApi } from '@devtools/core-plugin-api';
