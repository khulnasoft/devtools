...
Our next steps before switching to the Flightcontrol dashboard is to create a custom app-config.production file. The purposes of this is to update how Devtools will connect to the RDS hosted Postgres DB. The default `app-config.production` file uses the `host`, `port`, `user` and `password` environment variables.
...
Within your Devtools project create a new file called `app-config.production.flightcontrol.yaml`. Add the following configuration to it:
...
catalog:
  ...
  # See https://devtools.khulnasoft.com/docs/features/software-catalog/#adding-components-to-the-catalog for more details
  # on how to get entities into the catalog.
  locations: []
```

Make a note of the environment variable used in the `connectionString`. In our example above this is `POSTGRES_CON_STRING`. You will need this later when you have deployed the database via the Flightcontrol console, and wish to connect to it from Devtools.
...
2. Select the GitHub repo for your Devtools project
...
If you need a database or Redis for your Devtools plugins, you can easily add those to your Flightcontrol deployment. For more information, see [the flightcontrol docs](https://www.flightcontrol.dev/docs/guides/flightcontrol/using-code?ref=devtools#redis).
...
- [Flightcontrol Documentation](https://www.flightcontrol.dev/docs?ref=devtools)
- [Troubleshooting](https://www.flightcontrol.dev/docs/troubleshooting?ref=devtools)
